...
class AccountMultiPaymentRegister(models.TransientModel):
    ...
    available_payment_method_ids = fields.Many2many(
        "account.payment.method.line", compute="_compute_payment_method_fields"
    )
    ...
    amount_total = fields.Monetary(
        string="Total Amount", currency_field="currency_id", store=True, compute="_compute_amount_total"
    )
    ...
    @api.depends("register_line_ids.amount_total")
    def _compute_amount_total(self):
        for rec in self:
            rec.amount_total = sum(rec.register_line_ids.mapped("amount_total"))
    ...
    def _onchange_amount(self):
        for record in self:
            # if sum(record.register_line_ids.mapped("amount_residual")) < record.amount:
            #     record.amount = 0
    ...
    def _compute_payment_method_fields(self):
        for wizard in self:
            if wizard.payment_type == "inbound":
                wizard.available_payment_method_ids = wizard.journal_id.inbound_payment_method_line_ids
            else:
                wizard.available_payment_method_ids = wizard.journal_id.outbound_payment_method_line_ids

            wizard.hide_payment_method = (
                len(wizard.available_payment_method_ids) == 1
                and wizard.available_payment_method_ids.payment_method_id.code == "manual"
            )
    ...
    def _compute_payment_method_id(self):
        for wizard in self:
            ...
            if available_payment_methods:
                wizard.payment_method_id = available_payment_methods[0]
            else:
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        if not self.currency_id.is_zero(register_line_id.amount_writeoff):
            payment_vals["write_off_line_vals"] = {
