...
class ImportReplenishment(models.TransientModel):
    _name = "import.replenishment"
    _description = "Import Replenishment"
    ...
    def export_template(self):
        return self.env.ref("bi_import_replenishment.action_export_template").report_action(self, config=False)
    ...
    def import_xls(self):
                ...
                for j in range(0, sheet.ncols):
                    if i == 0:
                        ...
                        if sheet.cell(i, sl_col).value:
                            ...
                            elif excel_heading[j] == "To Order":
                                to_order = sheet.cell(i, j).value
                                if type(to_order) == float:
                                    to_order = int(to_order)
                                values["qty_to_order"] = str(to_order)
        ...
        return {
            "effect": {
                "fadeout": "slow",
                "message": " {} Replenishment Order Imported!".format(str(count)),
