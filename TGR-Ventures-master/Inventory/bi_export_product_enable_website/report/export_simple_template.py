...
class ExportemplateProductWebsiteExport(models.AbstractModel):
    _name = "report.bi_export_product_enable_website.export_simple_product"
    _inherit = "report.report_xlsx.abstract"
    _description = "Export Template"

    def generate_xlsx_report(self, workbook, data, lines):
        worksheet = workbook.add_worksheet("Simple Product Template")
        ...
        if products:
            for product_id in products:
                worksheet.write("A%s" % row, product_id.name if product_id.name else "", content_format)
                worksheet.write("B%s" % row, product_id.default_code if product_id.default_code else "", content_format)
                worksheet.write(
                    "C%s" % row,
