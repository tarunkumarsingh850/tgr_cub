...
class SetuInventoryLedger(models.TransientModel):
    ...
    report_by = fields.Selection(
        [("company_wise", "Company"), ("warehouse_wise", "Warehouse")], "Report by", default="warehouse_wise"
    )
    ...
    def download_report_in_listview(self):
        ...
        tree_view_id_cmpwise = self.env.ref(
            "setu_inventory_ledger_report.setu_inventory_ledger_bi_report_tree_cmpwise"
        ).id
        ...
        viewmode = "tree,form"
        return {
            "name": _("Invetory Ledger Report - {} to {}".format(self.start_date, self.end_date)),
    ...
    def get_products_movements_for_inventory_ledger(self):
        ...
        if self.report_by == "warehouse_wise":
            ...
            query = """
                    Select * from get_products_movements_for_inventory_ledger('{}','{}','{}','{}','{}','{}')
                """.format(
                company_ids,
                products,
                category_ids,
            ...
            query = """
                    Select * from get_products_movements_for_inventory_ledger_cmpwise('{}','{}','{}','{}','{}')
                """.format(
                company_ids,
                products,
                category_ids,
...
class SetuInventoryLedgerBIReport(models.TransientModel):
    ...
    def _calculate_ledger_details(self):
        ...
        if self.wizard_id.report_by == "company_wise":
            ...
            location_ids = self.env["stock.location"].search([("id", "child_of", warehouse.view_location_id.id)]).ids
            domain1 = ["|", ("location_id", "in", location_ids), ("location_dest_id", "in", location_ids)]
    ...
    def _amount_all(self):
        for movement in self:
            movement.total_in = (
                movement.purchase
                + movement.sales_return
                + movement.internal_in
                + movement.adjustment_in
                + movement.production_in
                + movement.transit_in
            )
            movement.total_out = (
                movement.purchase_return
                + movement.sales
                + movement.internal_out
                + movement.adjustment_out
                + movement.production_out
                + movement.transit_out
            )
