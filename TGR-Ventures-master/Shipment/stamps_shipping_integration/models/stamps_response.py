...
from odoo.addons.stamps_shipping_integration.models.utils import get_dom_tree, python_2_unicode_compatible
import json
...
class ResponseDataObject:
    ...
    def _setattr(self, name, value, datetime_nodes):
        if name.lower() in datetime_nodes:
            try:
                ts = "{} {}".format(value.partition("T")[0], value.partition("T")[2].partition(".")[0])
...
class Response:
    ...
    def _etree_to_dict(self, t):
        ...
        if children:
            ...
            d = {t.tag: {k: self._pullval(v) for k, v in list(dd.items())}}
