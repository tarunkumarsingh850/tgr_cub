...
class SaleOrderLine(models.Model):
    ...
    margin = fields.Float("Margin", digits="Product Price", store=True, groups="base.group_user", default=0.00)
    margin_percent = fields.Float(
        "Margin (%)", store=True, groups="base.group_user", group_operator="avg", default=0.00
    )
    purchase_price = fields.Float(
        string="Cost", digits="Product Price", store=True, readonly=False, groups="base.group_user", default=0.00
    )
...
class SaleOrder(models.Model):
    ...
    margin = fields.Monetary("Margin", store=True, default=0.00)
    margin_percent = fields.Float("Margin (%)", store=True, default=0.00)
    ...
    def _action_margin_sale_order(self):
        for rec in self:
            if rec.order_line:
                rec.order_line._compute_purchase_price()
