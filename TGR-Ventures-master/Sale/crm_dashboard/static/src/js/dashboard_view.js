odoo.define('crm_dashboard.CRMDashboard', function (require) {
    ...
    var DashBoard = AbstractAction.extend({
        ...
        events: {
            ...
            'click .view_sales': 'view_sales',
            'click .view_products_sold': 'view_products_sold',
            ...
            'change #sales_values': function (e) {
                e.stopPropagation();
                var $target = $(e.target);
                var value = $target.val();
                if (value == "sales_value_this_year") {
                    this.onclick_sales_value_this_year($target.val());
                } else if (value == "sales_value_this_quarter") {
                    this.onclick_sales_value_this_quarter($target.val());
                } else if (value == "sales_value_this_month") {
                    this.onclick_sales_value_this_month($target.val());
                } else if (value == "sales_value_this_week") {
                    this.onclick_sales_value_this_week($target.val());
                }
            },
            'change #sales_type_values': function (e) {
                e.stopPropagation();
                var $target = $(e.target);
                var value = $target.val();
                if (value == "sales_type_tiger_one") {
                    this.onclick_sales_type_tiger_one($target.val());
                } else if (value == "sales_type_barneys") {
                    this.onclick_sales_type_barneys($target.val());
                } else if (value == "sales_type_fastbuds") {
                    this.onclick_sales_type_fastbuds($target.val());
                }
            },
            ...
            'change #sale_orders_shipped_chart': function (e) {
                e.stopPropagation();
                var $target = $(e.target);
                var value = $target.val();
                if (value == "sales_shipped_last_12months") {
                    this.onclick_sales_shipped_last_12months($target.val());
                } else if (value == "sales_shipped_last_6months") {
                    this.onclick_sales_shipped_last_6months($target.val());
                } else if (value == "sales_shipped_last_month") {
                    this.onclick_sales_shipped_last_month($target.val());
                }
            },
        ...
        willStart: function () {
            ...
            return this._super()
                .then(function () {
                    ...
                    var def14 = self._rpc({
                        model: "sale.order",
                        method: "get_shipped_order_details",
                    })
                        .then(function (res) {
                            self.get_shipped_order_count = res['shipped_order_count'];
                            self.get_shipped_order_revenue = res['shipped_order_revenue'];
                            self.shipped_order_ids = res['shipped_order_ids'];
                        });

                    var def15 = self._rpc({
                        model: "sale.order",
                        method: "get_country_sales_revenue",
                    })
                        .then(function (res) {
                            self.top_country_sales_revenue = res['country_sales_revenue'];
                        });

                    var def16 = self._rpc({
                        model: "sale.order",
                        method: "get_top_sp_invoiced",
                    })
                        .then(function (res) {
                            self.top_salesperson_invoice = res['top_sales_person_invoice'];
                        });

                    var def17 = self._rpc({
                        model: "sale.order",
                        method: "get_top_products_sold",
                    })
                        .then(function (res) {
                            self.top_products_sold = res['top_products_sold'];
                        });

                    var def18 = self._rpc({
                        model: "sale.order",
                        method: "get_top_margin_product_brand",
                    })
                        .then(function (res) {
                            self.top_margin_product_brand = res['top_margin_product_brand'];
                            self.total_margin_product_brand = res['total_margin'];
                            self.total_margin_product_brand_currency = res['currency'];
                        });

                    //                return $.when(def0, def1, def2, def3, def4, def5, def6, def7, def8, def9, def10, def11, def12, def13);
                    return $.when(def0, def1, def2, def3, def4, def5, def6, def8, def9, def10, def11, def12,
                        def13, def14, def15, def16, def17, def18);
                });
        },
        ...
        onclick_this_year: function (ev) {
            ...
            rpc.query({
                ...
                .then(function (result) {
                    ...
                    $('#shipped_sales_count').append('<span>' + result.shipped_sales_count + '</span>');
                })
        },
        ...
        onclick_sales_value_this_year: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_value',
                args: [365],
            })
                .then(function (res) {
                    $('#shipped_order_count').empty();
                    $('#shipped_order_revenue').empty();
                    $('#shipped_order_count').append('<span>' + res.shipped_order_count + '</span>');
                    $('#shipped_order_revenue').append('<span>' + res.shipped_order_revenue + '</span>');
                    self.shipped_order_ids = res['shipped_order_ids'];
                    self.sales_period = 365
                })
        },

        onclick_sales_value_this_quarter: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_value',
                ...
                .then(function (res) {
                    ...
                    $('#shipped_order_revenue').append('<span>' + res.shipped_order_revenue + '</span>');
                    self.shipped_order_ids = res['shipped_order_ids'];
                    self.sales_period = 90
                })
        },

        onclick_sales_value_this_month: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_value',
                ...
                .then(function (res) {
                    ...
                    $('#shipped_order_revenue').append('<span>' + res.shipped_order_revenue + '</span>');
                    self.shipped_order_ids = res['shipped_order_ids'];
                    self.sales_period = 30
                })
        },

        onclick_sales_value_this_week: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_value',
        ...
        onclick_sales_type_barneys: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_type_value',
                args: ["BF"],
            })
                .then(function (res) {
                    $('#shipped_order_count').empty();
        ...
        onclick_sales_type_fastbuds: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_type_value',
        ...
        onclick_sales_type_tiger_one: function (ev) {
            var self = this;
            rpc.query({
                model: 'sale.order',
                method: 'get_sales_type_value',
        ...
        view_sales: function (e) {
            ...
            this.do_action({
                ...
                view_mode: 'tree,form,calendar',
                views: [[false, 'list'], [false, 'form']],
                domain: [['id', 'in', this.shipped_order_ids]],
                target: 'current',
            }, options)
        },

        //view_products_sold
        view_products_sold: function (e) {
            var self = this;
            e.stopPropagation();
            e.preventDefault();
            var options = {
            ...
            this.do_action({
                ...
                view_mode: 'tree',
                views: [[false, 'list']],
                domain: [['order_id.id', 'in', this.shipped_order_ids]],
        ...
        onclick_sales_shipped_last_12months: function (ev) {
            var self = this;
            if (self.is_manager == true) {
                self.initial_render = true;
                rpc.query({
                    model: "sale.order",
                    method: "get_sales_shipped",
                    args: ['12']
                }).then(function (result) {
                    var ctx = document.getElementById("canvas").getContext('2d');
                    // Define the data
        ...
        onclick_sales_shipped_last_6months: function (ev) {
            var self = this;
            self.initial_render = true;
            rpc.query({
                model: "sale.order",
        ...
        onclick_sales_shipped_last_month: function (ev) {
            var self = this;
            self.initial_render = true;
            rpc.query({
                model: "sale.order",
