...
class CRMSalesTeam(models.Model):
    ...
    crm_lead_state_id = fields.Many2one("crm.stage", string="CRM Lead", store=True)
...
class CRMLead(models.Model):
    ...
    crm_manager_id = fields.Many2one("res.users", string="CRM Manager", store=True)
    ...
    def get_the_sales_activity(self):
        """Sales Activity Pie"""
        self._cr.execute(
            """select mail_activity_type.name,COUNT(*) from mail_activity
    ...
    def get_the_annual_target(self):
        ...
        self._cr.execute(
            """SELECT res_users.id,res_users.sales,res_users.sale_team_id,
        ...
        if self.env.user.has_group("sales_team.group_sale_manager"):
            ...
            self._cr.execute(
                """select sum(expected_revenue) from crm_lead where stage_id=4
            ...
            self._cr.execute(
                """SELECT res_users.id,res_users.sales FROM res_users
    ...
    def get_the_source_pie(self):
        ...
        self._cr.execute(
            """select source_id,COUNT(*),(SELECT name FROM utm_source
    ...
    def get_the_medium_pie(self):
        ...
        self._cr.execute(
            """select medium_id,COUNT(*),(SELECT name FROM utm_medium
    ...
    def revenue_count_pie(self):
        ...
        session_user_id = self.env.uid
        self._cr.execute(
            """select sum(expected_revenue) as revenue from crm_lead
        ...
        self._cr.execute(
            """select sum(expected_revenue) as revenue from crm_lead
        where user_id=%s and type='opportunity' and stage_id=4"""
            % session_user_id
        )
    ...
    def get_upcoming_events(self):
        ...
        session_user_id = self.env.uid
        self._cr.execute(
            """select mail_activity.activity_type_id,mail_activity.date_deadline,
    ...
    def get_top_deals(self):
        """Top 10 Deals Table"""
        self._cr.execute(
            """SELECT crm_lead.user_id,crm_lead.id,crm_lead.expected_revenue,
    ...
    def get_monthly_goal(self):
        ...
        leads = self.env["crm.lead"].search(
            [("date_deadline", "!=", False), ("user_id", "=", uid), ("type", "=", "opportunity")]
        )
        leads_won = self.env["crm.lead"].search(
            [("date_closed", "!=", False), ("stage_id", "=", 4), ("user_id", "=", uid), ("type", "=", "opportunity")]
        )
        currency_symbol = self.env.company.currency_id.symbol

        goals = []
        achievement = 0
        for won in leads_won.filtered(lambda a: a.date_closed.month == fields.date.today().month):
    ...
    def get_top_sp_revenue(self):
        ...
        self._cr.execute(
            """SELECT user_id,id,expected_revenue,name,company_id
        from crm_lead where expected_revenue is not null AND user_id = %s
        GROUP BY user_id,id order by expected_revenue DESC limit 10"""
            % user.id
        )
        data1 = self._cr.fetchall()
    ...
    def get_country_revenue(self):
        ...
        self._cr.execute(
            """select country_id, sum(expected_revenue) from crm_lead
    ...
    def get_country_count(self):
        ...
        self._cr.execute(
            """select country_id,count(*) from crm_lead where country_id is not null
    ...
    def get_total_lost_crm(self, option):
        ...
        if option == "1":
            ...
            self._cr.execute(
                """select create_date::date,count(id) from crm_lead
            ...
            test = {"month": list(day_dict.keys()), "count": list(day_dict.values())}
        else:
            month_string = str(int(option)) + " Months"
            self._cr.execute(
                """select extract(month from create_date),count(id)
            ...
            test = {"month": list(month_dict.keys()), "count": list(month_dict.values())}
    ...
    def get_ratio_based_country(self):
        ...
        self._cr.execute(
            """select (select name from res_country where id=country_id),
    ...
    def get_ratio_based_sp(self):
        ...
        self._cr.execute(
            """select user_id,count(user_id) from crm_lead where
        probability=100 and active=true group by user_id order by count(user_id) desc"""
        )
        data_won = self._cr.fetchall()

        self._cr.execute(
            """select user_id,count(user_id) from crm_lead where probability=0
    ...
    def get_ratio_based_sales_team(self):
        ...
        self._cr.execute(
            """select (SELECT name FROM crm_team WHERE crm_team.id = team_id),
    ...
    def get_lost_lead_by_reason_pie(self):
        ...
        self._cr.execute(
            """select lost_reason, count(*), (SELECT name FROM crm_lost_reason
    ...
    def get_lost_lead_by_stage_pie(self):
        ...
        self._cr.execute(
            """select stage_id, count(*),(SELECT name FROM crm_stage
    ...
    def get_count_unassigned(self):
        ...
        count_unassigned = self.env["crm.lead"].search_count([("user_id", "=", False), ("type", "=", "lead")])
    ...
    def lead_details_user(self):
        ...
        self._cr.execute(
            """select COUNT(id) from crm_lead WHERE crm_lead.user_id = '%s'
        ...
        self._cr.execute(
            """select SUM(crm_lead.expected_revenue) from crm_lead WHERE
        ...
        if revenue_value > 0:
            self._cr.execute(
                """select case when b.count_two = 0 then 0 else (
        ...
        self._cr.execute(
            """SELECT active,count(active) FROM crm_lead where type='opportunity'
        ...
        if crm_lead_value == 1:
            self._cr.execute(
                """SELECT id, date_conversion, create_date
    ...
    def crm_year(self):
        ...
        self._cr.execute(
            """select SUM(crm_lead.expected_revenue) from crm_lead
        ...
        self._cr.execute(
            """SELECT active,count(active) FROM crm_lead
        ...
        sale_orders = self.env["sale.order"].search(
            [("state", "in", ["sale", "done"]), ("delivery_status", "=", "done")]
        )
        data_year.append({"shipped_sales_count": len(sale_orders)})
        return data_year
...
class CampaignSmartButton(models.Model):
    ...
    def _compute_ratio(self):
        ...
        total_won = self.env["crm.lead"].search_count(
            [("active", "=", True), ("probability", "=", 100), ("user_id", "=", self.env.uid)]
        )
        total_lose = self.env["crm.lead"].search_count(
            [("active", "=", False), ("probability", "=", 0), ("user_id", "=", self.env.uid)]
        )
...
class SaleOrder(models.Model):
    ...
    @api.model
    def get_shipped_order_details(self):
        sale_orders = self.env["sale.order"].search(
            [("state", "in", ["sale", "done"]), ("delivery_status", "=", "done")]
        )
        amount = round(sum(sale_orders.mapped("amount_total")), 2)
        currency = sale_orders[0].currency_id.symbol
        revenue = f"{amount:,} {currency}"
        return {
            "shipped_order_count": len(sale_orders),
            "shipped_order_revenue": revenue,
            "shipped_order_ids": sale_orders.ids,
        }

    @api.model
    def get_sales_shipped(self, option):
        month_dict = {}
        for i in range(int(option) - 1, -1, -1):
            last_month = datetime.now() - relativedelta(months=i)
            text = format(last_month, "%B")
        ...
        if option == "1":
            ...
            self._cr.execute(
                """select create_date::date,count(id) from sale_order
            where state in ('sale', 'done') and delivery_status='done' and create_date between (now() - interval '1 month') and now()
            group by create_date order by create_date;"""
            )
            data = self._cr.dictfetchall()
            ...
            self._cr.execute(
                """select extract(month from create_date),count(id)
            from sale_order where state in ('sale', 'done') and delivery_status='done' and
            create_date between (now() - interval '%s') and now()
            group by extract(month from create_date) order by extract(
            month from create_date);"""
                % month_string
    ...
    @api.model
    def get_sales_type_value(self, sale_type):
        domain = [("state", "in", ["sale", "done"]), ("delivery_status", "=", "done")]
        if sale_type == "BF":
            domain.append(("is_barneys_dropshipping", "=", True))
        elif sale_type == "FB":
            domain.append(("is_drop_shipping", "=", True))
        elif sale_type == "TO":
            domain.append(("magento_website_id", "!=", False))
    ...
    @api.model
    def get_country_sales_revenue(self):
        """Top 10 Country Wise Revenue - Heat Map"""
        company_id = self.env.company.id
        self._cr.execute(
            """select coun.id, sum(sale_order.amount_total) from sale_order
            join res_partner rp on rp.id = sale_order.partner_id
            join res_country coun on coun.id = rp.country_id
            where sale_order.state in ('sale','done') AND sale_order.delivery_status = 'done'
            group by coun.id order by sum(sale_order.amount_total) desc limit 10"""
        )
        data1 = self._cr.fetchall()
        ...
        return {"country_sales_revenue": country_revenue}

    @api.model
    def get_top_sp_invoiced(self):
        self._cr.execute(
            """select user_id,sum(amount_total) as total
        from sale_order where invoice_status='invoiced' and user_id not in (1,2)
        group by user_id order by total desc limit 10"""
        )
        data1 = self._cr.fetchall()
        ...
        return {"top_sales_person_invoice": sales_person_invoice}

    @api.model
    def get_top_products_sold(self):
        """Top 10 Sales Person by Invoice Table"""
        self._cr.execute(
            """select template.default_code, template.name, sum(line.price_unit*line.product_uom_qty) as total
        from sale_order_line as line
        join sale_order as so on so.id = line.order_id
        ...
        where so.state in ('sale','done') AND so.delivery_status = 'done' AND template.detailed_type = 'product'
        group by template.default_code, template.name order by total desc limit 10"""
        )
        data1 = self._cr.fetchall()
        ...
        for rec in data1:
            ...
            rec_list.append(currency)
            product_list.append(rec_list)

        return {"top_products_sold": product_list}

    @api.model
    def get_top_margin_product_brand(self):
        self._cr.execute(
            """select brand.breeder_name, sum(line.margin) as margin
        from sale_order_line as line
        join sale_order as so on so.id=line.order_id
        join product_product as product on product.id=line.product_id
        join product_template as template on template.id=product.product_tmpl_id
        join product_breeder as brand on brand.id=template.product_breeder_id
        where so.state='sale' and so.delivery_status='done'
        group by brand.breeder_name order by margin desc limit 10"""
        )
        data1 = self._cr.fetchall()
        top_margin_product_brand = []
        total_margin = 0
        for rec in data1:
            currency = self.env.company.currency_id.symbol
            rec_list = list(rec)
            rec_list.append(currency)
            top_margin_product_brand.append(rec_list)
            total_margin += rec[1]

        return {
            "top_margin_product_brand": top_margin_product_brand,
