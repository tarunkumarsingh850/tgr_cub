...
def pre_init_check(cr):
    ...
    if not len(report_excel) or report_excel.state != "installed":
        raise ValidationError(
            'This Module requires the installed module "Report Designer (XLSX, XLSM)". Please install the module!'
        )
    if int("".join([str(100 + int(d)) for d in report_excel.installed_version.split(".")[2:]])) < 101103131:
        raise ValidationError(
            'Module support "Report Designer (XLSX, XLSM)" module starting from Version 1.3.31, found Version {}. Please update the module "Report Designer (XLSX, XLSM)".'.format(
                ".".join(report_excel.installed_version.split(".")[2:])
            )
        )
    return True
