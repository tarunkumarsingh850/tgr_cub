...
class ImportChartAccount(models.TransientModel):
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        group_search = self.env["account.group"].search([("name", "=", field[5])])
                        if not group_search:
                            group_search.create({"name": field[5]})
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        self.create_chart_accounts(values)
        ...
        # ---------------------------------------
            ...
            for row_no in range(sheet.nrows):
                if row_no > 0:
                    ...
                    group_search = self.env["account.group"].search([("name", "=", line[5])])
                    if not group_search:
                        group_search.create({"name": line[5]})
    ...
    def find_group(self, group):
        ...
        if group_search:
            ...
            group_id = group_type.create({"name": group})
            return group_id
