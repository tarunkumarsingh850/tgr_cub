...
class StockValuationDateReport(models.TransientModel, StockValuationCategory):
    ...
    brand_ids = fields.Many2many(comodel_name="product.breeder", string="Product Brand")
    ...
    def onchange_warehouse(self):
        ...
        return {"domain": {"location_id": [("id", "in", location_ids)]}, "value": {"location_id": False}}
    ...
    def print_report(self):
        ...
        return (
            self.env.ref("stock_valuation_on_date.action_stock_valuation_ondate")
    ...
    def _product_detail(self, product_id):
        ...
        variant = ", ".join([v.name for v in product.product_template_attribute_value_ids])
    ...
    @api.model
    def _value_existed(
        self, beginning_inventory, product_qty_in, product_qty_out, product_qty_adjustment, ending_inventory
    ):
        value_existed = False
        if beginning_inventory or product_qty_in or product_qty_out or product_qty_adjustment or ending_inventory:
            value_existed = True
    ...
    def print_xls_report(self):
        ...
        for company in company_ids:
            ...
            sheet.write_merge(
                ...
                0,
                1,
                self.start_date.strftime(DF) + " To " + self.end_date.strftime(DF),
                other_tstyle_cr,
            )
            ...
            if self.only_summary:
                ...
                sheet.write(header_row_start, 4, "Total Adjustment", header_tstyle_c)
                sheet.col(5).width = 256 * 25
                sheet.write(header_row_start, 5, "Total Ending", header_tstyle_c)
                sheet.col(6).width = 256 * 25
                sheet.write(header_row_start, 6, "Total Values", header_tstyle_c)
                row = 9
                (
                    grand_total_qty_in,
                    grand_total_qty_out,
                ...
                for key, values in total_lines.items():
                    ...
                    for line in values:
                        ...
                        unit_cost = self._get_cost(
                            company, line.get("product_id", "") or "", self.end_date, ending_inventory
                        )
                        subtotal_cost = self._get_subtotal_cost(unit_cost, ending_inventory, line)
                    ...
                    sheet.write(row, 0, self._get_categ(key), other_tstyle_c)
                    sheet.write(row, 1, "%.2f" % total_beginning_inventory, other_tstyle_r)
                    sheet.write(row, 2, "%.2f" % total_qty_in, other_tstyle_r)
                    sheet.write(row, 3, "%.2f" % total_qty_out, other_tstyle_r)
                    sheet.write(row, 4, "%.2f" % total_product_qty_adjustment, other_tstyle_r)
                    sheet.write(row, 5, "%.2f" % total_ending_inventory, other_tstyle_r)
                    sheet.write(row, 6, "%.2f" % total_subtotal_cost, other_tstyle_r)
                    row += 1

                sheet.write(row, 0, "Grand Total", other_tstyle_grandc)
                sheet.write(row, 1, "%.2f" % grand_total_beginning_inventory, other_tstyle_grandr)
                sheet.write(row, 2, "%.2f" % grand_total_qty_in, other_tstyle_grandr)
                sheet.write(row, 3, "%.2f" % grand_total_qty_out, other_tstyle_grandr)
                sheet.write(row, 4, "%.2f" % grand_total_product_qty_adjustment, other_tstyle_grandr)
                sheet.write(row, 5, "%.2f" % grand_total_ending_inventory, other_tstyle_grandr)
                sheet.write(row, 6, "%.2f" % grand_total_subtotal_cost, other_tstyle_grandr)
            else:
                header_row_start = 8
                sheet.write(header_row_start, 0, "Category Name ", header_tstyle_c)
                ...
                for values in total_lines.values():
                    for line in values:
                        ...
                        subtotal_cost = self._get_subtotal_cost(unit_cost, ending_inventory, line)
                        product_name, product_barcode, product_code = self._product_detail(
                            line.get("product_id", "") or ""
                        )
                        ...
                        if self._value_existed(
                            ...
                            sheet.write(row, 0, self._get_categ(line.get("categ_id", "") or ""), other_tstyle_c)
                            sheet.write(row, 1, product_name or "", other_tstyle_c)
                            sheet.write(row, 2, product_barcode or "", other_tstyle_c)
                            sheet.write(row, 3, product_code or "", other_tstyle_c)
                            sheet.write(row, 4, "%.2f" % beginning_inventory, other_tstyle_r)
                            sheet.write(row, 5, "%.2f" % product_qty_in, other_tstyle_r)
                            sheet.write(row, 6, "%.2f" % product_qty_out, other_tstyle_r)
                            sheet.write(row, 7, "%.2f" % product_qty_adjustment, other_tstyle_r)
                            sheet.write(row, 8, "%.2f" % ending_inventory, other_tstyle_r)
                            sheet.write(row, 9, "%.2f" % unit_cost, other_tstyle_r)
                            sheet.write(row, 10, "%.2f" % subtotal_cost, other_tstyle_r)
                            row += 1

                sheet.write(row, 3, "Grand Total", other_tstyle_grandc)
                sheet.write(row, 4, "%.2f" % grand_total_beginning_inventory, other_tstyle_grandr)
                sheet.write(row, 5, "%.2f" % grand_total_qty_in, other_tstyle_grandr)
                sheet.write(row, 6, "%.2f" % grand_total_qty_out, other_tstyle_grandr)
                sheet.write(row, 7, "%.2f" % grand_total_product_qty_adjustment, other_tstyle_grandr)
                sheet.write(row, 8, "%.2f" % grand_total_ending_inventory, other_tstyle_grandr)
                sheet.write(row, 9, "-", other_tstyle_grandr)
                sheet.write(row, 10, "%.2f" % grand_total_subtotal_cost, other_tstyle_grandr)
        ...
        res_id = export_obj.create(
            {"file": base64.encodestring(stream.getvalue()), "fname": "Stock Valuation Report.xls"}
        )
        return {
            "type": "ir.actions.act_url",
            "url": "/web/binary/download_document?model=stock.valuation.success.box&field=file&id=%s&filename=Stock Valuation Report.xls"
            % (res_id.id),
            "target": "new",
        }
