...
class StockValuationCategory(models.AbstractModel):
    ...
    def _get_warehouses_block(self, warehouse_ids, company_id):
        ...
        warehouses = "ALL"
        if warehouse_ids:
            warehouse_rec = warehouse_obj.search([("id", "in", warehouse_ids), ("company_id", "=", company_id)])
            if warehouse_rec:
    ...
    def category_wise_value(self, start_date, end_date, locations, brand_ids, filter_product_categ_ids=[]):
        """
        ...
        self._cr.execute(
            """
                        ...
                        FROM product_product pp
                            ...
                            LEFT JOIN product_breeder pb ON (pp.product_breeder_id = pb.id)
                            where pp.product_breeder_id in %s
    ...
    def _remove_zero_inventory(self, values):
        ...
        for rm_zero in values:
            if (
                rm_zero["product_qty_in"] == 0.0
                and rm_zero["product_qty_out"] == 0.0
                and rm_zero["product_qty_adjustment"] == 0.0
            ):
                pass
            else:
    ...
    def find_brands(self, company_id):
        """
        Find all Brands
        """
        return [x.id for x in self.env["product.breeder"].search([])]
    ...
    def _get_lines(self, data, company):
        ...
        # brand
        brand_ids = data["form"] and data["form"].get("brand_ids", []) or []
        if not brand_ids:
            brand_ids = self.find_brands(company)

        # get data from all warehouses.
        records = self.category_wise_value(start_date, end_date, locations, brand_ids, filter_product_categ_ids)
