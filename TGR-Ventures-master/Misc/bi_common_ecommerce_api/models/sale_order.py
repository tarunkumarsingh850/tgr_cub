...
class SaleOrder(models.Model):
    ...
    is_dutch_dropshipping = fields.Boolean(string="Is Dutch Dropshipping", copy=False, default=False)
    

    def _check_dutch_sale_order_data(self, sale_order_data):
        """
    ...
    def _get_odoo_ecommerce_customer(self, customer_data, partner_id):
        """
    ...
    def _check_odoo_partner(self, customer_data, customer_code, warehouse_code):
        """
        @private - Find a partner assigned with the odoo system, otherwise raise an error
        """
        customer_class = self.env["customer.class"].search([("is_dutch_customer", "=", True)], limit=1)
        ...
        if not partner_id:
            ...
            dutch_customers = self.env["res.partner"].search([("customer_class_id", "=", customer_class.id)])
            taken_codes = dutch_customers.mapped("customer_code")
            new_code = self.env["ir.sequence"].next_by_code("dutch.customer")
            ...
            if not partner_id.customer_class_id or not partner_id.customer_class_id.is_dutch_customer:
                return {
                    "error": f"{partner_id.name} ({partner_id.customer_code}) is not a Dutch customer",
                    "status": 400,
                }
            return partner_id
        else:
    ...
    def _prepare_vals_order_lines(self, order_details, company_id, shipping_cost=0):
        """
    ...
    def get_odoo_order_company_id(self, warehouse_code, partner):
        """
    ...
    def _prepare_vals_sale_order_data(self, so_data, so_creation_type):
        """
        ...
        try:
            sale_order_data = []
            for data in so_data:
                partner_id = self._check_odoo_partner(
                    data["invoice_address"], data["customer_code"], data["warehouse_code"]
                )
                if "error" in partner_id:
                    return partner_id
                customer_id = self._get_odoo_ecommerce_customer(data["delivery_address"], partner_id)
                if "error" in customer_id:
                    return customer_id
                company_id = self.get_odoo_order_company_id(data["warehouse_code"], partner_id)
                if "error" in company_id:
                    return company_id
                order_line = self._prepare_vals_order_lines(data["order_details"], company_id, data["shipping_cost"])
                ...
                autoworkflow = self.env["sale.workflow.process.ept"].search(
                ...
                if payment_method_code_record and payment_method_code_record.workflow_id:
                    autoworkflow = payment_method_code_record.workflow_id
                prefix_of_order_name = partner_id.customer_class_id.shortcode
                if not prefix_of_order_name:
                    prefix_of_order_name = "DUTCH"
                sale_order_data.append(
                    {
                        "name": f"{prefix_of_order_name+data.get('order_reference', False)}",
                        "partner_id": partner_id.id,
                        "partner_invoice_id": customer_id.id,
                        "partner_shipping_id": customer_id.id,
                        "order_line": self._prepare_vals_order_lines(
                            data["order_details"], company_id, data["shipping_cost"]
                        ),
                        "company_id": company_id.id,
                        "so_creation_type": so_creation_type,
                        "is_dutch_dropshipping": True,
                        "auto_workflow_process_id": autoworkflow.id,
    ...
    def create_order_ecommerce_sale_order(self, data, so_creation_type="api", file_name=False):
        """
        Create the sale order from the data
        """
        if type(data) is not list:
            data = [data]
        sale_order_data = self._prepare_vals_sale_order_data(data, so_creation_type)
                ...
                for sale_order_id in sale_order_ids:
                    ...
                    if sale_order_id.picking_ids:
                        for picking_id in sale_order_id.picking_ids:
                            picking_id.is_dutch_dropshipping = (
                                sale_order_id.is_dutch_dropshipping and sale_order_id.is_dutch_dropshipping or False
                            )
        ...
        self.env["ecommerce.api.log"].create_log(
            data, response, sale_order_ids, so_creation_type, file_name, is_dutch=True
        )
        return response
    ...
    def cron_create_dutch_sale_orders(self):
        """
        @private - Create the sale order from xml file
        """
        try:
            folder_path = (
                self.env["ir.config_parameter"].sudo().get_param("bi_common_ecommerce_api.dutch_files_folder_path")
            )
            ...
            for filename in files:
                ...
                if "error" in data:
                    ...
                    sale_order_status = self.sudo().create_order_ecommerce_sale_order(
            ...
            _logger.info(error)
            self.env["ecommerce.api.log"].sudo().create_log(
                data=False, response=error, sale_order_ids=False, so_creation_type="", file_name=False, is_dutch=True
            )

    # ====================================================================================================

    def get_order_shipment_tracking_data(self, data):
