...
import operator
import logging
from odoo.addons.web.controllers.main import ExportXlsxWriter, serialize_exception
from odoo.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
import datetime
from odoo import http
from odoo.http import content_disposition, request
from odoo.tools import pycompat
from ..common_lib.ks_date_filter_selections import ks_convert_into_local, ks_get_date
import os
import pytz
                ...
                if isinstance(val, (float, int)):
                    if val >= 0:
                        try:
                            ...
                            ks_precision = 2
                        dataset["data"][ks_count] = (
                            item.env["ir.qweb.field.float"].sudo().value_to_html(val, {"precision": ks_precision})
                        )
