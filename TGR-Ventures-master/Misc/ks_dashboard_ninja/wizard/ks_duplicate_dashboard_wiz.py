...
class KSCreateDashboardWizard(models.TransientModel):
    ...
    def ks_duplicate_record(self):
        ...
        dup_dash = (
            self.env["ks_dashboard_ninja.board"]
            .browse(dashboard_id)
            .copy({"ks_dashboard_top_menu_id": self.ks_top_menu_id.id})
        )
...
class KSDeleteDashboardWizard(models.TransientModel):
    ...
    def DeleteDashBoard(self):
        """this function returns acion id of ks.dashboard.duplicate.wizard"""
        action = self.env["ir.actions.act_window"]._for_xml_id("ks_dashboard_ninja.ks_delete_dashboard_wizard")
        action["context"] = {"dashboard_id": self.id}
        return action
    ...
    def ks_delete_record(self):
        ...
        context = {
            "ks_reload_menu": True,
            "ks_menu_id": self.env["ir.ui.menu"].search([("name", "=", "My Dashboard")])[0].id,
        }
        return {
            "type": "ir.actions.client",
            "name": "Dashboard Ninja",
