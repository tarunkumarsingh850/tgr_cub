...
class KSCreateDashboardWizard(models.TransientModel):
    ...
    ks_top_menu_id = fields.Many2one(
        ...
        required=True,
        default=lambda self: self.env["ir.ui.menu"].search([("name", "=", "My Dashboard")])[0],
    )
    ks_sequence = fields.Integer(string="Sequence")
    ks_template = fields.Many2one(
    ...
    def CreateDashBoard(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id("ks_dashboard_ninja.ks_create_dashboard_wizard")
        return action
