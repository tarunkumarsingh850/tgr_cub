...
def ks_date_series_l(ks_date_selection, timezone, type, self):
    ks_date_data = {}
    date_filter_options = {"day": 0, "week": 7, "month": 30, "quarter": 90, "year": 365, "past": False, "future": False}
    end_time = datetime.strptime(
        datetime.now(pytz.timezone(timezone)).strftime("%Y-%m-%d 23:59:59"), "%Y-%m-%d %H:%M:%S"
    )
    start_time = datetime.strptime(
        (datetime.now(pytz.timezone(timezone)) - timedelta(days=date_filter_options[ks_date_selection])).strftime(
            "%Y-%m-%d 00:00:00"
        ),
        "%Y-%m-%d %H:%M:%S",
    )
