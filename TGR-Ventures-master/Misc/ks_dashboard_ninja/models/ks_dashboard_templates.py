...
class KsDashboardNinjaTemplate(models.Model):
    ...
    ks_template_type = fields.Selection(
        [("ks_default", "Predefined"), ("ks_custom", "Custom")], string="Template Format"
    )
    ks_dashboard_item_ids = fields.One2many(
        "ks_dashboard_ninja.item", "ks_dashboard_board_template_id", string="Template Type"
    )
    ...
    def create(self, vals_list):
        for val in vals_list:
            if val.get("ks_template_type", False) and val.get("ks_dashboard_board_id", False):
                ...
                val["ks_dashboard_item_ids"] = [
                    (4, x.copy({"ks_dashboard_ninja_board_id": False}).id) for x in dashboard_id.ks_dashboard_items_ids
                ]
    ...
    def write(self, val):
        if val.get("ks_dashboard_board_id", False):
            ...
            val["ks_item_count"] = len(dashboard_id.ks_dashboard_items_ids)
            val["ks_dashboard_item_ids"] = [
                (6, 0, [x.copy({"ks_dashboard_ninja_board_id": False}).id for x in dashboard_id.ks_dashboard_items_ids])
            ]
