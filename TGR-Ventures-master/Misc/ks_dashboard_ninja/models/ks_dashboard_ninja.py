...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_dashboard_items_ids = fields.One2many(
        "ks_dashboard_ninja.item", "ks_dashboard_ninja_board_id", string="Dashboard Items"
    )
    ks_dashboard_menu_name = fields.Char(string="Menu Name")
    ks_dashboard_top_menu_id = fields.Many2one(
        "ir.ui.menu",
        domain="['|',('action','=',False),('parent_id','=',False)]",
        string="Show Under Menu",
        default=lambda self: self.env["ir.ui.menu"].search([("name", "=", "My Dashboard")]),
    )
    ...
    ks_data_formatting = fields.Selection(
        [("global", "Global"), ("indian", "Indian"), ("exact", "Exact")], string="Format"
    )

    ks_gridstack_config = fields.Char("Item Configurations")
    ks_dashboard_default_template = fields.Many2one(
        "ks_dashboard_ninja.board_template",
    ...
    ks_dashboard_menu_sequence = fields.Integer(
    ...
    ks_dashboard_defined_filters_ids = fields.One2many(
        "ks_dashboard_ninja.board_defined_filters", "ks_dashboard_board_id", string="Dashboard Predefined Filters"
    )
    ks_dashboard_custom_filters_ids = fields.One2many(
        "ks_dashboard_ninja.board_custom_filters", "ks_dashboard_board_id", string="Dashboard Custom Filters"
    )
    multi_layouts = fields.Boolean(
        string="Enable Multi-Dashboard Layouts", help="Allow user to have multiple layouts of the same Dashboard"
    )
        ...
        if record.ks_dashboard_default_template and record.ks_dashboard_default_template.ks_item_count:
            ...
            for item_data in template_data:
                if record.ks_dashboard_default_template.ks_template_type == "ks_custom":
                    dashboard_item = (
                        self.env["ks_dashboard_ninja.item"]
                        .browse(int(item_data))
                        .copy({"ks_dashboard_ninja_board_id": record.id})
                    )
    ...
    def write(self, vals):
        if vals.get("ks_date_filter_selection", False) and vals.get("ks_date_filter_selection") != "l_custom":
            vals.update({"ks_dashboard_start_date": False, "ks_dashboard_end_date": False})
        record = super(KsDashboardNinjaBoard, self).write(vals)
        for rec in self:
            if "ks_dashboard_menu_name" in vals:
                if (
                    self.env.ref("ks_dashboard_ninja.ks_my_default_dashboard_board")
                    and self.env.ref("ks_dashboard_ninja.ks_my_default_dashboard_board").sudo().id == rec.id
                ):
            ...
            if "ks_dashboard_top_menu_id" in vals:
                rec.ks_dashboard_menu_id.write({"parent_id": vals["ks_dashboard_top_menu_id"]})
    ...
    def ks_get_grid_config(self):
        default_grid_id = self.env["ks_dashboard_ninja.child_board"].search(
            [
                ["id", "in", self.ks_child_dashboard_ids.ids],
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=False):
        ...
        dashboard_data = {
            ...
            "ks_dashboard_start_date": self._context.get("ksDateFilterStartDate", False)
            or self.browse(ks_dashboard_id).ks_dashboard_start_date,
            "ks_dashboard_end_date": self._context.get("ksDateFilterEndDate", False)
            or self.browse(ks_dashboard_id).ks_dashboard_end_date,
            "ks_date_filter_selection": self._context.get("ksDateFilterSelection", False)
            or self.browse(ks_dashboard_id).ks_date_filter_selection,
            "ks_gridstack_config": "{}",
            ...
            "ks_item_model_relation": {
                x["id"]: [x["ks_model_name"], x["ks_model_name_2"]]
        ...
        try:
            items = self.ks_dashboard_items_ids.search(
                [["ks_dashboard_ninja_board_id", "=", ks_dashboard_id]] + ks_item_domain
            ).ids
        except Exception:
            items = self.ks_dashboard_items_ids.search(
                [["ks_dashboard_ninja_board_id", "=", ks_dashboard_id]] + ks_item_domain
            ...
            if ks_date_filter_selection == "l_custom":
                ...
                self = self.with_context(
                    ksDateFilterStartDate=fields.datetime.strptime(
                        ks_start_dt_parse.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S"
                    )
                )
                self = self.with_context(
                    ksDateFilterEndDate=fields.datetime.strptime(
                        ks_end_dt_parse.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S"
                    )
                )
    ...
    def ks_export_item_data(self, rec):
        ...
        if ks_many2many_field_ordering.get("ks_list_view_fields", False):
            ks_list_view_fields_list = self.env["ir.model.fields"].search(
                [("id", "in", ks_many2many_field_ordering.get("ks_list_view_fields", False))]
            )
        if ks_many2many_field_ordering.get("ks_list_view_group_fields", False):
            ks_list_view_group_fields_list = self.env["ir.model.fields"].search(
                [("id", "in", ks_many2many_field_ordering.get("ks_list_view_group_fields", False))]
            )
        if ks_many2many_field_ordering.get("ks_chart_measure_field", False):
            ks_chart_measure_field_list = self.env["ir.model.fields"].search(
                [("id", "in", ks_many2many_field_ordering.get("ks_chart_measure_field", False))]
            )
        if ks_many2many_field_ordering.get("ks_chart_measure_field_2", False):
            ks_chart_measure_field_2_list = self.env["ir.model.fields"].search(
                [("id", "in", ks_many2many_field_ordering.get("ks_chart_measure_field_2", False))]
            )
        ...
        for res in rec.ks_dn_header_lines:
            ks_dn_header_line = {"ks_to_do_header": res.ks_to_do_header}
        ...
        selecred_rec = self.env["ks_dashboard_ninja.child_board"].search(
            [
                ["id", "in", rec.ks_dashboard_ninja_board_id.ks_child_dashboard_ids.ids],
                ["ks_active", "=", True],
                ["company_id", "=", self.env.company.id],
            ],
            limit=1,
        ...
        item = {
            ...
            "ks_date_filter_selection": rec.ks_date_filter_selection,
            "ks_item_start_date": rec.ks_item_start_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_start_date
            else False,
            "ks_item_end_date": rec.ks_item_end_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_end_date
            else False,
            "ks_date_filter_selection_2": rec.ks_date_filter_selection_2,
            "ks_item_start_date_2": rec.ks_item_start_date_2.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_start_date_2
            else False,
            "ks_item_end_date_2": rec.ks_item_end_date_2.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_end_date_2
            else False,
            "ks_previous_period": rec.ks_previous_period,
            "ks_target_view": rec.ks_target_view,
            "ks_data_comparison": rec.ks_data_comparison,
    ...
    def ks_dashboard_export(self, ks_dashboard_ids, **kwargs):
        ...
        for ks_dashboard_id in ks_dashboard_ids:
            ...
            selecred_rec = self.env["ks_dashboard_ninja.child_board"].search(
                [
                    ["id", "in", dash.ks_child_dashboard_ids.ids],
                    ["ks_active", "=", True],
                    ["company_id", "=", self.env.company.id],
                ],
                limit=1,
    ...
    def ks_prepare_export_data_vals(
        self,
        ks_dashboard_rec,
        grid_conf=None,
    ):
    ...
    def ks_import_dashboard(self, file, menu_id):
        ...
        for data in ks_dashboard_data:
            ...
            self.browse(dashboard_id.id).write({"ks_gridstack_config": json.dumps(ks_grid_stack_config)})
    ...
    def ks_create_item(self, item):
        ...
        if not model and not item["ks_dashboard_item_type"] == "ks_to_do":
            raise ValidationError(
                _("Please Install the Module which contains the following Model : %s " % item["ks_model_id"])
            )
    ...
    def ks_prepare_item(self, item):
        try:
            ...
            if ks_many2many_field_ordering:
                ...
                ks_list_view_group_fields_name = ks_many2many_field_ordering.get(
                    "ks_list_view_group_fields_name", False
                )
            ...
            item["ks_item_start_date"] = item["ks_item_start_date"] if item["ks_item_start_date"] else False
            item["ks_item_end_date"] = item["ks_item_end_date"] if item["ks_item_end_date"] else False
            item["ks_item_start_date_2"] = item["ks_item_start_date_2"] if item["ks_item_start_date_2"] else False
            item["ks_item_end_date_2"] = item["ks_item_end_date_2"] if item["ks_item_end_date_2"] else False
    ...
    def ks_prepare_dashboard_domain(self):
        pre_defined_filter_ids = self.env["ks_dashboard_ninja.board_defined_filters"].search(
            [
                ["id", "in", self.ks_dashboard_defined_filters_ids.ids],
                "|",
                ["ks_is_active", "=", True],
        ...
        for model_id in filter_model_ids:
            filter_ids = self.env["ks_dashboard_ninja.board_defined_filters"].search(
                [
                    ["id", "in", pre_defined_filter_ids.ids],
                    "|",
                    ["ks_model_id", "=", model_id],
                    ["display_type", "=", "line_section"],
                ],
                order="sequence",
            )
            ...
            for rec in filter_ids:
                ...
                if data.get(rec.ks_model_id.model) and rec.ks_domain:
                    ...
                    data[rec.ks_model_id.model] = {
                        ...
                        "item_ids": self.env["ks_dashboard_ninja.item"]
                        .search(
                            [
                                ["id", "in", self.ks_dashboard_items_ids.ids],
                                "|",
                                ["ks_model_id", "=", rec.ks_model_id.id],
                                ["ks_model_id_2", "=", rec.ks_model_id.id],
                            ]
                        )
                        .ids,
                    }
        ...
        for rec in custom_filter_ids:
            ...
            if rec.ks_domain_field_id.ttype == "selection":
                data[str(rec.id)]["special_data"] = {
                    "select_options": self.env[rec.ks_model_id.model].fields_get(
                        allfields=[rec.ks_domain_field_id.name]
