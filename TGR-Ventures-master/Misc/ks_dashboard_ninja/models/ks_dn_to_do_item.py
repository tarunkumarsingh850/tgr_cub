...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_header_bg_color = fields.Char(
        ...
        if self.ks_dn_header_lines:
            for ks_dn_header_line in self.ks_dn_header_lines:
                ...
                if " " in ks_dn_header_line.ks_to_do_header:
                    ...
                    ks_temp = ks_dn_header_line.ks_to_do_header.replace(ks_dn_header_line.ks_to_do_header[0], "z")
                    ks_to_do_data["ks_link"].append("#" + ks_temp + ks_dn_header_line_id)
                    ks_to_do_data["ks_href_id"].append(ks_temp + str(ks_dn_header_line.id))
                else:
                ...
                if len(ks_dn_header_line.ks_to_do_description_lines):
                    for ks_to_do_description_line in ks_dn_header_line.ks_to_do_description_lines:
                        if " " in ks_dn_header_line.ks_to_do_header or ks_dn_header_line.ks_to_do_header[0].isdigit():
                            if ks_to_do_data["ks_content"].get(ks_temp + str(ks_dn_header_line.id), False):

                                ks_to_do_data["ks_content"][ks_temp + str(ks_dn_header_line.id)].append(
                                    ks_to_do_description_line.ks_description
                                )
                                ks_to_do_data["ks_content_record_id"][ks_temp + str(ks_dn_header_line.id)].append(
                                    str(ks_to_do_description_line.id)
                                )
                                ks_to_do_data["ks_content_active"][ks_temp + str(ks_dn_header_line.id)].append(
                                    str(ks_to_do_description_line.ks_active)
                                )
                            else:
                                ks_to_do_data["ks_content"][ks_temp + str(ks_dn_header_line.id)] = [
                                    ks_to_do_description_line.ks_description
                                ]
                                ks_to_do_data["ks_content_record_id"][ks_temp + str(ks_dn_header_line.id)] = [
                                    str(ks_to_do_description_line.id)
                                ]
                                ks_to_do_data["ks_content_active"][ks_temp + str(ks_dn_header_line.id)] = [
                                    str(ks_to_do_description_line.ks_active)
                                ]
                        else:
                            if ks_to_do_data["ks_content"].get(
                                ks_dn_header_line.ks_to_do_header + str(ks_dn_header_line.id), False
                            ):

                                ks_to_do_data["ks_content"][
                                    ks_dn_header_line.ks_to_do_header + str(ks_dn_header_line.id)
                                ].append(ks_to_do_description_line.ks_description)
                                ks_to_do_data["ks_content_record_id"][
                                    ks_dn_header_line.ks_to_do_header + str(ks_dn_header_line.id)
                                ].append(str(ks_to_do_description_line.id))
                                ks_to_do_data["ks_content_active"][
                                    ks_dn_header_line.ks_to_do_header + str(ks_dn_header_line.id)
                                ].append(str(ks_to_do_description_line.ks_active))
                            else:
                                ks_to_do_data["ks_content"][
                                    ks_dn_header_line.ks_to_do_header + str(ks_dn_header_line.id)
                                ] = [ks_to_do_description_line.ks_description]
