...
from odoo.addons.ks_dashboard_ninja.common_lib.ks_date_filter_selections import (
    ks_get_date,
    ks_convert_into_utc,
...
def ks_read(self, records):
    if (
        self.name == "ks_list_view_fields"
        or self.name == "ks_list_view_group_fields"
        or self.name == "ks_chart_measure_field"
        or self.name == "ks_chart_measure_field_2"
    ):
...
def ks_time_addition(self, gb, query):
    ...
    if not field:
        raise ValueError("Invalid field {!r} on model {!r}".format(split[0], self._name))
    ...
    if temporal:
        ...
        if tz_convert:
            qualified_field = "timezone('{}', timezone('UTC',{}))".format(self._context.get("tz", "UTC"), qualified_field)
        qualified_field = "date_trunc('{}', {}::timestamp)".format(gb_function or "month", qualified_field)
...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_background_color = fields.Char(
        string="Background Color", default="#ffffff,0.99", help=" Select the background color with transparency. "
    )
    ...
    ks_default_icon_color = fields.Char(
        default="#ffffff,0.99", string="Icon Color", help="Select the icon to be displayed. "
    )
    ...
    ks_dashboard_item_theme = fields.Char(
        string="Theme", default="white", help="Select the color theme for the display. "
    )
    ks_layout = fields.Selection(
        [
            ("layout1", "Layout 1"),
            ("layout2", "Layout 2"),
            ("layout3", "Layout 3"),
    ...
    ks_record_count_type_2 = fields.Selection(
        [("count", "Count"), ("sum", "Sum"), ("average", "Average")], string="Kpi Record Type", default="sum"
    )
    ks_record_field_2 = fields.Many2one(
        "ir.model.fields",
    ...
    ks_record_count_2 = fields.Float(
        string="KPI Record Count", readonly=True, compute="ks_get_record_count_2", compute_sudo=False
    )
    ks_record_count_type = fields.Selection(
        [("count", "Count"), ("sum", "Sum"), ("average", "Average")],
        string="Record Type",
        default="count",
        help="Type of record how record will show as count,sum and average of the record",
    )
    ks_record_count = fields.Float(
        string="Record Count", compute="ks_get_record_count", readonly=True, compute_sudo=False
    )
    ks_record_field = fields.Many2one(
        "ir.model.fields",
    ...
    ks_record_data_limit_visibility = fields.Boolean(
        string="Record Limit Data Visibility", help="To enable the record data limit field"
    )
    ...
    ks_date_filter_field = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('store','=',True),'|',('ttype','=','date')," "('ttype','=','datetime')]",
    ...
    ks_date_filter_field_2 = fields.Many2one(
        "ir.model.fields",
    ...
    ks_previous_period = fields.Boolean(
    ...
    # ------------------------ Pro Fields --------------------
    ks_dashboard_ninja_board_id = fields.Many2one(
    ...
    ks_chart_relation_groupby = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),('ttype','!=','binary'),"
        "('ttype','!=','many2many'), ('ttype','!=','one2many')]",
        string="Group By",
        help=" Define the x-axis of the graph. ",
    )
    ks_chart_relation_sub_groupby = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),('ttype','!=','binary'),"
        "('ttype','!=','many2many'), ('ttype','!=','one2many')]",
    ...
    ks_chart_date_groupby = fields.Selection(
        [
            ("minute", "Minute"),
            ("hour", "Hour"),
            ("day", "Day"),
    ...
    ks_chart_date_sub_groupby = fields.Selection(
        [
            ("minute", "Minute"),
            ("hour", "Hour"),
            ("day", "Day"),
    ...
    ks_chart_data_count_type = fields.Selection(
        [("count", "Count"), ("sum", "Sum"), ("average", "Average")], string="Data Type", default="sum"
    )
    ...
    ks_chart_cumulative_field = fields.Many2many(
        "ir.model.fields",
        "ks_dn_cumulative_measure_field_rel",
        ...
        "('ttype','=','integer'),('ttype','=','float'),"
        "('ttype','=','monetary')]",
        string="Cumulative Fields",
        help="Data points to be selected.",
    )

    ks_chart_cumulative = fields.Boolean("Cumulative As Line")
    ks_chart_measure_field_2 = fields.Many2many(
        "ir.model.fields",
        "ks_dn_measure_field_rel_2",
        "measure_field_id_2",
        "field_id",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),'|','|',"
    ...
    ks_sort_by_field = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
        "('ttype','!=','one2many'),('ttype','!=','binary')]",
        string="Sort By Field",
        help="Select the desired sorting preference. ",
    )
    ks_sort_by_order = fields.Selection(
        [("ASC", "Ascending"), ("DESC", "Descending")], string="Sort Order", help=" Select the order of the sorting. "
    )
    ...
    ks_chart_item_color = fields.Selection(
        ...
        default="default",
        help="Select the display preference. ",
    )

    # ------------------------ List View Fields ------------------------------

    ks_list_view_type = fields.Selection(
    ...
    ks_list_view_fields = fields.Many2many(
        "ir.model.fields",
        "ks_dn_list_field_rel",
        "list_field_id",
        "field_id",
    ...
    ks_export_all_records = fields.Boolean(
    ...
    ks_list_view_group_fields = fields.Many2many(
        "ir.model.fields",
        "ks_dn_list_group_field_rel",
        "list_field_id",
        "field_id",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),'|','|',"
    ...
    ks_list_view_data = fields.Char(
        string="List View Data in JSon", compute="ks_get_list_view_data", compute_sudo=False
    )
    ...
    ks_list_target_deviation_field = fields.Many2one(
        "ir.model.fields",
        "list_field_id",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),'|','|',"
    ...
    ks_actions = fields.Many2one(
        "ir.actions.act_window",
        domain="[('res_model','=',ks_model_name)]",
        string="Actions",
        help="Redirects you to the selected view. ",
    )

    ks_compare_period = fields.Integer(
        string="Include Period",
        help=" Provide the number of Date Filter Selection you want to include while displaying the record.",
    )
    ks_year_period = fields.Integer(
        string="Same Period Previous Years", help=" Display the record for the same Date field for the last year. "
    )
    ...
    ks_multiplier_active = fields.Boolean(
        string="Apply Multiplier", default=False, help="Provides the visibility of multiplier field"
    )
    ...
    ks_unit_selection = fields.Selection(
        ...
        help="Select the unit to be assigned to the value. ",
    )
    ks_chart_unit = fields.Char(
        string="Enter Unit", size=5, default="", help="Maximum limit 5 characters, for ex: km, m"
    )

    # User can stop propagation of the tile item
    ks_show_records = fields.Boolean(
        string="Show Records",
        default=True,
        help="""This field Enable the click on
    ...
    ks_data_calculation_type = fields.Selection(
        [("custom", "Default Query"), ("query", "Custom Query")],
        string="Data Calculation Type",
        default="custom",
    ...
    ks_button_color = fields.Char(string="Top Button Color", default="#000000,0.99")

    ks_is_client_action = fields.Boolean("Client Action", default=False)
    ks_client_action = fields.Many2one(
        "ir.actions.client",
    ...
    ks_multiplier_lines = fields.One2many(
        "ks_dashboard_item.multiplier", "ks_dashboard_item_id", readonly=False, store=True, string="Multiplier Lines"
    )

    ks_precision_digits = fields.Integer("Digits", compute="_ks_compute_precision_digits", store=True, readonly=False)

    ks_data_label_type = fields.Selection(
    ...
    @api.onchange(
        "ks_multiplier_active", "ks_chart_measure_field", "ks_chart_measure_field_2", "ks_list_view_group_fields"
    )
    ...
    def ks_date_target(self):
        for rec in self:
            if (
                rec.ks_chart_date_groupby in ("minute", "hour") or rec.ks_chart_date_sub_groupby in ("minute", "hour")
    ...
    def create(self, values):
        ...
        if not values.get("ks_many2many_field_ordering", False):
            ...
            if values.get("ks_list_view_group_fields", False) and len(values["ks_list_view_group_fields"][0][2]) > 0:
                for measure in values["ks_list_view_group_fields"][0][2]:
                    ks_measure_id = self.env["ir.model.fields"].search([("id", "=", measure)])
            ...
            ks_many2many_field_ordering = {
                ...
                "ks_chart_measure_field": values["ks_chart_measure_field"][0][2]
                if values.get("ks_chart_measure_field", False)
                else [],
                "ks_chart_measure_field_name": ks_chart_measure_field_name,
                "ks_chart_measure_field_2": values["ks_chart_measure_field_2"][0][2]
                if values.get("ks_chart_measure_field_2", False)
                else [],
        ...
        return super(KsDashboardNinjaItems, self).create(values)
    ...
    def ks_list_view_group_fields_onchange(self):
        ...
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_list_view_group_fields_name"] = [
                x.name for x in rec.ks_list_view_group_fields
            ]
            rec.ks_many2many_field_ordering = json.dumps(ks_many2many_field_ordering)
    ...
    def ks_chart_measure_field_onchange(self):
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_chart_measure_field_name"] = [x.name for x in rec.ks_chart_measure_field]
            rec.ks_many2many_field_ordering = json.dumps(ks_many2many_field_ordering)
    ...
    def ks_chart_measure_field_2_onchange(self):
        ...
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_chart_measure_field_2_name"] = [
                x.name for x in rec.ks_chart_measure_field_2
            ]
            rec.ks_many2many_field_ordering = json.dumps(ks_many2many_field_ordering)
    ...
    def layout_four_font_change(self):
        if self.ks_dashboard_item_theme != "white":
            if self.ks_layout == "layout4":
                ...
                self.ks_default_icon_color = self.ks_get_dark_color(
                    self.ks_background_color.split(",")[0], self.ks_background_color.split(",")[1]
                )
            else:
                self.ks_default_icon_color = "#ffffff,0.99"
                self.ks_font_color = "#ffffff,0.99"
        ...
        for rec in self:
            ...
            if rec.ks_model_id:
                datetime_field_list = rec.ks_date_filter_field.search(
                    [("model_id", "=", rec.ks_model_id.id), "|", ("ttype", "=", "date"), ("ttype", "=", "datetime")]
                ).read(["id", "name"])
    ...
    def _ksGetRecordCount(self, domain=[]):
        ...
        if rec.ks_record_count_type == "count" or rec.ks_dashboard_item_type == "ks_list_view":
            ...
            rec.ks_record_count_type in ["sum", "average"]
            and rec.ks_record_field
            and rec.ks_dashboard_item_type != "ks_list_view"
        ):
            ks_records_grouped_data = rec.ks_fetch_model_data(
                rec.ks_model_name, rec.ks_domain, "read_group", rec, domain
            )
            if ks_records_grouped_data and len(ks_records_grouped_data) > 0:
                ks_records_grouped_data = ks_records_grouped_data[0]
                if (
                    rec.ks_record_count_type == "sum"
                    and ks_records_grouped_data.get("__count", False)
                    and (ks_records_grouped_data.get(rec.ks_record_field.name))
                ):
                    ks_record_count = ks_records_grouped_data.get(rec.ks_record_field.name, 0)
                elif (
                    rec.ks_record_count_type == "average"
                    and ks_records_grouped_data.get("__count", False)
                    and (ks_records_grouped_data.get(rec.ks_record_field.name))
                ):
                    ks_record_count = ks_records_grouped_data.get(
                        rec.ks_record_field.name, 0
                    ) / ks_records_grouped_data.get("__count", 1)
                else:
                    ks_record_count = 0
            else:
                ks_record_count = 0
        ...
        if rec.ks_date_filter_field:
            if not rec.ks_date_filter_selection or rec.ks_date_filter_selection == "l_none":
                ...
                if (
                    ...
                    and rec.ks_date_filter_field.ttype == "datetime"
                    and not ks_is_def_custom_filter
                ):
                ...
                if (
                    selected_start_date
                    and selected_end_date
                    and rec.ks_date_filter_field.ttype == "date"
                    and ks_is_def_custom_filter
                ):
                ...
                if self._context.get("ksDateFilterSelection", False) and self._context["ksDateFilterSelection"] not in [
                    ...
                    ks_date_data = ks_get_date(
                        self._context.get("ksDateFilterSelection"), self, rec.ks_date_filter_field.ttype
                    )
                ...
                if selected_start_date and selected_end_date:
                    if rec.ks_compare_period:
                        ...
                        if rec.ks_compare_period > 0:
                            selected_end_date = (
                                selected_end_date + (selected_end_date - selected_start_date) * ks_compare_period
                            )
                            ...
                            selected_start_date = (
                                selected_start_date - (selected_end_date - selected_start_date) * ks_compare_period
                            )
                            if rec.ks_date_filter_field.ttype == "date" and rec.ks_date_filter_selection == "l_day":
    ...
    def ks_empty_sub_group_by(self):
        for rec in self:
            if (
                not rec.ks_chart_relation_groupby
                or rec.ks_chart_groupby_type == "date_type"
                and not rec.ks_chart_date_groupby
            ):
                rec.ks_chart_relation_sub_groupby = False
                rec.ks_chart_date_sub_groupby = False
            if not (rec.ks_chart_relation_groupby.ttype == "datetime" or rec.ks_chart_relation_groupby.ttype == "date"):
                rec.ks_goal_lines = False
    ...
    def get_chart_sub_groupby_type(self):
        for rec in self:
            if (
                rec.ks_chart_relation_sub_groupby.ttype == "datetime"
                or rec.ks_chart_relation_sub_groupby.ttype == "date"
            ):
    ...
    def _ks_get_chart_data(self, domain=[]):
        rec = self
        if (
            rec.ks_dashboard_item_type
            and rec.ks_dashboard_item_type != "ks_tile"
            and rec.ks_dashboard_item_type != "ks_list_view"
            and rec.ks_model_id
            and rec.ks_chart_data_count_type
        ):
            ks_chart_data = {
                "labels": [],
                "datasets": [],
            ...
            if rec.ks_chart_data_count_type == "count":
                ...
                for res in range(0, len(rec.ks_chart_measure_field)):
                    ...
                    if (
                        rec.ks_chart_cumulative_field
                        and res < len(rec.ks_chart_cumulative_field)
                        and (rec.ks_chart_cumulative_field[res].id or rec.ks_chart_cumulative_field[res].id.origin)
                        in rec.ks_chart_measure_field.ids
                    ):

                        ks_chart_data["datasets"].append(
                            {
                                "data": [],
                                "label": rec.ks_chart_cumulative_field[res].field_description,
                        ...
                        ks_chart_data["datasets"].append(
                            {
                                "data": [],
                                "label": rec.ks_chart_measure_field[res].field_description,
            ...
            if (
                (rec.ks_chart_data_count_type != "count" and ks_chart_measure_field)
                or (rec.ks_chart_data_count_type == "count" and not ks_chart_measure_field)
                ...
                if (
                    rec.ks_chart_groupby_type == "date_type" and rec.ks_chart_date_groupby
                ) or rec.ks_chart_groupby_type != "date_type":
                    ks_chart_data = rec.ks_fetch_chart_data(
                        rec.ks_model_name,
                        ks_chart_domain,
                        ks_chart_measure_field_with_type,
                        ks_chart_measure_field_with_type_2,
                        ...
                        ks_chart_measure_field_ids,
                        ks_chart_measure_field_2_ids,
                        rec.ks_chart_relation_groupby.id,
                        ks_chart_data,
                    )

                    if (
                        rec.ks_chart_groupby_type == "date_type"
                        and rec.ks_goal_enable
                        and rec.ks_dashboard_item_type
                        in ["ks_bar_chart", "ks_horizontalBar_chart", "ks_line_chart", "ks_area_chart"]
                        ...
                        if rec.ks_date_filter_selection == "l_none":
                            ...
                            if rec.ks_date_filter_selection == "l_custom":
                                ...
                                ks_date_data = ks_get_date(
                                    rec.ks_date_filter_selection, self, rec.ks_date_filter_field.ttype
                                )
                                selected_start_date = ks_date_data["selected_start_date"]
                                selected_end_date = ks_date_data["selected_end_date"]
                        ...
                        if selected_start_date and selected_end_date:
                            ks_goal_domain.extend(
                                [
                                    ("ks_goal_date", ">=", selected_start_date.split(" ")[0]),
                                    ("ks_goal_date", "<=", selected_end_date.split(" ")[0]),
                                ]
                            )

                        ks_date_data = rec.ks_get_start_end_date(
                            rec.ks_model_name,
                            ks_chart_groupby_relation_field,
                        ...
                        if ks_date_data["start_date"] and ks_date_data["end_date"] and rec.ks_goal_lines:
                            labels = self.generate_timeserise(
                                ks_date_data["start_date"], ks_date_data["end_date"], ks_chart_date_groupby
                            )
            ...
            elif rec.ks_chart_relation_sub_groupby and (
                (rec.ks_chart_sub_groupby_type == "relational_type")
                or (rec.ks_chart_sub_groupby_type == "selection")
                or (rec.ks_chart_sub_groupby_type == "date_type" and rec.ks_chart_date_sub_groupby)
                or (rec.ks_chart_sub_groupby_type == "other")
            ):
                if rec.ks_chart_relation_sub_groupby.ttype == "date":
                    if rec.ks_chart_date_sub_groupby in ("minute", "hour"):
                        raise ValidationError(
                            _("Sub Groupby field: {} cannot be aggregated by {}").format(
                                rec.ks_chart_relation_sub_groupby.display_name, rec.ks_chart_date_sub_groupby
                            )
                        )
                ...
                if len(ks_chart_measure_field) != 0 or rec.ks_chart_data_count_type == "count":
                    ...
                    if rec.ks_chart_sub_groupby_type == "date_type" and rec.ks_chart_date_sub_groupby:
                        ks_chart_sub_groupby_field = (
                            rec.ks_chart_relation_sub_groupby.name + ":" + ks_chart_date_sub_groupby
                        )
                    else:
                        ...
                        ks_chart_record = self.env[rec.ks_model_name].read_group(
                            ...
                            list(
                                set(
                                    ks_chart_measure_field_with_type
                                    + ks_chart_measure_field_with_type_2
                                    + [ks_chart_groupby_relation_field, rec.ks_chart_relation_sub_groupby.name]
                                )
                            ),
                    ...
                    for res in ks_chart_record:
                        ...
                        if res.get(ks_chart_groupby_relation_fields[0], False):
                            if rec.ks_chart_groupby_type == "date_type":
                                ...
                                elif rec.ks_chart_date_groupby in [
                                    "minute",
                                    "hour",
                                ] and rec.ks_chart_date_sub_groupby in ["month", "week", "quarter", "year"]:
                                ...
                                label = dict(
                                    self.env[rec.ks_model_name].fields_get(
                                        allfields=[ks_chart_groupby_relation_fields[0]]
                                    )[ks_chart_groupby_relation_fields[0]]["selection"]
                                )[selection]
                            ...
                            if rec.ks_chart_data_count_type != "count":
                                for ress in rec.ks_chart_measure_field:
                                    if rec.ks_chart_sub_groupby_type == "date_type":
                                        if res[ks_chart_groupby_relation_fields[1]] is not False:
                                            ...
                                            labels.append(
                                                str(res[ks_chart_groupby_relation_fields[1]])
                                                + " "
                                                + ress.field_description
                                            )
                                    ...
                                    value.append(res.get(ress.name, 0))
                                ...
                                if rec.ks_chart_measure_field_2 and rec.ks_dashboard_item_type == "ks_bar_chart":
                                    for ress in rec.ks_chart_measure_field_2:
                                        ...
                                        value_2.append(res.get(ress.name, 0))
                    ...
                    if (
                        ...
                        ks_data = rec.ks_sort_sub_group_by_records(
                            ks_data,
                            rec.ks_chart_groupby_type,
                            rec.ks_chart_date_groupby,
                            rec.ks_sort_by_order,
                    ...
                    if len(ks_data) != 0:
                        ...
                        if (
                            rec.ks_goal_enable
                            and rec.ks_standard_goal_value
                            and rec.ks_dashboard_item_type
                            in ["ks_bar_chart", "ks_line_chart", "ks_area_chart", "ks_horizontalBar_chart"]
                        ):
                            goal_dataset = []
                            length = len(ks_chart_data["datasets"][0]["data"])
                            for i in range(length):
            ...
            if self.ks_multiplier_active:
                for ks_multiplier in self.ks_multiplier_lines:
                    for i in range(0, len(ks_chart_data["datasets"])):
                        if (
                            ks_multiplier.ks_multiplier_fields.field_description
                            in ks_chart_data["datasets"][i]["label"]
                        ):
    ...
    @api.depends(
        ...
        "ks_compare_period",
        "ks_year_period",
        "ks_list_target_deviation_field",
        "ks_goal_enable",
        "ks_standard_goal_value",
        "ks_goal_lines",
        "ks_domain_extension",
    ...
    def get_list_view_record(
        self, orderid, sort_order, ks_chart_domain, ksoffset=0, initial_count=0, ks_export_all=False
    ):
        ks_list_view_data = {
            "label": [],
            "fields": [],
        ...
        if self.ks_list_view_type == "ungrouped":
            if self.ks_list_view_fields:
                ks_list_view_data = self.ks_fetch_list_view_data(
                    self, ks_chart_domain, offset=ksoffset, initial_count=initial_count
                )
            ...
            if self.ks_chart_groupby_type == "relational_type":
                ...
                try:
                    ks_list_view_records = self.env[self.ks_model_name].read_group(
                        ks_chart_domain,
                        ks_list_fields,
                        [self.ks_chart_relation_groupby.name],
                        orderby=orderby,
                        limit=limit,
                        offset=ksoffset,
                        lazy=False,
                ...
                for res in ks_list_view_records:
                    if (
                        all(list_fields in res for list_fields in ks_list_fields)
                        and res[self.ks_chart_relation_groupby.name]
                    ):
                        counter = 0
                        data_row = {
                            "id": res[self.ks_chart_relation_groupby.name][0],
                            "data": [],
                ...
                if self.ks_chart_date_groupby == "month_year":
                    ks_chart_date_groupby = "month"
                ks_list_view_data["groupby"] = self.ks_chart_relation_groupby.name + ":" + ks_chart_date_groupby
                ...
                ks_list_view_data["label"].append(
                    self.ks_chart_relation_groupby.field_description + " : " + ks_chart_date_groupby.capitalize()
                )
                ks_list_view_data["fields"].append(self.ks_chart_relation_groupby.ids[0])
                ks_list_view_data["fields_type"].append(self.ks_chart_relation_groupby.ttype)
                ks_list_view_data["store"].append(self.ks_chart_relation_groupby.store)
                ...
                try:
                    ks_list_view_records = self.env[self.ks_model_name].read_group(
                        ks_chart_domain,
                        ks_list_field + list_target_deviation_field,
                ...
                if all(
                    list_fields in res
                    for res in ks_list_view_records
                    for list_fields in ks_list_fields + list_target_deviation_field
                ):
                    for res in ks_list_view_records:
                        counter = 0
                    ...
                    if self.ks_goal_enable:
                        ...
                        if self.ks_list_target_deviation_field:
                            ks_list_view_data["label"].append(self.ks_list_target_deviation_field.field_description)
                        ...
                        ks_list_view_data2 = self.get_target_list_view_data(
                            ks_list_view_records,
                            self,
                            ks_list_fields,
            ...
            for ks_multiplier in self.ks_multiplier_lines:
                ...
                if label in ks_list_view_data["label"]:
                    ...
                    for i in range(0, len(ks_list_view_data["data_rows"])):
                        data_values = (
                            ks_list_view_data["data_rows"][i]["data"][index] * ks_multiplier.ks_multiplier_value
                        )
                        ks_list_view_data["data_rows"][i]["data"][index] = data_values
        return ks_list_view_data

    def get_target_list_view_data(
        self, ks_list_view_records, rec, ks_list_fields, ks_group_by, target_deviation_field, ks_chart_domain
    ):
        ks_list_view_data = {}
        ks_list_labels = []
        ks_list_records = {}
        ...
        if selected_start_date and selected_end_date:
            ks_goal_domain.extend(
                [
                    ("ks_goal_date", ">=", selected_start_date.strftime("%Y-%m-%d")),
        ...
        ks_goal_records = self.env["ks_dashboard_ninja.item_goal"].read_group(
            ks_goal_domain, ["ks_goal_value"], ["ks_goal_date" + ":" + ks_chart_date_groupby], lazy=False
        )
        ...
        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
            ...
            ks_chart_records_dates = ks_list_labels + list(set(ks_goal_labels) - set(ks_list_labels))
    ...
    def ks_fetch_list_view_data(self, rec, ks_chart_domain, limit=15, offset=0, ks_export_all=False, initial_count=0):
        ...
        try:
            ks_list_view_records = self.env[self.ks_model_name].search_read(
                ks_chart_domain, ks_list_view_fields, order=orderby, limit=limit, offset=offset
            )
        except Exception:
            ks_list_view_data = False
            return ks_list_view_data
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                if type(res[field_rec]) == fields.datetime or type(res[field_rec]) == fields.date:
                    ...
                    res[field_rec] = dict(
                        self.env[rec.ks_model_name].fields_get(allfields=[field_rec])[field_rec]["selection"]
                    )[res[field_rec]]
                data_row["data"].append(res[field_rec])
    ...
    def set_color_palette(self):
        for rec in self:
            if (
                rec.ks_dashboard_item_type == "ks_bar_chart"
                or rec.ks_dashboard_item_type == "ks_horizontalBar_chart"
                or rec.ks_dashboard_item_type == "ks_line_chart"
                or rec.ks_dashboard_item_type == "ks_area_chart"
            ):
    ...
    def _ksGetKpiData(self, domain1=[], domain2=[]):
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == "ks_kpi" and rec.ks_model_id:
            ...
            if rec.ks_model_id_2 and rec.ks_record_count_type_2:
                ...
                ks_kpi_data_model_2["record_field"] = (
                    "count" if rec.ks_record_count_type_2 == "count" else rec.ks_record_field_2.field_description
                )
    ...
    def ks_get_previous_period_data(self, rec):
        ...
        if ks_date_data:
            ...
            proper_domain = rec.ks_get_previous_period_domain(
                rec.ks_domain, previous_period_start_date, previous_period_end_date, rec.ks_date_filter_field
            )
            ks_record_count = 0.0
            ...
            if rec.ks_record_count_type == "count":
                ...
                try:
                    data = self.env[rec.ks_model_name].read_group(
                        proper_domain, [rec.ks_record_field.name], [], lazy=False
                    )[0]
                except Exception:
                    data = {}
                if rec.ks_record_count_type == "sum":
                    return (
                        data.get(rec.ks_record_field.name, 0)
                        if data.get("__count", False) and (data.get(rec.ks_record_field.name))
                        else 0
                    )
                else:
                    return (
                        data.get(rec.ks_record_field.name, 0) / data.get("__count", 1)
                        if data.get("__count", False) and (data.get(rec.ks_record_field.name))
                        else 0
                    )
            else:
        ...
        if ks_domain:
            ...
            if ks_start_date and ks_end_date and date_filter_field:
                proper_domain.extend(
                    [(date_filter_field.name, ">=", ks_start_date), (date_filter_field.name, "<=", ks_end_date)]
                )

        else:
            if ks_start_date and ks_end_date and date_filter_field:
                proper_domain = [
                    (date_filter_field.name, ">=", ks_start_date),
    ...
    def _ksGetRecordCount_2(self, domain=[]):
        ...
        if rec.ks_record_count_type_2 == "count":
            ks_record_count = rec.ks_fetch_model_data_2(
                rec.ks_model_name_2, rec.ks_domain_2, "search_count", rec, domain
            )

        elif rec.ks_record_count_type_2 in ["sum", "average"] and rec.ks_record_field_2:
            ks_records_grouped_data = rec.ks_fetch_model_data_2(
                rec.ks_model_name_2, rec.ks_domain_2, "read_group", rec, domain
            )
            if ks_records_grouped_data and len(ks_records_grouped_data) > 0:
                ks_records_grouped_data = ks_records_grouped_data[0]
                if (
                    rec.ks_record_count_type_2 == "sum"
                    and ks_records_grouped_data.get("__count", False)
                    and (ks_records_grouped_data.get(rec.ks_record_field_2.name))
                ):
                    ks_record_count = ks_records_grouped_data.get(rec.ks_record_field_2.name, 0)
                elif (
                    rec.ks_record_count_type_2 == "average"
                    and ks_records_grouped_data.get("__count", False)
                    and (ks_records_grouped_data.get(rec.ks_record_field_2.name))
                ):
                    ks_record_count = ks_records_grouped_data.get(
                        rec.ks_record_field_2.name, 0
                    ) / ks_records_grouped_data.get("__count", 1)
                else:
                    ks_record_count = 0
            else:
        ...
        try:
            if ks_domain and ks_domain != "[]" and ks_model_name:
                ...
                if ks_func == "search_count":
                    ...
                    data = self.env[ks_model_name].read_group(
                        proper_domain, [rec.ks_record_field_2.name], [], lazy=False
                    )
            elif ks_model_name:
                # Have to put extra if condition here because on load,model giving False value
        ...
        if rec.ks_date_filter_field_2:
            if not rec.ks_date_filter_selection_2 or rec.ks_date_filter_selection_2 == "l_none":
                ...
                if (
                    selected_start_date
                    and selected_end_date
                    and rec.ks_date_filter_field_2.ttype == "datetime"
                    and not ks_is_def_custom_filter
                ):
                    selected_start_date = ks_convert_into_utc(selected_start_date, ks_timezone)
                    selected_end_date = ks_convert_into_utc(selected_end_date, ks_timezone)
                if (
                    selected_start_date
                    and selected_end_date
                    and rec.ks_date_filter_field_2.ttype == "date"
                    and ks_is_def_custom_filter
                ):
                    selected_start_date = ks_convert_into_local(selected_start_date, ks_timezone)
                    selected_end_date = ks_convert_into_local(selected_end_date, ks_timezone)
                if self._context.get("ksDateFilterSelection", False) and self._context["ksDateFilterSelection"] not in [
                    ...
                    ks_date_data = ks_get_date(
                        self._context.get("ksDateFilterSelection"), self, rec.ks_date_filter_field_2.ttype
                    )
                    selected_start_date = ks_date_data["selected_start_date"]
                    selected_end_date = ks_date_data["selected_end_date"]
                ...
                if rec.ks_date_filter_selection_2 and rec.ks_date_filter_selection_2 != "l_custom":
                    ...
                    if rec.ks_item_start_date_2 or rec.ks_item_end_date_2:
                        ...
                        if (
                            rec.ks_date_filter_field_2.ttype == "date"
                            and rec.ks_item_start_date_2
                ...
                if selected_start_date and selected_end_date:
                    if rec.ks_compare_period_2:
                        ...
                        if rec.ks_compare_period_2 > 0:
                            selected_end_date = (
                                selected_end_date + (selected_end_date - selected_start_date) * ks_compare_period_2
                            )
                            if rec.ks_date_filter_field.ttype == "date" and rec.ks_date_filter_selection == "l_day":
                                selected_end_date = selected_end_date + timedelta(days=ks_compare_period_2)
                        elif rec.ks_compare_period_2 < 0:
                            selected_start_date = (
                                selected_start_date - (selected_end_date - selected_start_date) * ks_compare_period_2
                            )
                            if rec.ks_date_filter_field.ttype == "date" and rec.ks_date_filter_selection == "l_day":
                    ...
                    if rec.ks_year_period_2 and rec.ks_year_period_2 != 0:
                        ...
                        if rec.ks_date_filter_field_2:
                            ...
                            ks_date_domain = [
                                (rec.ks_date_filter_field_2.name, ">=", selected_start_date),
    ...
    def ks_fetch_chart_data(
        ...
        ks_chart_measure_field_ids,
        ks_chart_measure_field_2_ids,
        ks_chart_groupby_relation_field_id,
        ks_chart_data,
    ):
        ...
        try:
            if self.ks_fill_temporal and ks_chart_date_groupby not in ["minute", "hour"]:
                ...
                ks_chart_records = self.env[ks_model_name].read_group(
                    ks_chart_domain,
                    list(
                        set(
                            ks_chart_measure_field_with_type
        ...
        for res in ks_chart_records:
            ...
            if all(measure_field in res for measure_field in ks_chart_measure_field):
                if ks_chart_groupby_type == "relational_type":
                    ...
                    if selection:
                        label = dict(
                            self.env[ks_model_name].fields_get(allfields=[ks_chart_groupby_field])[
                                ks_chart_groupby_field
                            ]["selection"]
                        )[selection]
                    else:
                ...
                if ks_chart_measure_field:
                    if ks_chart_measure_field_2:
                        ...
                        for field_rec in ks_chart_measure_field_2:
                            ks_groupby_equal_measures = (
                                res.get(ks_chart_groupby_relation_field + "_count", False) or res.get("__count", False)
                                if res.get(ks_chart_groupby_relation_field + "_count", False)
                                or res.get("__count", False)
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        if action_line.ks_chart_type == "ks_list_view":
            ...
            limit = (
                action_line.ks_record_limit
                if action_line.ks_record_limit and action_line.ks_record_limit > 0
                else False
            )
            ks_count = 0
            ...
            if action_line.ks_item_action_field.ttype == "many2one":
                ...
                ks_list_view_records = self.env[record.ks_model_name].read_group(
                    domain,
                    ks_list_fields,
                ...
                for res in ks_list_view_records:
                    ...
                    counter = 0
                    data_row = {
                        "id": res[action_line.ks_item_action_field.name][0]
                        if res[action_line.ks_item_action_field.name]
                        else res[action_line.ks_item_action_field.name],
                        "data": [],
                        "domain": json.dumps(res["__domain"]),
                        "sequence": sequence + 1,
                        "last_seq": ks_count,
                ...
                action_line.ks_item_action_field.ttype == "date" or action_line.ks_item_action_field.ttype == "datetime"
            ):
                ks_list_view_data["list_view_type"] = "date_type"
                ks_list_field = []
                ks_list_view_data["groupby"] = (
                ...
                for res in ks_list_view_records:
                    if all(list_fields in res for list_fields in ks_list_fields):
                        counter = 0
                        data_row = {
                            "id": action_line.ks_item_action_field.name,
                            "data": [],
                            "domain": json.dumps(res["__domain"]),
                            "sequence": sequence + 1,
                            "last_seq": ks_count,
                            ...
                            if counter == 0:
                                ...
                                if action_line.ks_item_action_field.name == field_rec:
                                    data_row["data"].append(
                                        res[field_rec]
                                        * (
                                            res.get(field_rec + "_count", False)
                                            if res.get(field_rec + "_count", False)
            ...
            # orderby = action_line.ks_sort_by_field.name if action_line.ks_sort_by_field else "id"
            if (
                record.ks_chart_data_count_type == "count"
                and not self.ks_fill_temporal
                and not action_line.ks_sort_by_field
            ):
                orderby = "count"
            else:
                orderby = action_line.ks_sort_by_field.name if action_line.ks_sort_by_field else "id"
            ...
            ks_chart_data = record.ks_fetch_chart_data(
                record.ks_model_name,
                domain,
                ks_chart_measure_field_with_type,
                ks_chart_measure_field_with_type_2,
                ks_chart_measure_field,
                ks_chart_measure_field_2,
                ks_chart_groupby_relation_field,
                ks_chart_date_group_by,
                ks_chart_relation_type,
                orderby,
                limit,
                record.ks_chart_data_count_type,
                ks_chart_measure_field_ids,
                ks_chart_measure_field_2_ids,
                ks_chart_groupby_relation_field_id,
                ks_chart_data,
    ...
    @api.model
    def ks_get_start_end_date(
        self, model_name, ks_chart_groupby_relation_field, ttype, ks_chart_domain, ks_goal_domain
    ):
        ks_start_end_date = {}
        try:
            model_field_start_date = self.env[model_name].search(
                ks_chart_domain + [(ks_chart_groupby_relation_field, "!=", False)],
                limit=1,
        ...
        goal_model_start_date = self.env["ks_dashboard_ninja.item_goal"].search(
            ks_goal_domain, limit=1, order="ks_goal_date ASC"
        )["ks_goal_date"]
        goal_model_end_date = self.env["ks_dashboard_ninja.item_goal"].search(
            ks_goal_domain, limit=1, order="ks_goal_date DESC"
        )["ks_goal_date"]
    ...
    def generate_timeserise(self, date_begin, date_end, aggr, ftype="date"):
        ...
        self.env.cr.execute(
            query, {"timestamp_begin": date_begin, "timestamp_end": date_end, "aggr": aggr, "aggr1": "1 " + aggr}
        )
        dates = self.env.cr.fetchall()
        display_formats = {
    ...
    def ks_sort_sub_group_by_records(
        self, ks_data, field_type, ks_chart_date_groupby, ks_sort_by_order, ks_chart_date_sub_groupby
    ):
        if ks_data:
            reverse = False
                ...
                if field_type == "date_type":
                    if ks_chart_date_groupby in ["minute", "hour"]:
                        if ks_chart_date_sub_groupby in ["month", "week", "quarter", "year"]:
                            ...
                            data["value"].sort(
                                key=lambda x: int(
                                    str(ks_sorted_months.index(x["x"].split(" ")[2]) + 1)
                                    + x["x"].split(" ")[1]
                                    + x["x"].split(" ")[0].replace(":", "")
                                ),
                                reverse=reverse,
                        ...
                        ks_sorted_days = self.generate_timeserise(
                        ...
                        ks_sorted_months = self.generate_timeserise(
...
class KsDashboardItemsActions(models.Model):
    ...
    ks_item_action_field = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
    ...
    ks_item_action_date_groupby = fields.Selection(
        [
            ("minute", "Minute"),
            ("hour", "Hour"),
            ("day", "Day"),
    ...
    ks_chart_type = fields.Selection(
        [
            ("ks_bar_chart", "Bar Chart"),
            ("ks_horizontalBar_chart", "Horizontal Bar Chart"),
            ("ks_line_chart", "Line Chart"),
    ...
    ks_sort_by_order = fields.Selection([("ASC", "Ascending"), ("DESC", "Descending")], string="Sort Order")
...
class KsDashboardItemMultiplier(models.Model):
    ...
    ks_multiplier_fields = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),'|','|',"
        "('ttype','=','integer'),('ttype','=','float'),"
