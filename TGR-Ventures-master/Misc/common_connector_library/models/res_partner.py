...
class ResPartner(models.Model):
    ...
    def get_country(self, country_name_or_code):
        ...
        country = self.env["res.country"].search(
            ["|", ("code", "=ilike", country_name_or_code), ("name", "=ilike", country_name_or_code)], limit=1
        )
        return country
        ...
        if response:
            ...
            if not country:
                country = country_obj.create(
                    {"name": response.get("country"), "code": response.get("country abbreviation")}
                )
            ...
            if state_code:
                state = state_obj.search([("code", "=ilike", state_code), ("country_id", "=", country.id)], limit=1)
            elif state_name:
                state = state_obj.search([("name", "=ilike", state_name), ("country_id", "=", country.id)], limit=1)
            if not state and state_code:
                state = state_obj.create({"name": state_name, "code": state_code, "country_id": country.id})
        return state
