...
class ProductPricelist(models.Model):
    ...
    #     else:
    #         product_var = self.env['product.product'].search([('product_tmpl_id', '=', product_id[0].id)])
    #         product_var |= self.env['product.product'].search([('product_tmpl_id', '=', product_id[0].id),('active','=',False)])
    #         product = product_var.id
    #     # domain = [("pricelist_id", "=", self.id), ("product_id", "=", product_id), ("min_quantity", "=", min_qty)]
    #     domain = [("pricelist_id", "=", self.id), ("product_id", "=", product), ("min_quantity", "=", min_qty)]
    ...
    #         vals = self.prepre_pricelistitem_vals(product, min_qty, price)
    #         new_record = pricelist_item_obj.new(vals)
    #         new_record._onchange_product_id()
    #         new_vals = pricelist_item_obj._convert_to_write({name: new_record[name] for name in new_record._cache})
