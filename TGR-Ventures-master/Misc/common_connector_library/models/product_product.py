...
class ProductProduct(models.Model):
    ...
    def get_products_based_on_movement_date_ept(self, from_datetime, company):
        ...
        qry = """select product_id from stock_move_line where date >= '%s' and company_id = %d and
    ...
    def prepare_location_and_product_ids(self, warehouse, product_list):
        ...
        """
        locations = self.env["stock.location"].sudo().search([("location_id", "child_of", warehouse.lot_stock_id.ids)])
    ...
    def check_for_bom_products(self, product_ids):
        ...
        if mrp_module:
            qry = (
                """select p.id as product_id from product_product as p
                        inner join mrp_bom as mb on mb.product_tmpl_id=p.product_tmpl_id
                        and p.id in (%s)"""
                % product_ids
            )
    ...
    def prepare_free_qty_query(self, location_ids, simple_product_list_ids):
        ...
        query = """select pp.id as product_id,
                ...
                where pp.id in ({}) group by pp.id;""".format(
            location_ids,
    ...
    def prepare_forecasted_qty_query(self, location_ids, simple_product_list_ids):
        ...
        query = """select product_id,sum(stock) as stock from (select pp.id as product_id,
                ...
                from product_product pp
                left join stock_quant sq on pp.id = sq.product_id and sq.location_id in ({})
                where pp.id in ({}) group by pp.id
                union all
                select product_id as product_id, sum(product_qty) as stock from stock_move
                where state in ('assigned') and product_id in ({}) and location_dest_id in ({})
                group by product_id) as test group by test.product_id""".format(
            location_ids,
            simple_product_list_ids,
            simple_product_list_ids,
            location_ids,
