...
class SaleWorkflowProcess(models.Model):
    ...
    post_payment = fields.Boolean(string="Post Payment", default=False, help="If it's checked, Payment will be Posted.")
    ...
    sale_journal_id = fields.Many2one(
        "account.journal", string="Sales Journal", default=_default_journal, domain=[("type", "=", "sale")]
    )
    ...
    def auto_workflow_process_ept(self, auto_workflow_process_id=False, order_ids=[]):
        ...
        if not order_ids:
            ...
            orders = sale_order_obj.search(
                [("auto_workflow_process_id", "in", work_flow_process_records.ids), ("id", "in", order_ids)]
            )
