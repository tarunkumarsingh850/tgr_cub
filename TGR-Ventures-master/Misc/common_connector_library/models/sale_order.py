...
class SaleOrder(models.Model):
    ...
    def _compute_stock_move(self):
        ...
        """
        self.moves_count = self.env["stock.move"].search_count(
    ...
    def prepare_order_vals_after_onchange_ept(self, vals, fpos):
        ...
        order_vals = {
            ...
            "note": vals.get("customer_order_nbr", ""),
        }
        return order_vals
    ...
    def get_fiscal_position_by_warehouse(self):
        ...
        if warehouse and self.partner_id and self.partner_id.allow_search_fiscal_based_on_origin_warehouse:
            origin_country_id = (
                warehouse.partner_id and warehouse.partner_id.country_id and warehouse.partner_id.country_id.id or False
            )
            origin_country_id = origin_country_id or (
                warehouse.company_id.partner_id.country_id and warehouse.company_id.partner_id.country_id.id or False
            )
            is_amz_customer = getattr(self.partner_id, "is_amz_customer", False)

            fiscal_position = (
                self.env["account.fiscal.position"]
                .with_context({"origin_country_ept": origin_country_id, "is_amazon_fpos": is_amz_customer})
                .with_company(warehouse.company_id.id)
    ...
    def _prepare_invoice(self):
        ...
        if self.magento_website_id:
            invoice_vals.update({"magento_website_id": self.magento_website_id.id})
    ...
    def validate_order_ept(self):
        ...
        self.action_confirm()
        # pass the customer order number in order notes to picking notes
        if self.note:
            for pick in self.picking_ids:
                pick.additional_notes = self.note
        self.write({"date_order": date_order})
        if self.is_resend_order:
            for picking in self.picking_ids:
                picking.write(
                    {
                        "payment_status": "paid",
                        "is_resend_order": self.is_resend_order,
                        "resend_reason": self.resend_reason,
                    }
                )
        return True
        ...
        if work_flow_process_record.create_invoice:
            if work_flow_process_record.invoice_date_is_order_date:
                ...
                if self.date_order.date() <= fiscalyear_lock_date:
                    ...
                    if log_book_id:
                        message = (
                            "You cannot create invoice for order (%s) "
                        ...
                        self.env["common.log.lines.ept"].create(
                            {"message": message, "order_ref": self.name, "log_book_id": log_book_id}
                        )
            ...
            for line in invoices.invoice_line_ids:
                if line.product_id.detailed_type == "product" and self.magento_website_id.income_account_id:
                    line.write({"account_id": self.magento_website_id.income_account_id.id})
                elif line.product_id.default_code == "MAGENTO DISCOUNT" and self.magento_website_id.income_account_id:
                    line.update({"account_id": self.magento_website_id.income_account_id.id})
                elif line.product_id.default_code == "MAGENTO_SHIP" and self.magento_website_id.shipment_account_id:
                    line.update({"account_id": self.magento_website_id.shipment_account_id.id})
                elif (
                    line.product_id.default_code == "DeliveryIsurance"
                    and self.magento_website_id.delivery_isurance_account_id
                ):
                    line.update({"account_id": self.magento_website_id.delivery_isurance_account_id.id})
                elif line.product_id.detailed_type == "product" and self.shopify_instance_id.income_account_id:
                    line.write({"account_id": self.shopify_instance_id.income_account_id.id})
                elif line.product_id.default_code == "shopifyshippingproduct":
                    line.write({"account_id": self.shopify_instance_id.income_account_id.id})

            if work_flow_process_record.post_invoice:
                self.validate_invoice_ept(invoices)
            else:
                credit_customer_invoices = invoices.filtered(
                    lambda inv: inv.partner_id.is_credit_customer or inv.partner_id.parent_id.is_credit_customer
                )
                if credit_customer_invoices:
                    self.validate_invoice_ept(credit_customer_invoices)
            if work_flow_process_record.register_payment:
                self.paid_invoice_ept(invoices)
        elif self.magento_website_id.magento_website_id in ["3", "1", "17"]:
            invoices = self._create_invoices()
            for line in invoices.invoice_line_ids:
                if line.product_id.detailed_type == "product" and self.magento_website_id.income_account_id:
                    line.write({"account_id": self.magento_website_id.income_account_id.id})
    ...
    def check_for_bom_product(self, product):
        ...
        try:
            ...
            bom_point_dict = bom_obj.sudo()._bom_find(
                products=product, company_id=self.company_id.id, bom_type="phantom"
            )
    ...
    def create_and_done_stock_move_ept(self, order_line, customers_location, bom_line=False, vendor_location=False):
        ...
        if product and product_qty and product_uom:
            vals = self.prepare_val_for_stock_move_ept(
                product, product_qty, product_uom, vendor_location, customers_location, order_line, bom_line
            )
    ...
    def prepare_val_for_stock_move_ept(
        self, product, product_qty, product_uom, vendor_location, customers_location, order_line, bom_line
    ):
