...
class PowerbiReport(models.Model):
    ...
    _description = "Powerbi Reports"

    powerbi_id = fields.Char(string="Report Powerbi Id", required=True, size=50)
    name = fields.Char(string="Report Name")
    dataset_id = fields.Many2one("powerbi.dataset", string="Dataset", required=True)
        ...
        if connObj:
            if not self.embed_url:
                ...
                if resp.get("status"):
                    ...
                    pass
            scopes = ["Report.ReadWrite.All", "Dataset.ReadWrite.All"]
            embed_token = self.env["powerbi.synchronization"].get_embed_token("reports", self, connObj, scopes)
        return {
            "embed_url": self.embed_url,
            "embed_token": embed_token.get("token", ""),
            "token_expiry": embed_token.get("expiry", ""),
        }
