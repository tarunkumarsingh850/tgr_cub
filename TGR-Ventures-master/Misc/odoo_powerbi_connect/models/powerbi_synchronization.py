...
class PowerbiSynchronization(models.TransientModel):
    ...
    _description = "Powerbi Synchronization"
    ...
    def callPowerbiApi(self, url, method, data={}, token="", scope=[]):
        ...
        if not resp.ok:
            status = False
            message = str(resp_val or "Error during API call")
    ...
    def get_embed_token(self, ttype, dataObj, connObj, scopes):
        if dataObj.workspace_id.default_workspace:
            ...
            token_data = {"datasets": [{"id": dataObj.dataset_id.powerbi_id}], f"{ttype}": [{"id": dataObj.powerbi_id}]}
        else:
            url = (
                f"{connObj.api_url}/groups/{dataObj.workspace_id.powerbi_id}/{ttype}/{dataObj.powerbi_id}/GenerateToken"
            )
            token_data = {"accessLevel": "View"}
            ...
            self.env["powerbi.sync.history"].create(
                {
                    ...
                    "action": "b",
                    "action_on": "token",
                    "error_message": "Token generation error - " + str(resp.get("message", "")),
                }
            )
        return {}
