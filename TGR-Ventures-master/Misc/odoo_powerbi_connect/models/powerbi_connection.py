...
class PowerbiConnection(models.Model):
    ...
    def _default_instance_name(self):
        return self.env["ir.sequence"].next_by_code("powerbi.connection")

    instance = fields.Char(string="Instance Name", default=lambda self: self._default_instance_name())
    api_url = fields.Char(
        string="API URL", track_visibility="onchange", default=lambda self: self._default_api_url(), readonly=True
    )
    ...
    connection_status = fields.Boolean(string="Connection Status", default=False, readonly=True)
    ...
    def create(self, vals):
        ...
        if method == "mu":
            ...
            if not (vals.get("tenent_id", "") and vals.get("client_secret", "") and vals.get("client_id", "")):
                raise ValidationError(
                    _(r"Warning!\Tenent Id, Client Id and Client Secret are required in Serviceprinciple access method.")
                )
        vals["instance"] = self.env["ir.sequence"].next_by_code("powerbi.connection")
    ...
    def update_credentials(self):
        self.ensure_one()
        partial = self.env["powerbi.credentials.wizard"].create({"user": self.user})
        return {
    ...
    def open_dashboard_wizard(self):
        ...
        view_id = request.env.ref("odoo_powerbi_connect.id_powerbi_wizard_view_form").id
        return {
            "name": "Import Powerbi Dashboards",
            "view_mode": "form",
            "view_id": view_id,
            "res_model": "powerbi.wizard",
            "res_id": partial.id,
        ...
        if connection.get("status"):
            ...
            if resp.get("status"):
                ...
                for workspace in workspaces:
                    ...
                    if not workspaceObj:
                        res = workspaceModel.create({"powerbi_id": pid, "name": name, "is_published": True})
