...
class PowerbiWorkspace(models.Model):
    ...
    state = fields.Selection(
        [("topublish", "To Publish"), ("published", "Published")], compute="_compute_state", string="State"
    )
    ...
    def publish_to_powerbi(self):
        ...
        for expObj in self:
            ...
            resp = self.env["powerbi.synchronization"].callPowerbiApi(
                url, "post", data, connection.get("token", ""), scopes
            )
                ...
                self.env["powerbi.sync.history"].create(
                    {
                        ...
                        "action": "b",
                        "action_on": "workspace",
                        "error_message": "Workspace export error, id "
                        + str(expObj.id)
                        + ": Reason - "
    ...
    def _create_sale_order_data(self):
        ...
        if saleModule:
            ...
            dataset = self.env["powerbi.dataset"].create(
                {"name": "Odoo Sales", "dataset_type": "temp", "workspace_id": workspace.id}
            )
            table = self.env["powerbi.table"].create(
                {"name": "Sales", "dataset_id": dataset.id, "model_id": modelObj.id, "model_name": model}
            )
            self._create_table_column(
                model, table, {"name": "", "amount_total": "", "date_order": "", "partner_id": "name"}
            )
        return True

    @api.model
    def _create_invoice_data(self):
        invModule = self.env["ir.module.module"].search(
        ...
        if invModule:
            ...
            if not workspace:
                workspace = self.create({"name": "My Workspace", "default_workspace": True, "is_published": True})
            dataset = self.env["powerbi.dataset"].create(
                {"name": "Odoo Invoices", "dataset_type": "temp", "workspace_id": workspace.id}
            )
            table = self.env["powerbi.table"].create(
                {"name": "Invoices", "dataset_id": dataset.id, "model_id": modelObj.id, "model_name": model}
            )
            self._create_table_column(
                model,
                table,
                {"name": "", "amount_total": "", "invoice_date": "", "invoice_origin": "", "move_type": ""},
            )
        return True

    @api.model
    def _create_stock_data(self):
        stockModule = self.env["ir.module.module"].search(
            [("name", "=", "stock"), ("state", "=", "installed")], limit=1
        )
        if stockModule:
            ...
            if not workspace:
                workspace = self.create({"name": "My Workspace", "default_workspace": True, "is_published": True})
            dataset = self.env["powerbi.dataset"].create(
                {"name": "Odoo Inventory", "dataset_type": "temp", "workspace_id": workspace.id}
            )
            table = self.env["powerbi.table"].create(
                {"name": "Inventory", "dataset_id": dataset.id, "model_id": modelObj.id, "model_name": model}
            )
            self._create_table_column(model, table, {"name": "", "origin": "", "price_unit": "", "product_id": "name"})
        return True

    @api.model
    def _create_purchase_data(self):
        purchaseModule = self.env["ir.module.module"].search(
            [("name", "=", "purchase"), ("state", "=", "installed")], limit=1
        )
        if purchaseModule:
            ...
            if not workspace:
                workspace = self.create({"name": "My Workspace", "default_workspace": True, "is_published": True})
            dataset = self.env["powerbi.dataset"].create(
                {"name": "Odoo Purchase", "dataset_type": "temp", "workspace_id": workspace.id}
            )
            table = self.env["powerbi.table"].create(
                {"name": "Purchase", "dataset_id": dataset.id, "model_id": modelObj.id, "model_name": model}
            )
            self._create_table_column(
                model, table, {"name": "", "origin": "", "amount_total": "", "date_order": "", "partner_id": "name"}
            )
        return True

    @api.model
    def _create_pos_data(self):
        posModule = self.env["ir.module.module"].search(
        ...
        if posModule:
            ...
            if not workspace:
                workspace = self.create({"name": "My Workspace", "default_workspace": True, "is_published": True})
            dataset = self.env["powerbi.dataset"].create(
                {"name": "Odoo POS", "dataset_type": "temp", "workspace_id": workspace.id}
            )
            table = self.env["powerbi.table"].create(
                {"name": "POS", "dataset_id": dataset.id, "model_id": modelObj.id, "model_name": model}
            )
            self._create_table_column(
                model, table, {"name": "", "amount_total": "", "date_order": "", "partner_id": "name"}
            )
    ...
    def _create_table_column(self, model, table, fields):
        for field in fields.keys():
            field_id = self.env["ir.model.fields"].search(
                [("model_id.model", "=", model), ("name", "=", field)], limit=1
            )
            child = []
            if fields.get(field):
                ch_field_id = self.env["ir.model.fields"].search(
                    [("model_id.model", "=", field_id.relation), ("name", "=", fields.get(field))], limit=1
                )
