...
class PowerbiDataset(models.Model):
    ...
    dataset_type = fields.Selection([("perm", "Permanent"), ("temp", "Temporary")], string="Type", required=True)
    ...
    workspace_id = fields.Many2one("powerbi.workspace", string="Workspace", required=True)
    table_ids = fields.One2many("powerbi.table", inverse_name="dataset_id", string="Tables", readonly=True)
    ...
    def publish_to_powerbi(self):
        ...
        for expObj in self:
            ...
            if not tables:
                ...
                continue
            data = {"name": expObj.name, "defaultMode": "Push", "tables": tables}
                ...
                self.env["powerbi.sync.history"].create(
                    {
                        ...
                        "action": "b",
                        "action_on": "dataset",
                        "error_message": "Dataset export error, id "
                        + str(expObj.id)
                        + ": Reason - "
    ...
    def get_tables_data(self):
        ...
        for table in self.table_ids:
            table_data = {"name": table.name, "columns": self.get_table_columns(table)}
        ...
        for col in table.column_ids:
            if col.child_field_ids:
                for child in col.child_field_ids:
                    columns.append(
                        {"name": col.name + "." + child.name, "dataType": self.powerbiDatatypes[child.ttype]}
                    )
            else:
                columns.append({"name": col.name, "dataType": self.powerbiDatatypes[col.field_type]})
        return columns
