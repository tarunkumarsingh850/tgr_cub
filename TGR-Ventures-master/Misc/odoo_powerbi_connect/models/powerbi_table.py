...
class PowerbiTable(models.Model):
    ...
    dataset_id = fields.Many2one(
        "powerbi.dataset", string="Dataset", domain="[('is_published','!=',True)]", required=True
    )
    ...
    def onchange_model(self):
        ...
        if model == "sale.order":
            self._create_column(
                model,
                {"name": "", "amount_total": "", "date_order": "", "partner_id": "name", "order_line": "product_id"},
            )
        elif model == "account.move":
            self._create_column(
                model, {"name": "", "invoice_origin": "", "amount_total": "", "invoice_date": "", "move_type": ""}
            )
        elif model == "stock.move":
            self._create_column(model, {"name": "", "origin": "", "price_unit": "", "product_id": "name"})
                    ...
                    self.env["powerbi.sync.history"].create(
                        {
                            ...
                            "action": "b",
                            "action_on": "table",
                            "error_message": "Table data export error, id "
                            + str(expObj.id)
                            + ": Reason - "
    ...
    def get_row_data(self, dataObj):
        ...
        for col in self.column_ids.filtered(lambda l: l.field_type in ["one2many", "many2many", "many2one"]):
            if col.child_field_ids:
                ...
                if not Objs:
                    continue
                for child in col.child_field_ids.filtered(
                    lambda ch: ch.ttype not in ["one2many", "many2one", "many2many"]
                ):
                    rRows = []
    ...
    def delete_powerbi_data(self):
        ...
        for Obj in self:
            ...
            resp = self.env["powerbi.synchronization"].callPowerbiApi(
                url, "delete", token=connection.get("token", ""), scope=scopes
            )
            if resp.get("status"):
                ...
                self.env["powerbi.sync.history"].create(
                    {
                        ...
                        "action": "d",
                        "action_on": "table",
                        "error_message": "Table data delete error, id "
                        + str(Obj.id)
                        + ": Reason - "
                        + str(resp.get("message")),
                    }
    ...
    def update_table_schema(self):
        ...
        for Obj in self:
            ...
            if not columns:
                no_columns += 1
                continue
            data = {"name": Obj.name, "columns": columns}
            if Obj.dataset_id.workspace_id.default_workspace:
            ...
            resp = self.env["powerbi.synchronization"].callPowerbiApi(
                url, "put", data=data, token=connection.get("token"), scope=scopes
            )
            if resp.get("status"):
                ...
                self.env["powerbi.sync.history"].create(
                    {
                        ...
                        "action": "c",
                        "action_on": "table",
                        "error_message": "Table update error, id "
                        + str(Obj.id)
                        + ": Reason - "
                        + str(resp.get("message")),
                    }
    ...
    def create_server_action(self):
        ...
        existing_action = self.search([("model_id", "=", model_id), ("export_action", "!=", 0)], limit=1)
