...
class PowerbiWizard(models.TransientModel):
    ...
    workspace_id = fields.Many2one("powerbi.workspace", string="Workspace")
    ...
    exp_workspace_id = fields.Many2one("powerbi.workspace", string="Workspace")
    exp_table_id = fields.Many2one("powerbi.table", string="Table")
        ...
        return {
            "domain": {
                "exp_table_id": [
                    ...
                    ("is_published", "=", True),
                    ("dataset_id.workspace_id.id", "=", self.exp_workspace_id.id),
                ]
            }
        }
    ...
    def action_import_report(self):
        ...
        resp = self.env["powerbi.synchronization"].callPowerbiApi(
            url, "get", token=connection.get("token"), scope=scope
        )
        if resp.get("status"):
                ...
                self.env["powerbi.sync.history"].create(
                    {
                        ...
                        "action": "a",
                        "action_on": "report",
                        "error_message": "Report id(s) " + str(success) + " successfully imported.",
                    }
                )
            ...
            self.env["powerbi.sync.history"].create(
                {
                    ...
                    "action": "a",
                    "action_on": "report",
                    "error_message": "Report import error, Reason: " + resp.get("message"),
                }
            )
                ...
                self.env["powerbi.sync.history"].create(
                    {
                        ...
                        "action": "a",
                        "action_on": "dashboard",
                        "error_message": "Dashboard id(s) " + str(success) + " successfully imported.",
                    }
                )
            ...
            self.env["powerbi.sync.history"].create(
                {
                    ...
                    "action": "a",
                    "action_on": "dashboard",
                    "error_message": "Dashboard import error, Reason: " + resp.get("message"),
                }
            )
            return msgModel.genrated_message(resp.get("message", ""))

    def fetch_powerbi_report(self):
        msgModel = self.env["powerbi.message.wizard"]
        connObj = self.env["powerbi.connection"].get_active_connection()
        if not connObj:
            return msgModel.genrated_message("No active connection found!")
        reportObj = self.report_id
        partial = self.env["powerbi.report.wizard"].create({"powerbi_id": reportObj.powerbi_id, "name": reportObj.name})
        return {
            "name": "powerbi.report.view",
            "view_mode": "form",
            "view_id": False,
            "res_model": "powerbi.report.wizard",
            "res_id": partial.id,
    ...
    def fetch_powerbi_dashboard(self):
        msgModel = self.env["powerbi.message.wizard"]
        connObj = self.env["powerbi.connection"].get_active_connection()
        if not connObj:
            return msgModel.genrated_message("No active connection found!")
        dashboardObj = self.dashboard_id
        partial = self.env["powerbi.dashboard.wizard"].create(
            {"powerbi_id": dashboardObj.powerbi_id, "name": dashboardObj.name}
        )
        return {
            "name": "powerbi.dashboard.view",
            "view_mode": "form",
            "view_id": False,
            "res_model": "powerbi.dashboard.wizard",
            "res_id": partial.id,
            "type": "ir.actions.act_window",
            "context": self._context,
            "domain": "[]",
    ...
    def export_now(self, table, records):
        ...
        resp = self.env["powerbi.synchronization"].callPowerbiApi(
            url, "post", data=data, token=connection.get("token"), scope=scopes
        )
        if resp.get("status"):
