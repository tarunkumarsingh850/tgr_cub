...
class MagentoProductCategory(models.Model):
    ...
    is_active = fields.Boolean(
        string="Is Active?", help="Enable the category by choosing Yes for Is Active field.", default=True
    )
    complete_category_name = fields.Char(
        string="Category Name", help="Category Name", compute="_compute_complete_name", recursive=True
    )
    ...
    def _get_parent_category(self, instance, category):
        ...
        if parent_id:
            ...
            category = self.search([("category_id", "=", parent_id), ("instance_id", "=", instance.id)], limit=1)
            if not category:
    ...
    def create_magento_category(self, instance, category):
        ...
        m_category = self.search(
            [("category_id", "=", category.get("id", 0)), ("instance_id", "=", instance.id)], limit=1
        )
        if not m_category:
            if parent_category:
