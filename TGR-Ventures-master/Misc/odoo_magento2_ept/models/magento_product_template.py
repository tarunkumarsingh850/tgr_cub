...
class MagentoProductTemplate(models.Model):
    ...
    magento_product_template_id = fields.Char(string="Magento Product Id", help="Magento Product Id")
    ...
    odoo_product_template_id = fields.Many2one(
        comodel_name="product.template", ondelete="restrict", string="Odoo Product Template", required=True
    )
    ...
    magento_product_image_ids = fields.One2many(
        comodel_name="magento.product.image",
        inverse_name="magento_tmpl_id",
    ...
    active_template = fields.Boolean(string="Odoo Template Active", related="odoo_product_template_id.active")
    ...
    list_price = fields.Float(
        string="Sales Price", related="odoo_product_template_id.list_price", readonly=False, digits="Product Price"
    )
    standard_price = fields.Float(
        string="Cost", related="odoo_product_template_id.standard_price", readonly=False, digits="Product Price"
    )
    ...
    magento_tax_class = fields.Many2one(comodel_name="magento.tax.class", string="Tax Class", help="Magento Tax Class")
    website_instance_ids = fields.One2many("product.website.magento", "product_website_id", string="Website Lines")
    is_website = fields.Boolean("Is website", default=False, compute="_compute_magento_website_line")
    _sql_constraints = [
        (
            "_magento_template_unique_constraint",
    ...
    def _compute_total_magento_variant(self):
        for template in self:
            ...
            for each in template.with_prefetch().magento_product_ids:
                each.product_type = "simple"
    ...
    def __search_layer_template(self, line, item, values):
        ...
        if not template:
            template = (
                ...
                .search([("magento_sku", "=", item.get("sku")), ("magento_instance_id", "=", line.instance_id.id)])
                .magento_tmpl_id
            )
        if not template:
                    ...
                    .search(
                        [
                            (
                                "magento_sku",
                                "=",
                                item.get("extension_attributes").get("child_products")[0].get("simple_product_sku"),
                            ),
                            ("magento_instance_id", "=", line.instance_id.id),
                        ]
                    )
        ...
        if is_verify:
            ...
            for product in e_attributes.get("child_products", list()):
                for attribute in product.get("simple_product_attribute", list()):
                    attribute_dict = m_attribute.search_attribute_by_value(
                        instance, attribute, item.get("attribute_set_id")
                    )
    ...
    def _verify_attribute_count(self, line, item, o_template, e_attributes):
        ...
        if o_attr_count > 0 and o_attr_count != m_attr_count:
            ...
            message = (
                f"Product {item.get('sku')} having mismatch Attribute count. \n"
                f"Product having {m_attr_count} attribute at magento side "
                f"and {o_attr_count} Attribute at odoo side."
            )
            line.queue_id.log_book_id.write({"log_lines": [(0, 0, {"message": message, field_name: line.id})]})
            return False
        return True
    ...
    def _update_variant_sku(self, line, m_template, item, child_products, data):
        ...
        for variant in variants:
            ...
            if child:
                variant.write({"default_code": child.get("simple_product_sku", "")})
    ...
    def __add_variant(self, template, data):
        ...
        line = line.search([("product_tmpl_id", "=", template.id), ("attribute_id", "=", data.get("attribute_id"))])
    ...
    def create_layer_image(self, instance, images, template=False, variant=False):
        ...
        for image in images:
            ...
            if template:
                ...
                self._search_layer_image(
                    image, instance_id=instance.id, c_image_id=c_image.id, template_id=template.id, sequence=sequence
                )
            elif variant:
                odoo_variant = variant.odoo_product_id
                c_image = self._search_common_image(image, variant_id=odoo_variant.id)
                self._search_layer_image(
                    image, instance_id=instance.id, c_image_id=c_image.id, variant_id=variant.id, sequence=sequence
                )
        return True

    def _search_layer_image(self, image, **kwargs):
        m_image = self.env["magento.product.image"]
        domain = [("magento_instance_id", "=", kwargs.get("instance_id")), ("magento_image_id", "=", image.get("id"))]
    ...
    def get_website_category_attribute_tax_class(self, item, instance):
        ...
        m_set = m_set.search(
            [("attribute_set_id", "=", item.get("attribute_set_id")), ("instance_id", "=", instance.id)], limit=1
        )
        websites = item.get("extension_attributes", dict()).get("website_ids", list())
        if websites:
            website = website.search(
                [("magento_instance_id", "=", instance.id), ("magento_website_id", "in", websites)]
            )
        categories = category.get_categories(instance, item)
        tax_class = tax_class.search(
            [("magento_instance_id", "=", instance.id), ("magento_tax_class_id", "=", attribute.get("tax_class_id"))]
        )
        ...
        return {"website": website.ids, "attribute_set": m_set.id, "category": categories, "tax_class": tax_class.id}
    ...
    def __update_path(item, base_url):
        for image in item.get("media_gallery_entries", list()):
            image.update({"full_path": "{}{}{}".format(base_url, "catalog/product", image.get("file", ""))})
        return True
    ...
    def __prepare_template_values(self, instance, item, data, product=False, template=False):
        ...
        if product:
            if product._name == "product.product":
                ...
                values.update({"odoo_product_template_id": product[0].id})
        elif template:
            values.update({"odoo_product_template_id": template})
    ...
    #             instance.pricelist_id.set_product_price_ept(product_id.id, price.get("price"), min_qty=1)
    #         else:
    #             website = website.search(
    #                 [("magento_instance_id", "=", instance.id), ("magento_website_id", "=", price.get("website_id"))],
    #                 limit=1,
    #             )
    ...
    #         price_list.set_product_price_ept(product, price.get("price"), min_qty=1)
    #     return True
            ...
            for option in configurable_options:
                ...
                if attribute_data.get("frontend_label"):
                    odoo_attribute = self.env["product.attribute"].get_attribute(
                        attribute_data.get("frontend_label"), create_variant="always", auto_create=True
                    )
                    attr_val_ids = []
                    for option_values in attribute_data.get("opt_values"):
                        attrib_value = self.find_odoo_attribute_value_id(odoo_attribute, option_values)
    ...
    def unlink_product_from_magento(self):
        ...
        if m_product_ids:
            self.browse(m_product_ids).write({"sync_product_with_magento": False, "magento_product_template_id": ""})
        else:
    ...
    def get_magento_attribute_set(product, product_type, is_it_child, attribute_set_id, common_log_book_id):
        if product_type == "simple" and is_it_child and not attribute_set_id:
            ...
            attribute_set = (
                attribute_set_id.attribute_set_id if attribute_set_id else product.attribute_set_id.attribute_set_id
            )
        else:
    ...
    def get_magento_product_price(
        self, instance, conf_simple_product, magento_is_set_price, website, currency, common_log_book_id
    ):
        product_price = 0
        ...
        if magento_is_set_price and instance.catalog_price_scope == "global":
            ...
            if price_list:
                ...
                product_price = self.get_scope_wise_product_price(
                    instance, common_log_book_id, conf_simple_product, price_list
                )
    ...
    def get_scope_wise_product_price(instance, common_log_book_id, product, price_list):
        ...
        if price_list:
            ...
            common_log_book_id.write(
                {"log_lines": [(0, 0, {"message": "Still price list not set for the Instance : %s" % instance.name})]}
            )
        return product_price
    ...
    def get_simple_product(is_it_child, product, product_type=False):
        if (not product_type or product_type and product_type == "simple") and is_it_child:
            ...
            conf_simple_product_product = (
                product.magento_product_ids[0] if len(product.magento_product_ids) > 1 else product.magento_product_ids
            )
    ...
    def prepare_export_images_values(self, simple_product_tmpl, product_type, is_it_child):
        ...
        if magento_product_images and not main_product_img and product_type == "simple" and is_it_child:
            main_product_img = simple_product_tmpl.magento_tmpl_id.magento_product_image_ids.filtered(
                lambda x: x.sequence == 0 and not x.magento_product_id
            ).image

        sku = (
            simple_product_tmpl.magento_sku
            if simple_product_tmpl.magento_sku
    ...
    def prepare_export_image_values_dict(self, image, sku, temp, media_gallery, is_main_img):
        ...
        img_name = sku + "_{}.{}".format(temp, cimg_format)
    ...
    def prepare_image_vals_ept(self, img_base64, name, format, is_main_img=False):
        ...
        img_val = {
            ...
            "file": "",
            "content": {"Base64EncodedData": img_base64, "Type": "image/%s" % format, "Name": "%s" % name},
        }
        return img_val
    ...
    def translate_description(
        export_product_data, store_view, custom_attributes, simple_product_tmpl, configurable_custom_attributes=[]
    ):
        """
        ...
        for attribute in custom_attributes:
            ...
            if attribute.get("attribute_code") == "short_description":
                product_short_description = simple_product_tmpl.with_context(
                    lang=store_view.lang_id.code
                ).short_description
                export_product_data["product"]["custom_attributes"].append(
                    {"attribute_code": "short_description", "value": product_short_description}
                )
        export_product_data["product"]["name"] = simple_product_tmpl.with_context(
            lang=store_view.lang_id.code
        ...
        try:
            ...
            data = {
                "product": {
                    ...
                    "extension_attributes": {"website_ids": website_id},
    ...
    def write_product_id_in_odoo(self, response, product_type, is_it_child, product):
        ...
        """
        _logger.info(
            "Product created in magento successfully with Product SKU : %s and "
    ...
    def prepare_configurable_option_vals(
        self, conf_product_sku, common_log_book_id, instance, product, attribute_set_id
    ):
        """
        ...
        for line in attribute_line:
            ...
            attribute_options = self.get_magento_attribute_options(magento_att, instance, line.value_ids)
            ...
            if not is_attribute_assigned and attribute_set_id:
                ...
                if skip:
                    return True
            if magento_att:
                self.export_attribute_in_magento(magento_att, line, conf_product_sku, instance, common_log_book_id)
            else:
    ...
    def export_attribute_in_magento(self, magento_att, line, conf_product_sku, instance, common_log_book_id):
        """
            ...
            common_log_book_id.write(
                {
                    "log_lines": [
                        (
                            ...
                            0,
                            {
                                "message": "Attribute options can not able to assign in product SKU : %s in magento"
                                % conf_product_sku,
            ...
            common_log_book_id.write(
                {
                    "log_lines": [
                        (
                            ...
                            0,
                            {
                                "message": "In Magento can not able to  create new attribute for product SKU : %s"
                                % sku,
                                "default_code": sku,
                            },
                        )
    ...
    @staticmethod
    def check_attribute_in_magento(instance, attribute_set_id, magento_attribute_id):
        """
    ...
    def assign_attribute_in_attribute_set(
        self, instance, product, attribute_set_id, common_log_book_id, attribute_code
    ):
    ...
    def export_attribute_options(self, magento_attribute, instance, common_log_book_id, sku, attribute_options):
        """
    ...
    @staticmethod
    def bind_simple_with_configurable_product(instance, product_magento_sku_list, conf_product_sku, common_log_id):
        """
        ...
        for product_magento_sku in product_magento_sku_list:
            try:
                data = {"childSku": product_magento_sku}
                ...
                """
                common_log_id.write({"log_lines": [(0, 0, {"message": message, "default_code": product_magento_sku})]})
        return True
    ...
    def prepare_export_stock_data(self, product_stock, instance, log, layer_ids, source_code=False, msi=False):
        """
        ...
        for product_id, stock in product_stock.items():
            product = [
                product for product in layer_ids if product.get("id") == product_id and product.get("magento_sku")
            ][0]
            if product and stock >= 0.0:
                if product.get("type") != "product":
                    ...
                    if msi:
                        ...
                        stock_data.append({"sku": product.get("magento_sku"), "qty": stock, "is_in_stock": 1})
            if consumable:
    ...
    def open_product_in_magento(self):
        ...
        if m_url:
            return {
                ...
                "url": "{}/catalog/product/edit/id/{}".format(m_url, m_product_id),
                "target": "new",
            }

    @api.model
    def default_get(self, fields):
        result = super(MagentoProductTemplate, self).default_get(fields)
    ...
    def _compute_magento_website_line(self):
        ...
        if self.website_instance_ids:
            for line in self.website_instance_ids:
                if line.website_instance_id.name == "Tiger One UK":
                    if self.wholesale_uk:
                        line.price_unit = self.wholesale_uk
    ...
    # def action_update_price_magento(self):
    #     instance = (
    #         request.env["magento.instance"].sudo().search([],limit=1)
    #     )
    #     api_url = instance.magento_price_update_url
    #     headers = self.get_headers(instance.access_token)

    #     products = self.env["magento.product.template"].search([])
    #     for line in products:
    #         for res in line.website_instance_ids:
    #             if res.price_unit > 0:
    #                         data = {"prices": [{
    #                                 "price": res.price_unit,
    #                                 "store_id": res.website_instance_id.magento_website_id,
...
class ProductWebsiteMagento(models.Model):
    _name = "product.website.magento"

    product_website_id = fields.Many2one("magento.product.template", string="Website ID")
