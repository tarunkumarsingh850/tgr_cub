...
class DeliveryCarrier(models.Model):
    ...
    magento_carrier = fields.Many2one(
        comodel_name="magento.delivery.carrier",
    ...
    magento_carrier_code = fields.Char(
        compute="_compute_carrier_code", string="Base Carrier Code", help="Magento Carrier Code"
    )
    ...
    def _check_magento_carrier_id(self):
        ...
        if delivery_carrier_obj:
            raise UserError(
                _("Can't set this same Magento carrier " "with multiple Delivery Methods for the same Magento Instance")
            )
        ...
        if method:
            ...
            if magento_carrier:
                ...
                if not carrier:
                    ...
                    carrier.create(
                        {"name": carrier_label, "product_id": product.id, "magento_carrier": magento_carrier.id}
                    )
                item.update({"magento_carrier_id": magento_carrier.id, "delivery_carrier_id": carrier.id})
        return True

    def __find_magento_carriers(self, instance, shipping_method):
        carrier = self.env["magento.delivery.carrier"]
        carrier = carrier.search(
            [("carrier_code", "=", shipping_method), ("magento_instance_id", "=", instance.id)], limit=1
        )
        if not carrier:
