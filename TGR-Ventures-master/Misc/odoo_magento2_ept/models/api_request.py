            ...
            if verify_ssl:
                if data:
                    ...
                    response = getattr(requests, method)(
                        url=api_url, headers=headers, data=data, verify=True, params=params
                    )
                else:
                    response = getattr(requests, method)(url=api_url, headers=headers, verify=True, params=params)
            else:
                if data:
                    ...
                    # If we all the data = '' as blank then also it gives an error from Magento end.
                    data = json.dumps(r)
                    response = getattr(requests, method)(url=api_url, headers=headers, data=data, params=params)
                else:
                    response = getattr(requests, method)(url=api_url, headers=headers, params=params)
...
def handle_response(response, is_raise=False):
    ...
    if response.status_code == 401:
        ...
        raise UserError(_("Given Credentials are incorrect, Kindly use correct Credentials."))
...
def get_common_error_message(error=""):
    ...
    2. There is no API change plugin installed on Magento2 or the plugin is not
...
def create_search_criteria(filters, **kwargs):
    ...
    if not filters:
        searchcriteria = {
            "searchCriteria": {"filterGroups": [{"filters": [{"field": "id", "value": -1, "condition_type": "gt"}]}]}
        }
    else:
