...
class MagentoAttributeGroup(models.Model):
    ...
    attribute_group_id = fields.Char(string="Attribute Group", help="Magento Attribute Group ID")
    sort_order = fields.Integer(string="Attribute Sort Order", readonly=True)
    attribute_set_id = fields.Many2one(comodel_name="magento.attribute.set", string="Magento Attribute Set")
    magento_attribute_ids = fields.Many2many(
        comodel_name="magento.product.attribute", string="Magento Attribute IDs", help="Magento Attribute"
    )
    ...
    def create_attribute_group(self, instance, attr_set, group):
        ...
        m_group = self.search([("attribute_group_id", "=", group_id), ("instance_id", "=", instance.id)], limit=1)
        if not m_group:
            m_group = self.create(
                {
