...
class MagentoProductImage(models.Model):
    ...
    odoo_image_id = fields.Many2one(
        comodel_name="common.product.image.ept", ondelete="cascade", string="Odoo Images", help="Odoo Images"
    )
    ...
    magento_image_id = fields.Char(string="Magento Product Image Id", help="Magento Product Image ID")
    magento_product_id = fields.Many2one(
        comodel_name="magento.product.product", string="Magento Product", ondelete="cascade", help="Magento Product"
    )
    magento_tmpl_id = fields.Many2one(
        comodel_name="magento.product.template",
        string="Magento Template",
        ondelete="cascade",
        help="Magento Product Template",
    )
    name = fields.Text(string="File Name", help="File name")
    image = fields.Image(related="odoo_image_id.image", string="Product Image", help="Magento Product Image")
    url = fields.Char(string="File Location", help="Image URL")
    sequence = fields.Integer(
        string="Image Sequence", help="The sequence number will use this to order the product images"
    )
