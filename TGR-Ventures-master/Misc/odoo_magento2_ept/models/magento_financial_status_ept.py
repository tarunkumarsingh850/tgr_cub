...
class MagentoFinancialStatusEpt(models.Model):
    ...
    _sql_constraints = [
        (
            "_magento_workflow_unique_constraint",
            "unique(financial_status,magento_instance_id,payment_method_id,company_id)",
    ...
    def create_financial_status(self, magento_instance, financial_status):
        ...
        for payment_method in payment_methods:
            ...
            if payment_method.magento_workflow_process_id:
                ...
                workflow = self.env["sale.workflow.process.ept"].search(
            ...
            all_companies = self.env["magento.website"].search([]).mapped("company_id")
            for each_company in all_companies:
                workflow = self.env["sale.workflow.process.ept"].search([("company_id", "=", each_company.id)], limit=1)
                self.create(
                    {
                        "magento_instance_id": magento_instance.id,
    ...
    def search_financial_status(self, order_response, magento_instance, payment_option):
        ...
        mapped_order_status = order_status_ojb.search(
            [
                ("magento_instance_id", "=", magento_instance.id),
                ("m_order_status_code", "=", order_response.get("status")),
            ]
        )
        if not mapped_order_status:
            mapped_order_status = order_status_ojb.search(
                [
                    ("magento_instance_id", "=", magento_instance.id),
                    ("m_order_status_code", "=", "complete"),
                ]
            )
        ...
        self.__get_status_code(mapped_order_status.main_status, is_invoice, is_shipment)
