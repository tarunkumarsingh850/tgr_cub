...
class MagentoProductAttribute(models.Model):
    ...
    odoo_attribute_id = fields.Many2one(
        comodel_name="product.attribute", required=False, string="Odoo Attributes", ondelete="cascade"
    )
    ...
    def import_magento_attributes(self, instance, attr_sets, is_raise=False):
        ...
        for attr_set in attr_sets.get("items", []):
            ...
            for attribute in attributes:
                if (attribute.get("attribute_code") == "brand") and attribute.get("options", []):
                    ...
                    if (attribute.get("attribute_code") == "brand") and attribute.get("options", []):
                        brand_master = self.env["product.breeder"]
                        existing_brands_magento_ids = brand_master.search([("magento_id", "!=", False)]).mapped(
                            "magento_id"
                        )
                        for incoming_brand_details in attribute.get("options", []):
                            incoming_magento_id = incoming_brand_details
                            if incoming_magento_id["value"] not in existing_brands_magento_ids:
                                brand_vals = {
                                    "breeder_name": incoming_brand_details["label"],
                                    "magento_id": incoming_brand_details["value"],
                                }
                                brand_master.create(brand_vals)
                if (attribute.get("attribute_code") == "seeds_flowering_type") and attribute.get("options", []):
                    self.__update_attribute_type(attribute)
                    m_attribute = self._search_layer_attribute(attribute, instance)
                    o_attribute = self._search_odoo_attribute(attribute, m_attribute)
                    m_attribute.write({"odoo_attribute_id": o_attribute.id})
    ...
    def _search_odoo_attribute(self, attribute, m_attribute):
        ...
        if not p_attribute:
            p_attribute = p_attribute.search(
                [("name", "=", attribute.get("default_frontend_label")), ("magento_attribute_id", "=", False)], limit=1
            )
            if not p_attribute:
    ...
    def search_attribute_by_value(self, instance, attribute, set_id):
        ...
        m_attribute = self.search(
            [("name", "=ilike", attribute.get("label")), ("instance_id", "=", instance.id)], limit=1
        )
        if not m_attribute:
    ...
    def __prepare_sets(set_id):
        return {"items": [{"attribute_set_id": set_id}]}
