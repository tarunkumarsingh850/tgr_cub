...
class MagentoInstance(models.Model):
    ...
    def _default_set_import_product_category(self):
        return (
            self.env.ref("product.product_category_all").id if self.env.ref("product.product_category_all") else False
        )
    ...
    def _default_magento_discount_product(self):
        ...
        if not discount_product:
            raise UserError(
                _(
                    "Please upgrade the module and then try to create new instance.\n"
    ...
    def set_magento_import_after_date(self):
        ...
        for instance in instances:
            if not instance.import_order_after_date:
                order = (
                    sale_order_obj.search([("magento_instance_id", "=", instance.id)], order="date_order asc", limit=1)
                    or False
                )
                if order:
    ...
    catalog_price_scope = fields.Selection(
        ...
        help="Scope of Price in Magento",
        default="global",
    )
    pricelist_id = fields.Many2one(
        comodel_name="product.pricelist", string="Pricelist", help="Product Price is set in selected Pricelist"
    )
    ...
    allow_import_image_of_products = fields.Boolean(
        comodel_name="Import Images of Products",
        default=False,
    ...
    last_order_import_date = fields.Datetime(
        string="Last Shipped Orders import date", help="Last Shipped Orders import date"
    )

    last_unshipped_order_import_date = fields.Datetime(
        string="Last Unshipped Orders import date", help="Last Unshipped Orders import date"
    )

    last_cancel_order_import_date = fields.Datetime(
        string="Last Cancel Orders import date", help="Last Cancel Orders import date"
    )
    ...
    is_import_product_stock = fields.Boolean(
        string="Is Import Magento Product Stock?", default=False, help="Import Product Stock from Magento to Odoo"
    )
    ...
    company_id = fields.Many2one(comodel_name="res.company", string="Magento Company", help="Magento Company")
    invoice_done_notify_customer = fields.Boolean(
        string="Invoices Done Notify customer", default=False, help="while export invoice send email"
    )
    is_multi_warehouse_in_magento = fields.Boolean(
        string="Is Multi Warehouse in Magento?", default=False, help="If checked, Multi Warehouse used in " "Magento"
    )
    # Require filed for cron
    auto_import_sale_orders = fields.Boolean(
        string="Auto Import Shipped Sale Orders?",
        default=False,
    ...
    auto_import_cancel_sale_orders = fields.Boolean(
        string="Auto Import Cancel Sale Orders?", default=False, help="This Field relocate auto import sale orders."
    )
    ...
    auto_export_product_stock = fields.Boolean(
        string="Auto Export Product Stock?", help="Automatic Export Product Stock"
    )
    ...
    auto_export_shipment_order_status = fields.Boolean(
        string="Auto Export Shipment Information?", help="Automatic Export Shipment Information"
    )
    payment_method_ids = fields.One2many(
        comodel_name="magento.payment.method", inverse_name="magento_instance_id", help="Payment Methods for Magento"
    )
    shipping_method_ids = fields.One2many(
        comodel_name="magento.delivery.carrier", inverse_name="magento_instance_id", help="Shipping Methods for Magento"
    )
    ...
    import_product_category = fields.Many2one(
        ...
        help="While importing a product, the selected " "category will set in that product.",
    )
    ...
    delivery_isurance_product_id = fields.Many2one(
        comodel_name="product.template",
        string="Delivery Insurance Product",
        domain=[("type", "=", "service")],
        help="This is used for set insurance product in a Carrier",
    )
    surcharge_product_id = fields.Many2one(
        comodel_name="product.template",
        string="Surcharg Product",
        domain=[("type", "=", "service")],
        help="This is used for set surcharge product in a Carrier",
    )
    ...
    magento_verify_ssl = fields.Boolean(
        string="Verify SSL", default=False, help="Check this if your Magento site is using SSL " "certificate"
    )
    active_user_ids = fields.One2many(
        comodel_name="magento.api.request.page",
        inverse_name="magento_instance_id",
        string="Active Users",
        help="Active Users",
    )
    magento_apply_tax_in_order = fields.Selection(
        [("odoo_tax", "Odoo Default Tax Behaviour"), ("create_magento_tax", "Create New Tax If Not Found")],
        copy=False,
    ...
    magento_invoice_tax_account_id = fields.Many2one(
        comodel_name="account.account", string="Invoice Tax Account For Magento Tax"
    )
    magento_credit_tax_account_id = fields.Many2one(
        comodel_name="account.account", string="Credit Note Tax Account For Magento Tax"
    )
    ...
    activity_type = fields.Many2one(comodel_name="mail.activity.type", string="Activity Type")
    is_create_activity = fields.Boolean(string="Is Create Schedule Activity?")
    activity_user_ids = fields.Many2many(
        comodel_name="res.users", string="Activity User", help="Schedule activity will be create against this user."
    )
    activity_lead_days = fields.Integer(string="Lead Days", help="Activity will be created after this days.")
    # Export Product
    batch_size = fields.Integer(string="Export Stock Batch Size", default=200, help="Export product batch size.")
    ...
    magento_price_update_url = fields.Char("Magento Price Update URL")
    magento_stock_update_url = fields.Char("Magento Stock Update URL")
    last_product_update_date = fields.Datetime("Last Product Update Date", copy=False)
    last_config_product_update_date = fields.Datetime("Last Product(Configurable) Update Date", copy=False)

    _sql_constraints = [
        (
            "unique_magento_host",
            "unique(magento_url, access_token)",
    ...
    cron_count = fields.Integer(
        string="Scheduler Count", compute="_compute_get_scheduler_list", help="This Field relocates Scheduler Count."
    )
    ...
    def sync_website(self):
        ...
        for data in website_response:
            ...
            if magento_website_id != 0:
                ...
                website_name = mage_website_id.name
                website_name = self.env["magento.website"].search([("name", "=", data.get("name"))])
                if not website_name:
                    self.env["magento.website"].sudo().create(
                        {
                            "name": data.get("name"),
    ...
    def search_magento_website_id(self, magento_website_id):
        return self.magento_website_ids.filtered(
            ...
            and x.magento_instance_id.id == self.id
        ...
        for storeview_data in response:
            ...
            if magento_storeview_id != 0:
                storeview = storeview_obj.search(
                    [("magento_storeview_id", "=", magento_storeview_id), ("magento_instance_id", "=", self.id)]
                )
    ...
    def update_pricelist_in_website(self, storeview_data):
        ...
        odoo_website_id = website_obj.search(
            [("magento_website_id", "=", storeview_data.get("website_id")), ("magento_instance_id", "=", self.id)],
            limit=1,
        )
        if odoo_website_id:
    ...
    def import_payment_method(self):
        ...
        for payment_method in payment_methods:
            ...
            new_payment_method = payment_method_obj.search(
                [("payment_method_code", "=", payment_method_code), ("magento_instance_id", "=", self.id)]
            )
        ...
        for delivery_method in delivery_methods:
            for method_value in delivery_method.get("value"):
                ...
                new_delivery_carrier = delivery_method_obj.search(
                    [("carrier_code", "=", delivery_method_code), ("magento_instance_id", "=", self.id)]
                )
            ...
            if response.get("items"):
                for inventory_location in response.get("items"):
                    ...
                    magento_location = magento_inventory_location_obj.search(
                        [("magento_location_code", "=", location_code), ("magento_instance_id", "=", self.id)]
                    )
    ...
    def import_currency(self):
        ...
        for active_currency in magento_currency.get("exchange_rates"):
            currency_id = currency_obj.with_context(active_test=False).search(
                [("name", "=", active_currency.get("currency_to"))], limit=1
            )
    ...
    @api.model
    def _scheduler_import_unshipped_sale_orders_long_range_days(self, args=None):
        """
        This method is used to import sale order from Magento via cron job.
        :param args: arguments to import pending and processing sale orders
        :return:
        ...
        magento_instance = self.env["magento.instance"]
        magento_instance_id = args.get("magento_instance_id")
        no_of_days = args.get("days")
        if magento_instance_id:
            instance = magento_instance.browse(magento_instance_id)
            instance.import_magento_order_status_ids.mapped("status")
            last_unshipped_order_import_date = instance.last_unshipped_order_import_date
            if not last_unshipped_order_import_date:
                last_unshipped_order_import_date = ""
            from_date = last_unshipped_order_import_date - timedelta(days=no_of_days)
            to_date = datetime.now()
            magento_order_data_queue_obj.create_order_queues(
                instance=instance,
                from_date=from_date,
    ...
    # SCHEDULER TO GET THE PROCESS ORDERS
    @api.model
    def _scheduler_import_process_sale_orders(self):
        """
        This method is used to import sale order from Magento via cron job.
        :param args: arguments to import complete sale orders
        :return:
        ...
        magento_order_data_queue_obj = self.env["magento.order.data.queue.ept"]
        magento_instance = self.env["magento.instance"].search([], limit=1)
        if magento_instance:
            last_order_import_date = magento_instance.last_order_import_date
            if not last_order_import_date:
                last_order_import_date = ""
            from_date = last_order_import_date
            to_date = datetime.now()
            magento_order_data_queue_obj.create_order_queues(
                instance=magento_instance,
                from_date=from_date,
                to_date=to_date,
                status="processing",
            )
    ...
    def _scheduler_import_cancel_sale_orders(self, args=None):
        ...
        if magento_instance_id:
            ...
            to_date = datetime.now()
            sale_order.import_cancel_order(instance=instance, from_date=from_date, to_date=to_date, status="canceled")
    ...
    def _scheduler_import_product(self, args=None):
        ...
        if instance_id:
            ...
            if instance.last_product_import_date:
                from_date = instance.last_product_import_date - timedelta(days=1)
            to_date = datetime.now()
            ...
            instance.write({"last_product_import_date": datetime.now()})
        return True
    ...
    def _scheduler_update_order_status(self, args=None):
        ...
        for picking in pickings:
            picking.magento_send_shipment(raise_error=False)
        return True
    ...
    def search_magento_instance(self):
        ...
        if not instance:
            instance = self.search(
                [("company_id", "=", company.id), ("is_onboarding_configurations_done", "=", False)],
                limit=1,
                order="id desc",
            )
    ...
    website_display_currency = fields.Many2one(
        "res.currency", readonly=True, help="Display currency of the magento website."
    )
    ...
    def get_total_orders(self, record, state=False):
        ...
        def orders_of_current_week(record):
            self._cr.execute(
                """select id from sale_order where date(date_order)
    ...
    def get_magento_invoice_records(self, state):
        ...
        invoice_query = """select account_move.id
            ...
            where sale_order.magento_instance_id={}
            and account_move.state in ('{}')
            and account_move.move_type in ('out_invoice','out_refund')""".format(
            self.id,
            state,
        )
    ...
    def perform_operation(self, record_id):
        ...
        """
        view = (
            self.env.ref("odoo_magento2_ept." "action_wizard_magento_instance_import_export_operations")
            .sudo()
            .read()[0]
        )
        action = self.prepare_action(view, [])
    ...
    def open_report(self, record_id):
        ...
        action.update(
            {
                "context": {
                    "search_default_magento_instances": record_id,
    ...
    def get_graph_data(self, record):
        ...
        def get_current_week_date(record):
            self._cr.execute(
                """SELECT to_char(date(d.day),'DAY'), t.amount_untaxed as sum
                                    FROM  (
                                       SELECT day
        ...
        def graph_of_current_month(record):
            self._cr.execute(
                """select EXTRACT(DAY from date(date_day)) :: integer,sum(amount_untaxed) from (
                            SELECT
        ...
        def graph_of_current_year(record):
            self._cr.execute(
                """select TRIM(TO_CHAR(DATE_TRUNC('month',month),'MONTH')),sum(amount_untaxed) from
        ...
        def graph_of_all_time(record):
            self._cr.execute(
                """select TRIM(TO_CHAR(DATE_TRUNC('month',date_order),'YYYY-MM')),sum(amount_untaxed)
    ...
    def get_compared_week_data(self, record):
        ...
        self._cr.execute(
            """select sum(amount_untaxed) as current_week from sale_order
        ...
        # Previous week data
        self._cr.execute(
            """select sum(amount_untaxed) as previous_week from sale_order
    ...
    def get_compared_month_data(self, record):
        ...
        self._cr.execute(
            """select sum(amount_untaxed) as current_month from sale_order
                                where date(date_order) >= (select date_trunc('month', date(current_date)))
                                and magento_instance_id=%s and state in ('sale','done')"""
            % (record.id)
        )
        ...
        # Previous week data
        self._cr.execute(
            """select sum(amount_untaxed) as previous_month from sale_order where date(date_order)
    ...
    def get_compared_year_data(self, record):
        ...
        self._cr.execute(
            """select sum(amount_untaxed) as current_year from sale_order
        ...
        # Previous week data
        self._cr.execute(
            """select sum(amount_untaxed) as previous_year from sale_order where date(date_order)
    ...
    def create_schedule_activity(self, queue, note):
        if queue.process_count == 3 and queue.instance_id.is_create_activity:
            self.create_activity(
                model_name=queue._name, res_id=queue.id, message=note, summary=queue.name, instance=queue.instance_id
            )
    ...
    def show_popup_notification(self, message):
        self.env["bus.bus"]._sendone(
            ...
            "simple_notification",
            {"title": "Magento Connector", "message": message, "sticky": False, "warning": True},
        )
        return True
