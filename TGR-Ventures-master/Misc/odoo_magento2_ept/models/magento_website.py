...
class MagentoWebsite(models.Model):
    ...
    import_partners_from_date = fields.Datetime(
        string="Last partner import date", help="Date when partner last imported"
    )
    ...
    currency_id = fields.Many2one(
        comodel_name="res.currency", related="pricelist_id.currency_id", readonly=True, help="Currency"
    )
    magento_base_currency = fields.Many2one(
        comodel_name="res.currency", readonly=True, help="Magento Website Base Currency"
    )
    ...
    website_display_currency = fields.Many2one(
        comodel_name="res.currency", readonly=True, help="Display currency of the magento website."
    )
    ...
    shipment_account_id = fields.Many2one(
        string="Shipment Account",
        comodel_name="account.account",
    )
    delivery_isurance_account_id = fields.Many2one(
        string="Delivery Isurance Account",
        comodel_name="account.account",
    )
    ...
    def get_total_products(self, record, exported, product_type=False):
        ...
        where magento_product_template_magento_website_rel.magento_website_id = {} and
        magento_product_template.magento_instance_id = {}""".format(
            record.id,
            record.magento_instance_id.id,
        )
        product_domain = []
    ...
    def get_customers(self, record):
        ...
        main_sql = """select DISTINCT(rp.id) as partner_id from res_partner as rp
                        ...
                        where mp.magento_website_id = {} and
                        mp.magento_instance_id = {}""".format(
            record.id,
            record.magento_instance_id.id,
        )
    ...
    def get_magento_invoice_records(self, state):
        ...
        where sale_order.magento_website_id={}
        and sale_order.magento_instance_id={}
        and account_move.state in ('{}')
        and account_move.move_type in ('out_invoice','out_refund')""".format(
            self.id,
            self.magento_instance_id.id,
            state,
        )
    ...
    def open_report(self, record_id):
        ...
        action = self.prepare_action(
            view, [("magento_instance_id", "=", website.magento_instance_id.id), ("magento_website_id", "=", record_id)]
        )

        action.update(
            {
                "context": {
                    "search_default_magento_websites": record_id,
