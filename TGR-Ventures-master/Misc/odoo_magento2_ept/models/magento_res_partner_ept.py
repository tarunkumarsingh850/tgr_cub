...
key_list = ["name", "street", "street2", "city", "zip", "phone", "state_id", "country_id", "company_name"]
...
class MagentoResPartnerEpt(models.Model):
    ...
    magento_website_id = fields.Many2one(
        comodel_name="magento.website", string="Magento Website", help="Magento Website"
    )
    ...
    def create_magento_customer(self, line, is_order=False):
        ...
        if not customer:
            ...
            values = self._prepare_magento_customer_values(
                partner_id=partner.id, instance=instance, data=data, customer_id=data.get("id")
            )
    ...
    def __search_customer(self, **kwargs):
        ...
        if kwargs.get("child"):
            ...
            return self.search(
                [
                    ("magento_customer_id", "=", kwargs.get("id")),
                    ("magento_instance_id", "=", kwargs.get("instance_id")),
                    ("partner_id.active", "=", True),
                ],
                limit=1,
    ...
    def _prepare_magento_customer_values(**kwargs):
        ...
        website = instance.magento_website_ids.filtered(lambda w: int(w.magento_website_id) == data.get("website_id"))
        return {
    ...
    def _find_state_country(self, data):
        ...
        state = partner.create_or_update_state_ept(data.get("country_id"), state_code, zip_code, country_obj=country)
        return {"country_id": country.id, "state_id": state.id}
    ...
    def _create_customer_addresses(self, data, instance):
        ...
        for address in data.get("addresses"):
            ...
            customer = self.__search_customer(
                child=True, id=address.get("id"), customer_id=address.get("customer_id"), instance_id=instance.id
            )
            if not customer:
                # If customer not found in layer then we will create the customer in layer.
                layer_values = self._prepare_magento_customer_values(
                    instance=instance,
                    data=data,
    ...
    def _create_odoo_partner(self, data, instance):
        ...
        if len(partner) > 1:
            ...
            partner = partner.search([("email", "=", data.get("email")), ("parent_id", "=", False)], limit=1)
        if not partner:
    ...
    def _prepare_partner_values(self, data, instance, **kwargs):
        ...
        parent_customer = self.env["res.partner"].sudo().browse(kwargs.get("parent_id"))
        ...
        values = {
            ...
            else parent_customer.name,
            "email": data.get("email") if data.get("email", False) else parent_customer.email,
            "vat": (data.get("vat_id", "") or data.get("taxvat", ""))
            if (data.get("vat_id", False) or data.get("taxvat", False))
            else parent_customer.vat,
            "customer_rank": 1,
            ...
            "customer_class_id": customer_class_id or False,
            "company_type": company_type,
            "customer_code": website_id.short_code + str(data['id'])
        }
        if not street.get("street", False):
            values["street"] = parent_customer.street
            values["street2"] = parent_customer.street2
            values["city"] = parent_customer.city
            values["phone"] = parent_customer.phone
            values["zip"] = parent_customer.zip
    ...
    def __merge_street(streets):
        ...
        return {"street": street1, "street2": street2}
