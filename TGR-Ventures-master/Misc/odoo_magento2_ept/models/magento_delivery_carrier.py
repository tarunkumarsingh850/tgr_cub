...
class MagentoDeliveryCarrier(models.Model):
    ...
    delivery_carrier_ids = fields.One2many(
        comodel_name="delivery.carrier", inverse_name="magento_carrier", help="Delivery Methods for Magento"
    )
    ...
    _sql_constraints = [
        (
            "unique_magento_delivery_code",
            "unique(magento_instance_id,carrier_code)",
    ...
    def name_get(self):
        ...
        for delivery in self:
            ...
            name = (
                "[" + delivery.carrier_label + "]  " + delivery.carrier_code + instance_name
                if instance_name
                else delivery.carrier_code
            )
