...
class MagentoCustomerQueueEpt(models.Model):
    ...
    instance_id = fields.Many2one(
        comodel_name="magento.instance", string="Magento Instance", help="Customer imported from this Magento Instance."
    )
    state = fields.Selection(
        ...
        store=True,
        help="Status of Customer Data Queue",
    )
    ...
    log_lines_ids = fields.One2many(
        related="log_book_id.log_lines", help="Log lines of Common log book for particular customer queue"
    )

    line_ids = fields.One2many("magento.customer.data.queue.line.ept", "queue_id", help="Customer data queue line ids")
    ...
    def create(self, vals):
        ...
        sequences = self.env["ir.sequence"].search(
            [("company_id", "=", self.env.company.id), ("name", "=", "Customer Queue")]
        )
        if not sequences:
            sequence_obj = (
        ...
        name = f"Customer Queue/{end_code}"
    ...
    def _prepare_customer_filter(**kwargs):
        ...
        filters = {
            "updated_at": {"from": last_import, "to": to_date},
            "website_id": {"in": kwargs.get("website").magento_website_id},
        }
    ...
    def process_customer_queues(self, is_manual=False):
        for queue in self.filtered(lambda q: q.state not in ["completed", "failed"]):
            ...
            if not is_manual and queue.process_count >= 3:
                note = (
                    f"Attention {queue.name} Customer Queue are processed 3 times and it failed. \n"
                    f"You need to process it manually"
                )
