...
class MagentoPaymentMethod(models.Model):
    ...
    magento_workflow_process_id = fields.Many2one(
        comodel_name="sale.workflow.process.ept", string="Automatic Workflow", help="Workflow for Order"
    )
    ...
    import_rule = fields.Selection(
        [("always", "Always"), ("never", "Never"), ("paid", "Paid")],
        string="Import Rules",
        default="always",
        required=True,
    ...
    is_cancel_delivery_order = fields.Boolean(string="Cancel Delivery Order", default=False, copy=False)
    days_before_cancel_delivery_order = fields.Integer(
        string="Cancel Delivery Order X Days", help="Mention the how-many days before delivery should be canceled."
    )

    _sql_constraints = [
        (
            "unique_payment_method_code",
            "unique(magento_instance_id,payment_method_code)",
