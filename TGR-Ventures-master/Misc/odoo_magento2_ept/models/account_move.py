...
class AccountInvoice(models.Model):
    ...
    magento_payment_method_id = fields.Many2one(
        comodel_name="magento.payment.method", string="Magento Payment Method", help="Magento Payment Method"
    )
    is_magento_invoice = fields.Boolean(string="Magento Invoice?", help="If True, It is Magento Invoice")
    is_exported_to_magento = fields.Boolean(string="Exported to Magento", copy=False, help="Exported to Magento")
    ...
    def export_invoices_to_magento(self, instance):
        ...
        #     )
        #     .mapped("picking_ids")
        #     .filtered(lambda x: x.is_exported_to_magento)
        # )
        # if picking_orders:
        #     picking = self.env["sale.order"].search(
        #         [
        #             ("picking_ids", "in", picking_orders.ids),
        #         ]
        #     )
        #     invoices = picking.invoice_ids
    ...
    def export_invoice_magento(self, wizard=True):
        ...
        for invoice in self:
            ...
            if (create_invoice_on == "in_payment_paid" and invoice.payment_state in ["in_payment", "paid"]) or (
                create_invoice_on == "open" and invoice.payment_state not in ["in_payment", "paid"]
            ):
        ...
        #     message = _(
        #         f"""
        #         Delivery is not Updated to Magento against this Invoice
        #         for Sale Order : {sale_order.name} & Invoice : {self.name}
        #     """
        #     )
        #     if not log_book_id:
        #         log_book_id = self.create_common_logbook(instance)
        #     log_book_id.write({"log_lines": [(0, 0, {"message": message, "order_ref": sale_order.name})]})
        return True
    ...
    def call_export_invoice_api(self, instance, log_book_id):
        ...
        m_inv_id = 0
        try:
            api_url = f"{instance.magento_url}/rest/V1/order/{sale_order.magento_order_id}/invoice"
            ...
            if r.status_code in [200, 202]:
                self.is_exported_to_magento = True
            else:
                self.magento_message = r.text
            ...
            if self.max_no_of_attempts == 2:
                ...
                self.env["magento.instance"].create_activity(
                    model_name=self._name, res_id=self.id, message=note, summary=self.name, instance=instance
                )
            _logger.error(error)
            if not log_book_id:
                log_book_id = self.create_common_logbook(instance)
            self.write(
                {
                    ...
                    "magento_message": _(
                        "The request could not be satisfied while export this invoice."
    ...
    def _prepare_export_invoice_data(self):
        ...
        return {"items": order_item, "notify": self.magento_instance_id.invoice_done_notify_customer}
    ...
    def _prepare_line_values(line, item_id, items):
        ...
        for item in items:
            if item.get("order_item_id") == item_id:
                item.update({"qty": item.get("qty") + line.quantity})
                return dict()
        return {
    ...
    def _prepare_line_data(self):
        ...
        credit_lines = self.invoice_line_ids.filtered(lambda l: l.product_id.id and l.product_id.id not in product_ids)
    ...
    def _get_payload_values(self, refund_type, return_stock, order):
        ...
        if order.magento_order_id:
            ...
            values = self._prepare_order_payload(
                items=items, ship_charge=ship_charge, refund_type=refund_type, return_stock=return_stock
            )
        return values
    ...
    def _prepare_order_payload(**kwargs):
        ...
        return {
            ...
            "extension_attributes": {"return_to_stock_items": item_ids},
        }
    ...
    def _create_log_process(self, success):
        ...
        :return: True
        """
        log = self.env["magento.process.log"].create_process_log(self.magento_instance_id, "credit_memo")
    ...
    def action_post(self):
        for invoice in self.filtered(lambda inv: inv.move_type == "out_invoice"):
            if "is_auto_workflow" in self.env.context.keys() and not self.env.context.get("is_auto_workflow", False):
                if invoice.picking_id and invoice.picking_id.state != "done":
                    raise UserError(
                        _(
                            f"Delivery not done for {invoice.picking_id.name}.\n"
                            "Complete the delivery to post this invoice."
                        )
                    )
