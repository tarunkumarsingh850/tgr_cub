...
class MagentoAttributeSet(models.Model):
    ...
    instance_id = fields.Many2one(
        "magento.instance", string="Instance", ondelete="cascade", help="This field relocates magento instance record"
    )
    ...
    attribute_group_ids = fields.One2many(
        "magento.attribute.group", "attribute_set_id", string="Attribute group", help="Attribute group"
    )
    ...
    def _compute_get_display_name(self):
        for attr_set in self:
            attr_set.display_name = "{} - {}".format(attr_set.attribute_set_name, attr_set.instance_id.name)
    ...
    def create_attribute_set(self, instance, attr_sets):
        ...
        for attr_set in attr_sets.get("items", []):
            ...
            m_attr_set = self.search([("attribute_set_id", "=", set_id), ("instance_id", "=", instance.id)], limit=1)
            if not m_attr_set:
    ...
    def auto_import_product_attribute_cron(self):
        magento_instance = self.env["magento.instance"].search([])
        for instance in magento_instance:
                self.import_attribute_set(instance)
