...
class MagentoInventoryLocations(models.Model):
    ...
    name = fields.Char(
        string="Magento Location Name", required=True, readonly=True, help="Magento Inventory Location Name"
    )
    ...
    ship_from_location = fields.Many2one(
        comodel_name="stock.location",
        string="Export Shipment Location",
        help="Location for export shipment from Odoo to Magento",
    )
    domain_location_ids = fields.Many2many(
        "stock.location", store=True, string="Location", compute="_compute_domain_location_ids"
    )

    @api.depends("export_stock_warehouse_ids")
    def _compute_domain_location_ids(self):
        for each in self:
            if each.export_stock_warehouse_ids:
                inv_locs = self.env["stock.warehouse"].search(
                    [
                        ("id", "=", each.export_stock_warehouse_ids.ids),
                    ]
                )
                each.domain_location_ids = [(6, 0, inv_locs.mapped("lot_stock_id").ids)]
            else:
                each.domain_location_ids = False
        ...
        for inv_loc in inv_locs:
            if inv_locs and inv_loc.id != self.id:
                raise ValidationError(
                    _(
                        "It is not possible to set this location multiple times in the same "
