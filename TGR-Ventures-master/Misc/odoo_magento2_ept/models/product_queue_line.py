...
class MagentoProductQueueLine(models.Model):
    ...
    def auto_import_magento_product_queue_data(self):
        ...
        queues = self.instance_id.get_draft_queues(
            model="sync_import_magento_product_queue_line", name=self.queue_id._name
        )
    ...
    def process_queue_line(self):
        for line in self:
            ...
            if is_processed:
                line.sudo().write({"state": "done", "processed_at": datetime.now()})
            else:
                line.sudo().write({"state": "failed", "processed_at": datetime.now()})
        ...
        if item.get("type_id") == "simple":
            if "simple_parent_id" in list(attribute.keys()):
                ...
                if not m_product or "is_order" in list(self.env.context.keys()) or line.do_not_update_existing_product:
                    ...
                    for item in items:
                        return m_product.import_configurable_product(line, item, product_values_received)
            else:
                # This case identifies that we get only simple product which are not set as an
                # Child product of any configurable product.
                return m_product.with_context(is_order=self.env.context.get("is_order", False)).search_product_in_layer(
                    line, item
                )
    ...
    def __search_product(self, line, item):
        ...
        if not product:
            ...
            line.instance_id.create_log_line(
                message=message,
                model=line._name,
                res_id=line.id,
