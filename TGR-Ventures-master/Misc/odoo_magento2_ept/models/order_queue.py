...
class MagentoOrderDataQueueEpt(models.Model):
    ...
    log_book_id = fields.Many2one("common.log.book.ept", help="Related Log book which has all logs for current queue.")
    log_lines_ids = fields.One2many(
        related="log_book_id.log_lines", help="Log lines of Common log book for particular order queue"
    )
    line_ids = fields.One2many("magento.order.data.queue.line.ept", "queue_id", help="Order data queue line ids")
    total_count = fields.Integer(
        string="Total Records", compute="_compute_record", help="Returns total number of order data queue lines"
    )
    draft_record = fields.Integer(
        string="Draft Records", compute="_compute_record", help="Returns total number of draft order data queue lines"
    )
    failed_count = fields.Integer(
        string="Fail Records", compute="_compute_record", help="Returns total number of Failed order data queue lines"
    )
    done_count = fields.Integer(
        string="Done Records", compute="_compute_record", help="Returns total number of done order data queue lines"
    )
    cancel_count = fields.Integer(
        string="Cancel Records", compute="_compute_record", help="Returns total number of cancel order data queue lines"
    )
    ...
    process_count = fields.Integer(
        string="Queue Process Times", default=0, help="It is used know queue how many time processed"
    )
    company_id = fields.Many2one("res.company", string="Company")

    # def auto_delete_empty_queue_data(self):
    #     """
    #     This method used to delete empty queue.
    #     This method is called from cron job.
    #     """
    #     empty_queue = self.env["magento.order.data.queue.ept"].search([]).filtered(lambda x: not x.line_ids)
    #     empty_queue_line = self.env["magento.order.data.queue.line.ept"].search([]).filtered(lambda x: not x.queue_id)
    #     for empty in empty_queue:
    #         empty.sudo().unlink()
    #     for empty_line in empty_queue_line:
    #         empty_line.sudo().unlink()
        ...
        for record in self:
            if record.total_count == record.done_count + record.cancel_count:
                ...
                if record.state != "failed":
                    record.state = "failed"
                    note = "Attention {} Order Queue is failed. " "\nYou need to process it manually".format(
                        record.name
                    )
    ...
    def create(self, vals):
        ...
        sequences = self.env["ir.sequence"].search(
            [("company_id", "=", self.env.company.id), ("name", "=", "Order Queue")]
        )
        if not sequences:
            sequence_obj = (
            ...
            if orders.get("items"):
                ...
                for order in orders.get("items"):
                    if len(queue.line_ids) == 50:
                        ...
                        queue = self._create_order_queue(instance)
                        queue_ids.append(queue.id)
                    queue_line.sudo().create_order_queue_line(instance, order, queue)
    ...
    def _prepare_order_filter(**kwargs):
        ...
        filters = {"updated_at": {"from": last_import, "to": to_date}, "state": {"in": kwargs.get("status")}}
    ...
    def process_order_queues(self, is_manual=False):
        ...
        for queue in self.filtered(lambda q: q.state not in ["completed"]):
            ...
            #     note = (
            #         "Attention {} Order Queue is processed 3 times and it failed. "
            ...
            for line in lines:
                is_processed = line.sudo().process_order_queue_line(line, log)
                if is_processed:
