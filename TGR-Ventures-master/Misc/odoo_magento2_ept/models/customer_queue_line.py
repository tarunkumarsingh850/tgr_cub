...
class MagentoCustomerQueueLineEpt(models.Model):
    ...
    partner_id = fields.Many2one(comodel_name="res.partner", copy=False, help="Customer created in Odoo.")
    data = fields.Text(string="Data", copy=False, help="Data imported from Magento of current customer.")
    processed_at = fields.Datetime(
        string="Process Time", copy=False, help="Shows Date and Time, When the data is processed"
    )
    log_lines_ids = fields.One2many(
        "common.log.lines.ept", "magento_customer_data_queue_line_id", help="Log lines created against which line."
    )
    ...
    def auto_process_customer_queues(self):
        queues = self.instance_id.get_draft_queues(
            model="magento_customer_data_queue_line_ept", name=self.queue_id._name
        )
