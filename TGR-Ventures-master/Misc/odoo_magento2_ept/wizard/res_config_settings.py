...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_magento_default_financial_statuses(self):
        if self._context.get("default_magento_instance_id", False):
            financial_status_ids = (
                self.env[MAGENTO_FINANCIAL_STATUS_EPT]
                .search([("magento_instance_id", "=", self._context.get("default_magento_instance_id", False))])
                .ids
            )
    ...
    magento_website_warehouse_id = fields.Many2one(
        STOCK_WAREHOUSE, string="Warehouse", help="Warehouse to be used to deliver an order from this website."
    )
    warehouse_ids = fields.Many2many(
        STOCK_WAREHOUSE, string="Warehouses", help="Warehouses used to compute stock to update on Magento."
    )
    catalog_price_scope = fields.Selection(
        [("global", "Global"), ("website", "Website")],
        string="Magento Catalog Price Scope",
    ...
    is_import_product_stock = fields.Boolean(
        "Is Import Magento Product Stock?", default=False, help="Import Product Stock from Magento to Odoo"
    )
    import_stock_warehouse = fields.Many2one(
        STOCK_WAREHOUSE, string="Import Product Stock Warehouse", help="Warehouse for import stock from Magento to Odoo"
    )
    magento_stock_field = fields.Selection(
        [("onhand_qty", "On Hand Quantity"), ("free_qty", "Free Quantity"), ("virtual_available", "Forecast Quantity")],
    ...
    invoice_done_notify_customer = fields.Boolean(
        string="Invoices Done Notify customer", default=False, help="while export invoice send email to the customer"
    )
    ...
    magento_financial_status_ids = fields.Many2many(
        ...
        "workflow_id",
        string="Magento Financial Status",
        default=_get_magento_default_financial_statuses,
    )
    ...
    tax_calculation_method = fields.Selection(
        ...
        string="Tax Calculation Method into Magento Website",
    ...
    module_odoo_magento2_extended_bundle_products_order_ept = fields.Boolean(
        "Import bundle product orders from Magento?", help="If Yes, then mrp module will be install in the database."
    )
    ...
    magento_analytic_tag_ids = fields.Many2many(
        "account.analytic.tag", "magento_res_config_analytic_account_tag_rel", string="Magento Analytic Tag"
    )
    ...
    def onchange_magento_instance_id(self):
        ...
        if magento_instance_id:
            self.write(
                {
                    "warehouse_ids": [(6, 0, magento_instance_id.warehouse_ids.ids)]
                    if magento_instance_id.warehouse_ids
                    else False,
                    ...
                    "import_stock_warehouse": magento_instance_id.import_stock_warehouse.id
                    if magento_instance_id.import_stock_warehouse
                    else False,
                    ...
                    "import_product_category": magento_instance_id.import_product_category
                    if magento_instance_id.import_product_category
                    else False,
    ...
    def magento_res_config_view_action(self, view_id):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("odoo_magento2_ept.action_magento_config_settings")
        action_data = {
            "view_id": view_id.id,
    ...
    def magento_save_financial_status_configurations(self):
        ...
        if instance:
            ...
            financials_status = magento_financial_status_obj.search([("magento_instance_id", "=", instance.id)])
    ...
    def download_bundle_product_module(self):
        ...
        attachment = (
            ...
            .sudo()
            .search([("name", "=", "odoo_magento2_extended_bundle_products_order_ept.zip")], limit=1)
        )
        if attachment:
        ...
        return UserError(
            "Bundle product extended module is not found. " "Please upgrade Magento Connector then try to download."
        )
