...
class ResConfigMagentoInstance(models.TransientModel):
    ...
    access_token = fields.Char(string="Magento Access Token", help="Set Access token: Magento=>System=>Integrations")
    ...
    is_use_odoo_order_sequence = fields.Boolean(
        "Is Use Odoo Order Sequences?", default=False, help="If checked, Odoo Order Sequence is used"
    )
    ...
    def create_magento_instance(self):
        ...
        if magento_instance_exist:
            raise UserError(
                _(
                    "The instance already exists for the given Hostname. "
    ...
    def download_magento_api_module(self):
        ...
        """
        attachment = self.env["ir.attachment"].search([("name", "=", "emipro_magento_api_change.zip")])
        return {
    ...
    def action_open_magento_instance_wizard(self):
        ...
        magento_instance_obj = self.env["magento.instance"]
        action = ir_action_obj._for_xml_id("odoo_magento2_ept.magento_on_board_instance_configuration_action")
