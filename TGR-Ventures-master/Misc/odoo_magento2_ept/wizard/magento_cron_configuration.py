...
class MagentoCronConfiguration(models.TransientModel):
    ...
    magento_instance_id = fields.Many2one(
        "magento.instance", string="Magento Instance", default=_get_magento_instance, readonly=True
    )
    ...
    auto_import_product_interval_number = fields.Integer(
        string="Auto Import Product Interval Numbers", help="Import product every x interval.", default=1
    )
    auto_import_product_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        string="Auto Import Product Interval Unit",
        help="Auto Import Product Interval Unit",
    )
    auto_import_product_next_execution = fields.Datetime(
        string="Auto Next Product Execution", help="Next execution time for import product"
    )
    auto_import_product_user_id = fields.Many2one(
        RES_USERS, string="Auto Import Product User", help="Responsible User for import product"
    )
    ...
    auto_import_sale_orders = fields.Boolean(
        string="Auto import sale orders?", help="Automatic Import Shipped Sale Orders"
    )
    auto_import_sale_orders_interval_number = fields.Integer(
        string="Auto Import sale orders Interval Number", help="Import sale order every x interval.", default=1
    )
    auto_import_sale_orders_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        string="Auto Import Sale Order Interval Unit",
        help="Auto Import Sale Order Interval Unit",
    )
    auto_import_sale_orders_next_execution = fields.Datetime(
        string="Auto Next Sale Execution", help="Next execution time for import sale order"
    )
    auto_import_sale_order_user_id = fields.Many2one(
        RES_USERS, string="Auto Import Shipped Sale Order User", help="Responsible User for import sale order"
    )

    # Auto import sale orders
    auto_import_unshipped_sale_orders = fields.Boolean(
        string="Auto import Unshipped sale orders?", help="Automatic Import Sale Orders"
    )
    ...
    auto_import_unshipped_sale_orders_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        string="Auto Import Unshipped Sale Order Interval Unit",
        help="Auto Import Sale Order Interval Unit",
    )
    ...
    auto_import_unshipped_sale_order_user_id = fields.Many2one(
        RES_USERS, string="Auto Import Unshipped Sale Order User", help="Responsible User for import sale order"
    )

    # Auto import cancel sale orders
    auto_import_cancel_sale_orders = fields.Boolean(
        string="Auto cancel import sale orders?", help="Automatic Cancel Import Sale Orders"
    )
    ...
    auto_import_cancel_sale_orders_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        string="Auto Import Cancel Sale Order Interval Unit",
        help="Auto Import Cancel Sale Order Interval Unit",
    )
    auto_import_cancel_sale_orders_next_execution = fields.Datetime(
        string="Auto Next Cancel Sale Execution", help="Next execution time for import Cancel sale order"
    )
    auto_import_cancel_sale_order_user_id = fields.Many2one(
        RES_USERS, string="Auto Import Cancel Sale Order User", help="Responsible User for import Cancel sale order"
    )
    ...
    auto_export_product_stock_interval_number = fields.Integer(
        string="Auto Export Product Stock Interval Numbers", help="Export product stock every x interval.", default=1
    )
    ...
    auto_export_product_stock_next_execution = fields.Datetime(
        string="Auto Next Export Product Execution", help="Next execution time for export product stock"
    )
    auto_export_product_stock_user_id = fields.Many2one(
        RES_USERS, string="Auto Export Product User", help="Responsible User for export product stock"
    )
    ...
    auto_export_invoice_interval_number = fields.Integer(
        string="Auto Export Invoice Interval Numbers", help="Export Invoice every x interval.", default=1
    )
    auto_export_invoice_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        string="Auto Export Invoice  Interval Unit",
        help="Auto Export Invoice  Interval Unit",
    )
    auto_export_invoice_next_execution = fields.Datetime(
        string="Auto Next Export Invoice  Execution", help="Next execution time for export invoice"
    )
    auto_export_invoice_user_id = fields.Many2one(
        RES_USERS, string="Auto Export Invoice User", help="Responsible User for export invoice"
    )
    ...
    auto_export_shipment_order_status_interval_number = fields.Integer(
        string="Auto Update Order Status Interval Number", help="Export shipment every x interval.", default=1
    )
    auto_export_shipment_order_status_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        string="Auto Export Shipment Interval Unit",
        help="Auto Export Shipment Interval Unit",
    )
    auto_export_shipment_order_status_next_execution = fields.Datetime(
        string="Auto Next Order Status Execution", help="Next execution time for export shipment"
    )
    auto_export_shipment_order_status_user_id = fields.Many2one(
        RES_USERS, string="Auto Update Order User", help="Responsible User for export shipment"
    )
    ...
    def magento_import_sale_order_cron_field(self, instance):
        ...
        """
        try:
            magento_import_order_cron_exist = instance and self.env.ref(IMPORT_SALE_ORDER_CRON % instance.id)
        except Exception:
    ...
    def export_product_stock_cron_field(self, instance):
        ...
        """
        try:
            magento_export_product_stock_cron_exist = instance and self.env.ref(EXPORT_PRODUCT_STOCK_CRON % instance.id)
        except Exception:
    ...
    def import_magento_product_cron_field(self, instance):
        ...
        """
        try:
            import_product_cron_exist = instance and self.env.ref(IMPORT_MAGENTO_PRODUCT_CRON % instance.id)
        except Exception:
    ...
    def save_cron_configuration(self):
        ...
        if self._context.get("is_calling_from_magento_onboarding_panel", False):
            action = ir_action_obj._for_xml_id("odoo_magento2_ept.magento_onboarding_confirmation_wizard_action")
    ...
    def auto_import_sale_order(self, magento_instance):
        ...
        if self.auto_import_sale_orders:
            cron_exist = self.env.ref(IMPORT_SALE_ORDER_CRON % magento_instance.id, raise_if_not_found=False)
            ...
            vals = {
                ...
                "code": "model._scheduler_import_shipped_sale_orders({'magento_instance_id' : %d})"
                % magento_instance.id,
            ...
            if cron_exist:
                ...
                import_sale_orders_cron = self.env.ref(
                    "odoo_magento2_ept.ir_cron_import_shipped_sale_orders", raise_if_not_found=False
                )
    ...
    def auto_import_unshipped_sale_order(self, magento_instance):
        ...
        if self.auto_import_unshipped_sale_orders:
            cron_exist = self.env.ref(IMPORT_UNSHIPPED_SALE_ORDER_CRON % magento_instance.id, raise_if_not_found=False)
            ...
            vals = {
                ...
                "code": "model._scheduler_import_unshipped_sale_orders({'magento_instance_id' : %d})"
                % magento_instance.id,
            ...
            if cron_exist:
                ...
                import_unshipped_sale_orders_cron = self.env.ref(
                    "odoo_magento2_ept.ir_cron_import_unshipped_sale_orders", raise_if_not_found=False
                )
    ...
    def auto_import_cancel_sale_order(self, magento_instance):
        ...
        if self.auto_import_cancel_sale_orders:
            cron_exist = self.env.ref(IMPORT_CANCEL_SALE_ORDER_CRON % magento_instance.id, raise_if_not_found=False)
            ...
            vals = {
                ...
                "code": "model._scheduler_import_cancel_sale_orders({'magento_instance_id' : %d})"
                % magento_instance.id,
            ...
            if cron_exist:
                ...
                import_cancel_sale_orders_cron = self.env.ref(
                    "odoo_magento2_ept.ir_cron_import_cancel_sale_orders", raise_if_not_found=False
                )
        ...
        if self.auto_export_product_stock:
            ...
            if cron_exist:
                ...
                export_product_stock_cron = self.env.ref(
                    "odoo_magento2_ept.ir_cron_export_product_stock_qty", raise_if_not_found=False
                )
        ...
        if self.auto_export_shipment_order_status:
            ...
            if cron_exist:
                ...
                update_order_status_cron = self.env.ref(
                    "odoo_magento2_ept.ir_cron_export_shipment_order_status", raise_if_not_found=False
                )
    ...
    def auto_import_magento_product_cron(self, magento_instance):
        ...
        if self.auto_import_product:
            cron_exist = self.env.ref(IMPORT_MAGENTO_PRODUCT_CRON % magento_instance.id, raise_if_not_found=False)
            ...
            vals = {
                ...
                "code": "model._scheduler_import_product({'magento_instance_id' : %d, 'update_existing_product': %s})"
                % (magento_instance.id, self.do_not_update_existing_product),
            ...
            if cron_exist:
                ...
                export_inovice_cron = self.env.ref(
                    "odoo_magento2_ept.ir_cron_import_magento_product", raise_if_not_found=False
                )
                if not export_inovice_cron:
                    raise UserError(_(CRON_ERROR_MSG))
    ...
    def action_magento_open_cron_configuration_wizard(self):
        ...
        ir_action_obj = self.env["ir.actions.actions"]
        action = ir_action_obj._for_xml_id("odoo_magento2_ept.action_magento_wizard_cron_configuration_ept")
        instance = magento_instance_obj.search_magento_instance()
        action["context"] = {"is_calling_from_magento_onboarding_panel": True}
        if instance:
            action.get("context").update({"default_magento_instance_id": instance.id, "is_instance_exists": True})
        return action
