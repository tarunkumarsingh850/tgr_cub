...
class MagentoExportProductEpt(models.TransientModel):
    ...
    m_update_description = fields.Boolean(string="Update Product Description/Short Description ?", default=False)
    ...
    def default_get(self, field_list):
        ...
        global_product_description = (
            self.env["ir.config_parameter"].sudo().get_param("odoo_magento2_ept.set_magento_sales_description")
        )
        ...
        for instance in instances:
            ...
            for m_template in m_templates:
                ...
                if attribute_set_id:
                    ...
                    self.export_configurable_product(instance, m_template, attribute_set_id, log)
    ...
    def export_simple_product(self, instance, m_template, attribute_set_id, common_log_id):
        ...
        :return:
        """
        product_tmpl = m_template.filtered(lambda x: x.product_type == "simple" and not x.sync_product_with_magento)
        if product_tmpl:
    ...
    def export_configurable_product(self, instance, m_template, attribute_set_id, common_log_id):
        """
        ...
        if product_tmpl:
            for product in product_tmpl:
                ...
                self.create_product_in_magento(
                    instance, product, attribute_set_id, common_log_id, product_type="configurable"
                )
                if product.attribute_line_ids:
                    ...
                    if configurable_opt_vals:
                        continue
                    self.__find_attribute_for_product(product, instance, attribute_set_id, common_log_id)
                    magento_product_obj.bind_simple_with_configurable_product(
                        instance, product.magento_product_ids.mapped("magento_sku"), conf_product_sku, common_log_id
                    )
        return True
    ...
    def __find_attribute_for_product(self, product, instance, attribute_set_id, common_log_id):
        ...
        for product_product in product.magento_product_ids:
            if product_product.product_template_attribute_value_ids:
                self.fine_product_attributes(product_product, instance, attribute_set_id, common_log_id)
    ...
    def __do_verification(self):
        ...
        if not self.is_set_price and not self.magento_publish and not self.is_set_image and not self.attribute_set_id:
    ...
    def process_update_products_in_magento(self):
        ...
        for instance in instances:
            ...
            if m_templates:
                ...
                _logger.info("Updated total templates  %s  in %s seconds.", len(m_templates), str(end - start))
    ...
    def __update_configurable_product(self, instance, m_templates, log):
        ...
        if len(product_tmpl) > 0:
            for conf_product in product_tmpl:
                product_dict = self.__prepare_conf_product_dict(conf_product)
                api_url = "/all/V1/products/%s" % Php.quote_sku(conf_product.magento_sku)
                self.env["magento.product.template"].update_product_request(instance, product_dict, api_url)
                for m_variant in conf_product.magento_product_ids:
                    self.__update_product(instance, m_variant, log, True)
        return True

    def __prepare_conf_product_dict(self, m_template):
        product_dict = {}
        if m_template.magento_product_ids:
            product_dict = self.__prepare_update_product_dict(m_template, is_child=False)
    ...
    def __update_product(self, instance, m_template, log, is_child=False):
        ...
        if website_ids:
            product_dict = self.__prepare_update_product_dict(m_template, is_child)
            if self.m_update_basic_details:
                website_ids_dict = [
                    int(magento_website_id) for magento_website_id in website_ids.mapped("magento_website_id")
                ]
                product_dict.get("product").get("extension_attributes").update({"website_ids": website_ids_dict})
            product = m_product if is_child else m_template
            price_dict = self.__update_product_website_vise(instance, website_ids, product, product_dict)
    ...
    def __find_store_view(self, m_website):
        magento_storeview = self.env["magento.storeview"].search([("magento_website_id", "in", m_website.ids)])
    ...
    def __find_m_categories(self, product, is_child=False):
        ...
        categories = []
        if category_ids and self.m_update_basic_details or self.magento_publish in ["publish", "unpublish"]:
    ...
    def __find_tax_class(self, product, is_child=False):
        ...
        if (
            not is_child
            and product.magento_tax_class
            and self.m_update_basic_details
            or self.magento_publish in ["unpublish", "publish"]
        ):
            tax_dict = {"attribute_code": "tax_class_id", "value": product.magento_tax_class.magento_tax_class_id}
        return tax_dict

    def __find_descriptions(self, product):
        custom_attributes = []
        description = product.description
        sale_description_export = (
            self.env["ir.config_parameter"].sudo().get_param("odoo_magento2_ept.set_magento_sales_description")
        )
        if sale_description_export and self.m_update_description:
            if description:
                values = {"attribute_code": "description", "value": description}
                custom_attributes.append(values)

            if "short_description" in product:
                description_sale = product.short_description
                short_description = {"attribute_code": "short_description", "value": description_sale}
                custom_attributes.append(short_description)
        return custom_attributes

    def __prepare_update_product_dict(self, m_template, is_child):
        categories = self.__find_m_categories(m_template, is_child)
        tax_class = self.__find_tax_class(m_template, is_child)
        update_product_dict = {
            "product": {"name": m_template.magento_product_name, "extension_attributes": {}, "custom_attributes": []}
        }
        if categories:
            update_product_dict.get("product").get("extension_attributes").update({"category_links": categories})
        if tax_class:
    ...
    def __update_product_website_vise(self, instance, website_ids, product, product_dict):
        ...
        for website in website_ids:
            ...
            price_dict = self.__prepare_update_website_price_dict(instance, website, m_store_view, product, price_dict)
            self.__update_product_in_magento(instance, product, m_store_view, descriptions, product_dict)
        return price_dict

    def __update_product_in_magento(self, instance, product, m_store_view, custom_attrs, product_dict):
        if m_store_view:
            for store_view in m_store_view:
                ...
                api_url = "/{}/V1/products/{}".format(store_view.magento_storeview_code, Php.quote_sku(product.magento_sku))
    ...
    def __prepare_product_price_dict(self, price_list, m_template, store_view_id):
        is_it_child = False
        m_product = self.env["magento.product.template"].get_simple_product(is_it_child, m_template)
        product_price = price_list.get_product_price(m_product.odoo_product_id, price_list.id, False)
        _logger.info("{} : in product.product price is : {}".format(m_product.odoo_product_id.name, product_price))
        price_vals = {"sku": m_template.magento_sku, "price": product_price, "store_id": store_view_id}
        return price_vals

    def __prepare_update_website_price_dict(self, instance, website, m_store_view, m_template, price_dict):
        ...
        if self.update_price:
            if instance.catalog_price_scope == "global":
                ...
                if instance.pricelist_id:
                    price_vals = self.__prepare_product_price_dict(instance.pricelist_id, m_template, 0)
                    price_dict.append(price_vals)
                else:
                    log.write(
                        {
            ...
            if price_dict:
                ...
                self.env["magento.product.template"].update_product_base_price(instance, price_url, log, price_data)
    ...
    def export_stock_in_magento_ept(self):
        ...
        logs = list()
        m_templates = m_template.search([("id", "in", m_product_ids), ("sync_product_with_magento", "=", True)])
        for instance in instances:
    ...
    def magento_create_log_book(self, instance):
        ...
        log_book_id = self.env["common.log.book.ept"].create(
            {"type": "export", "module": "magento_ept", "model_id": model_id, "magento_instance_id": instance.id}
        )
        return log_book_id
    ...
    def export_product_stock_operation(self, instances):
        ...
        for instance in instances:
            ...
            log = log.create_common_log_book(
                process_type="export",
    ...
    def export_product_stock_magento(self, instance, product_ids, log):
        ...
        if not instance.is_multi_warehouse_in_magento:
    ...
    def _filter_export_stock_products(self, instance, product_ids):
        ...
        m_products = m_product.search_read(
            [("odoo_product_id", "in", product_ids), ("magento_instance_id", "=", instance.id)],
        ...
        for product in m_products:
            ...
            if product.get("magento_sku"):
                ...
                datas.append(
                    {"id": product.get("odoo_product_id")[0], "magento_sku": product.get("magento_sku"), "type": p_type}
                )
        return datas
    ...
    def export_stock_non_msi(self, instance, product_ids, log, layer_ids):
        ...
        if not instance.warehouse_ids:
            raise UserError(_("Please select Export Stock Warehouse for {} instance.").format(instance.name))
        for each in instance.warehouse_ids:
            product_stock = m_product.get_magento_product_stock_ept(instance, product_ids, each)
            if product_stock:
                stock_data = m_template.prepare_export_stock_data(
                    product_stock=product_stock, instance=instance, log=log, layer_ids=layer_ids
                )
                if stock_data:
                    api_url = "/V1/product/updatestock"
                    m_product.exp_prd_stock_in_batches(stock_data, instance, api_url, "skuData", "PUT", log)
        return True
    ...
    def export_stock_with_msi(self, instance, product_ids, log, layer_ids):
        ...
        for location in locations:
            ...
            if not stock_locations:
                raise UserError(_("Please select Export Stock Warehouse " "for {} location.").format(location.name))
            product_stock = m_product.get_magento_product_stock_ept(instance, product_ids, stock_locations)
            if product_stock:
                stock_data += m_template.prepare_export_stock_data(
        ...
        if stock_data:
            count = 10
            subsets = (stock_data[x : x + count] for x in range(0, len(stock_data), count))
            for subset in subsets:
                api_url = "/V1/inventory/source-items"
                m_product.exp_prd_stock_in_batches(subset, instance, api_url, "sourceItems", "POST", log)
        return True

    def create_product_in_magento(
        self, instance, product, attribute_set_id, log, product_type="simple", is_it_child=False, custom_attributes=[]
    ):
        """
        ...
        custom_attributes = m_template.prepare_main_product_description_array(custom_attributes, product)
        ...
        attribute_set = m_template.get_magento_attribute_set(product, product_type, is_it_child, attribute_set_id, log)
        data = self.prepare_product_data(product, is_it_child, product_type, attribute_set, website_ids)
        if conf_custom_attrs:
            for configurable_opt in conf_custom_attrs:
                if (
                    configurable_opt.get("attribute_code") != "description"
                    and configurable_opt.get("attribute_code") != "short_description"
                ):
                    data.get("product", dict()).get("custom_attributes").append(configurable_opt)
        if website_ids:
            response = self.__create_product_website_vise(
                instance, conf_simple_product, website_ids, product, data, log, is_it_child
            )
            website_vals = self.__prepare_website_vise_dict(
                product, website_ids, custom_attributes, product_type, is_it_child
            )
            self.__set_websites_images_in_magento(instance, product, website_vals, log)
        else:
            log.write(
                {
                    "log_lines": [
        ...
        if not log.log_lines:
            magento_product_images = m_template.get_magento_product_images(product, product_type, is_it_child)
            if magento_product_images:
                self.set_magento_product_image(magento_product_images)
        return True

    def __create_product_website_vise(
        self, instance, conf_simple_product, website_ids, product, data, log, is_it_child
    ):
        m_template = self.env["magento.product.template"]
        descriptions = self.__find_descriptions(product)
        magento_website_id = []
        ...
        for website in website_ids:
            ...
            for store in m_store_view:
                ...
                if descriptions:
                    data = m_template.translate_description(data, store, descriptions, product)
    ...
    def __prepare_website_vise_dict(self, product, website_id, custom_attributes, product_type, is_child):
        ...
        data = {
            "product": {
                ...
                "extension_attributes": {"website_ids": website_id.mapped("magento_website_id")},
        ...
        if self.is_set_price:
            if instance.catalog_price_scope == "global":
                ...
                if price_list:
                    ...
                    product_price = m_template_obj.get_scope_wise_product_price(instance, log, m_template, price_list)
    ...
    def fine_product_attributes(self, o_product, instance, attribute_set_id, common_log_id):
        """
        ...
        for value_id in o_product.product_template_attribute_value_ids:
            ...
            magento_attribute = magento_attribute_obj.browse(magento_attribute_option.magento_attribute_id.id)
            if not magento_attribute_option or not magento_attribute:
                _logger.info(
                    "%s attribute option or its value still not sync with odoo.",
            ...
            o_product.magento_product_name = "{} - {}".format(
                o_product.magento_product_name, magento_attribute_option.magento_attribute_option_name
            )
