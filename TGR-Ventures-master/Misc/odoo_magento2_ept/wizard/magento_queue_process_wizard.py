...
class MagentoQueueProcessEpt(models.TransientModel):
    ...
    def process_customer_queue_manually(self):
        ...
        customer_queue_ids = (
            self.env["magento.customer.data.queue.ept"].browse(customer_queue_ids).filtered(lambda x: x.state != "done")
        )
        for customer_queue_id in customer_queue_ids:
            queue_lines = customer_queue_id.line_ids.filtered(lambda line: line.state in ["draft", "failed"])
    ...
    def set_to_completed_order_queue_manually(self):
        ...
        order_queue_ids = self._context.get("active_ids")
        order_queue_ids = (
            self.env["magento.order.data.queue.ept"].browse(order_queue_ids).filtered(lambda x: x.state != "done")
        )
        self.env.cr.execute(
        ...
        self._cr.commit()
        for order_queue_id in order_queue_ids:
            queue_lines = order_queue_id.line_ids.filtered(lambda line: line.state in ["draft", "failed"])
    ...
    def set_to_completed_product_queue_manually(self):
        ...
        product_queue_ids = self._context.get("active_ids")
        product_queue_ids = (
            self.env["sync.import.magento.product.queue"]
            .browse(product_queue_ids)
        ...
        self._cr.commit()
        for product_queue_id in product_queue_ids:
            queue_lines = product_queue_id.line_ids.filtered(lambda line: line.state in ["draft", "failed"])
            queue_lines.write({"state": "cancel"})
