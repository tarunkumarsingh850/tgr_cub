...
class Binary(http.Controller):
    ...
    def download_document(self, model, id, filename=None):
        ...
        if model == 'stock.valuation.success.box':
    ...
    def place_order(self, **kwargs):
        ...
        magento_instance = (
            request.env["magento.instance"].sudo().search([("magento_url", "=", magento_url.rstrip("/"))])
        )
    ...
    def cancel_order(self, **kwargs):
        ...
        sale_order = (
            ...
            .sudo()
            .search(
                [("magento_instance_id", "=", magento_instance.id), ("magento_order_id", "=", int(order_id))], limit=1
            )
        )
        if sale_order:
