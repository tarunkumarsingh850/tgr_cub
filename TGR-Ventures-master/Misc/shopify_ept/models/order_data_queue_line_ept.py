...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    shopify_order_common_log_lines_ids = fields.One2many(
        "common.log.lines.ept", "shopify_order_data_queue_line_id", help="Log lines created against which line."
    )
    ...
    def create_order_queue_line(self, order_dict, instance, order_data, customer_name, customer_email, order_queue_id):
        ...
        order_queue_line_vals = {
            "shopify_order_id": order_dict.get("id", False),
    ...
    def create_order_data_queue_line(self, orders_data, instance, queue_type, created_by="import"):
        ...
        for order in orders_data:
            if created_by == "webhook" and not is_new_order:
                order_queue, need_to_create_queue = self.search_webhook_order_queue(
                    created_by, instance, order, queue_type, need_to_create_queue
                )
    ...
    def search_webhook_order_queue(self, created_by, instance, order, queue_type, need_to_create_queue):
        ...
        if order_queue:
            message = "Order {} added into Order Queue {}.".format(order.get("name"), order_queue.name)
            need_to_create_queue = False
            _logger.info(message)
    ...
    def generate_simple_notification(self, message):
        ...
        bus_bus_obj._sendone(
            ...
            "simple_notification",
            {"title": "Shopify Connector", "message": message, "sticky": False, "warning": True},
        )
    ...
    def get_customer_name_and_email(self, order):
        ...
        try:
            ...
            customer_name = "{} {}".format(customer_data.get("first_name"), customer_data.get("last_name"))
    ...
    def filter_order_queue_lines_and_post_message(self, queues):
        ...
        for queue in queues:
            ...
            if queue.queue_process_count > 3:
                ...
                note = (
                    "<p>Need to process this order queue manually.There are 3 attempts been made by "
