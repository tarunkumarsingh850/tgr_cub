...
class SaleOrder(models.Model):
    ...
    shopify_payment_gateway_id = fields.Many2one("shopify.payment.gateway.ept", string="Payment Gateway", copy=False)
    ...
    updated_in_shopify = fields.Boolean(
        "Updated In Shopify ?", compute=_get_shopify_order_status, search="_search_shopify_order_ids"
    )
    ...
    shopify_payment_ids = fields.One2many("shopify.order.payment.ept", "order_id", string="Payment Lines")
    ...
    def create_shopify_log_line(self, message, queue_line, log_book, order_name):
        ...
        common_log_line_obj.shopify_create_order_log_line(
            message, log_book.model_id.id, queue_line, log_book, order_name
        )
        if queue_line:
    ...
    def prepare_shopify_customer_and_addresses(self, order_response, pos_order, instance, order_data_line, log_book):
        ...
        if pos_order:
            ...
            if not partner:
                message = (
                    "Default POS Customer is not set.\nPlease set Default POS Customer in " "Shopify Configuration."
                )
        else:
            if not any(
                [
                    order_response.get("customer", {}),
                    order_response.get("billing_address", {}),
    ...
    def set_shopify_location_and_warehouse(self, order_response, instance, pos_order):
        ...
        if shopify_location_id:
            shopify_location = shopify_location_obj.search(
                [("shopify_location_id", "=", shopify_location_id), ("instance_id", "=", instance.id)], limit=1
            )
    ...
    def create_shopify_order_lines(self, lines, order_response, instance):
        ...
        for line in lines:
            ...
            order_line = self.shopify_create_sale_order_line(
                line, product, line.get("quantity"), product.name, line.get("price"), order_response
            )
            ...
            if float(total_discount) > 0.0:
                ...
                if discount_amount > 0.0:
                    _logger.info(
                        "Creating discount line for Odoo order(%s) and Shopify order is (%s)", self.name, order_number
                    )
                    ...
                    _logger.info(
                        "Created discount line for Odoo order(%s) and Shopify order is (%s)", self.name, order_number
                    )
    ...
    def import_shopify_orders(self, order_data_lines, log_book):
        ...
        for order_data_line in order_data_lines:
            ...
            _logger.info(
                "Started processing Shopify order(%s) and order id is(%s)", order_number, order_response.get("id")
            )

            date_order = self.convert_order_date(order_response)
            if str(instance.import_order_after_date) > date_order:
                message = (
                    "Order %s is not imported in Odoo due to configuration mismatch.\n Received order date is "
                    "%s. \n Please check the order after date in shopify configuration." % (order_number, date_order)
                )
                _logger.info(message)
                self.create_shopify_log_line(message, order_data_line, log_book, order_response.get("name"))
                continue
            ...
            if sale_order:
                order_data_line.write({"state": "done", "processed_at": datetime.now(), "sale_order_id": sale_order.id})
                _logger.info(
                    "Done the Process of order Because Shopify Order(%s) is exist in Odoo and Odoo order is(" "%s)",
                    order_number,
                    sale_order.name,
                )
                continue
            ...
            if self.check_mismatch_details(lines, instance, order_number, order_data_line, log_book):
                _logger.info(
                    "Mismatch details found in this Shopify Order(%s) and id (%s)",
                    order_number,
            ...
            sale_order = self.shopify_create_order(
                instance,
                partner,
                delivery_address,
                invoice_address,
            ...
            if not sale_order:
                message = "Configuration missing in Odoo while importing Shopify Order({}) and id ({})".format(
                    order_number,
                    order_response.get("id"),
                )
                _logger.info(message)
            ...
            _logger.info(
                "Starting auto workflow process for Odoo order(%s) and Shopify order is (%s)",
                sale_order.name,
                order_number,
            )
            message = ""
            try:
                if sale_order.shopify_order_status == "fulfilled":
                    ...
                    ).shipped_order_workflow_ept(sale_order)
                ...
                if order_data_line:
                    order_data_line.write(
                        {"state": "failed", "processed_at": datetime.now(), "sale_order_id": sale_order.id}
                    )
                ...
                continue
            _logger.info(
                "Done auto workflow process for Odoo order(%s) and Shopify order is (%s)", sale_order.name, order_number
            )

            if message:
                model_id = common_log_line_obj.get_model_id(self._name)
                common_log_line_obj.shopify_create_order_log_line(message, model_id, order_data_line, log_book)
                order_data_line.write({"state": "failed", "processed_at": datetime.now()})
            else:
                order_data_line.write({"state": "done", "processed_at": datetime.now(), "sale_order_id": sale_order.id})
    ...
    def create_shipped_order_refund(self, shopify_financial_status, order_response, sale_order):
        ...
        if shopify_financial_status == "refunded":
            ...
            refunded = sale_order.create_shopify_refund(order_response.get("refunds"), total_refund)
            if refunded[0] == 0:
                message = (
                ...
                message = (
                    ...
                    "related invoice but it is not in 'Post' status." % (order_response.get("name"))
                )
            elif refunded[0] == 3:
                message = (
    ...
    def search_existing_shopify_order(self, order_response, instance, order_number):
        ...
        if not sale_order:
            sale_order = self.search(
                [("shopify_instance_id", "=", instance.id), ("client_order_ref", "=", order_response.get("name"))]
            )
        ...
        for line in lines:
            ...
            if line.get("gift_card", False):
                ...
                if product:
                    continue
                message = (
                    "Please upgrade the module and then try to import order(%s).\n Maybe the Gift Card "
                    "product "
            ...
            if not shopify_variant:
                ...
                if line_product_id and line_variant_id:
                    shopify_product_template_obj.shopify_sync_products(
                        False, line_product_id, instance, log_book_id, order_data_queue_line
                    )
                    shopify_variant = self.search_shopify_variant(line, instance)
                    if not shopify_variant:
                        message = "Product [{}][{}] not found for Order {}".format(
                            line.get("sku"),
                            line.get("name"),
                            order_number,
                        )
    ...
    def search_shopify_variant(self, line, instance):
        ...
        if line.get("variant_id", None):
            shopify_variant = shopify_product_obj.search(
                [("variant_id", "=", line.get("variant_id")), ("shopify_instance_id", "=", instance.id)]
            )
    ...
    def shopify_create_order(
        ...
        payment_gateway, workflow = payment_gateway_obj.shopify_search_create_gateway_workflow(
        ...
        if len(order_response.get("payment_gateway_names")) > 1:
            payment_vals = self.prepare_vals_shopify_multi_payment(
                instance, order_data_queue_line, order_response, log_book_id, payment_gateway, workflow
            )
            if not payment_vals:
                return False
    ...
    def prepare_shopify_order_vals(
        ...
        if not instance.is_use_default_sequence:
            if instance.shopify_order_prefix:
                name = "{}_{}".format(instance.shopify_order_prefix, order_response.get("name"))
            else:
    ...
    def shopify_create_sale_order_line(
        ...
        order_line_vals = self.shopify_set_tax_in_sale_order_line(
            instance, line, order_response, is_shipping, is_discount, previous_line, order_line_vals
        )
        if is_discount:
    ...
    def shopify_set_tax_in_sale_order_line(
        self, instance, line, order_response, is_shipping, is_discount, previous_line, order_line_vals
    ):
        ...
        if instance.apply_tax_in_order == "create_shopify_tax":
            ...
            if line and line.get("tax_lines"):
                if line.get("taxable"):
                    ...
                    # taxable
                    tax_ids = self.shopify_get_tax_id_ept(instance, line.get("tax_lines"), taxes_included)
                if is_shipping:
    ...
    def shopify_get_tax_id_ept(self, instance, tax_lines, tax_included):
        ...
        for tax in tax_lines:
            ...
            if rate != 0.0 and price != 0.0:
                if tax_included:
                    name = "{}_({} {} included)_{}".format(title, str(rate), "%", company.name)
                else:
                    name = "{}_({} {} excluded)_{}".format(title, str(rate), "%", company.name)
    ...
    def prepare_vals_shopify_multi_payment(
        self, instance, order_data_queue_line, order_response, log_book_id, payment_gateway, workflow
    ):
        ...
        for transaction in transactions:
            ...
            if result.get("kind") == "sale" and result.get("status"):
                ...
                if order_response.get("gateway") == gateway:
                    ...
                    continue
                new_payment_gateway, new_workflow = payment_gateway_obj.shopify_search_create_gateway_workflow(
                    instance, order_data_queue_line, order_response, log_book_id, gateway
                )
    ...
    def prepare_tracking_numbers_and_lines_for_fulfilment(self, picking):
        ...
        """
        shopify_line_ids = (
            not self.is_service_tracking_updated
            and self.order_line.filtered(
                lambda l: l.shopify_line_id
                and l.product_id.type == "service"
                and not l.is_delivery
                and not l.is_gift_card_line
        ...
        if picking.shopify_instance_id and not picking.shopify_instance_id.auto_fulfill_gift_card_order:
            shopify_line_ids = (
                not self.is_service_tracking_updated
                and self.order_line.filtered(
                    lambda l: l.shopify_line_id and l.product_id.type == "service" and not l.is_delivery
                ).mapped("shopify_line_id")
                or []
            )
        ...
        for line in shopify_line_ids:
            quantity = sum(self.order_line.filtered(lambda l: l.shopify_line_id == line).mapped("product_uom_qty"))
    ...
    def prepare_tracking_numbers_and_lines_for_multi_tracking_order(self, moves, product_moves):
        ...
        kit_move_lines = moves.filtered(lambda x: x.sale_line_id.product_id.id != x.product_id.id and x.state == "done")
    ...
    def update_order_status_in_shopify(self, instance, picking_ids=[]):
        ...
        log_book = common_log_book_obj.create_common_log_book(
            "export", "shopify_instance_id", instance, model_id, "shopify_ept"
        )
        ...
        for picking in picking_ids:
            ...
            if not line_items:
                message = "No order lines found for the update order shipping status for order [%s]" % sale_order.name
                _logger.info(message)
                self.create_shopify_log_line(message, False, log_book, sale_order.client_order_ref)
                continue
            ...
            fulfillment_vals = self.prepare_vals_for_fulfillment(
                sale_order, shopify_location_id, tracking_numbers, picking, carrier_name, line_items, notify_customer
            )

            is_create_mismatch, fulfillment_result, new_fulfillment = self.post_fulfilment_in_shopify(
                fulfillment_vals, sale_order, log_book
            )
            if is_create_mismatch:
                continue

            self.process_shopify_fulfilment_result(
                fulfillment_result, order_response, picking, sale_order, log_book, new_fulfillment
            )
    ...
    def search_shopify_location_for_update_order_status(self, sale_order, instance, log_book):
        ...
        if not shopify_location_id:
            ...
            if not shopify_location_id:
                message = "Primary Location not found for instance %s while update order " "shipping status." % (
                    instance.name
                )
                _logger.info(message)
                self.create_shopify_log_line(message, False, log_book, sale_order.client_order_ref)
    ...
    def prepare_vals_for_fulfillment(
        self, sale_order, shopify_location_id, tracking_numbers, picking, carrier_name, line_items, notify_customer
    ):
    ...
    def process_shopify_fulfilment_result(
        self, fulfillment_result, order_response, picking, sale_order, log_book, new_fulfillment
    ):
    ...
    def process_shopify_order_via_webhook(self, order_data, instance, update_order=False):
        ...
        if order_data.get("fulfillment_status") == "fulfilled":
            queue_type = "shipped"
        queue = order_queue_line_obj.create_order_data_queue_line(
            [order_data], instance, queue_type, created_by="webhook"
        )
        if not update_order:
    ...
    def update_shopify_order(self, queue_lines, log_book):
        ...
        for queue_line in queue_lines:
            ...
            if order_data.get("cancel_reason"):
                ...
                if not cancelled:
                    message = (
                        "System can not cancel the order {} as one of the Delivery Order "
                ...
                if not message:
                    ...
                    if refunded[0] == 0:
                        ...
                        message = (
                            "- Refund can only be generated if it's related order "
                            "invoice is in 'Post' status.\n- For order [%s], system found "
                            "related invoice but it is not in 'Post' status." % (order_data.get("name"))
                        )
                    elif refunded[0] == 3:
                        message = (
                            "- Partial refund is received from Shopify for order [%s].\n "
            ...
            if message:
                model_id = common_log_line_obj.get_model_id(self._name)
                common_log_line_obj.shopify_create_order_log_line(message, model_id, queue_line, log_book)
                queue_line.write({"state": "failed", "processed_at": datetime.now()})
            else:
    ...
    def create_shopify_refund(self, refunds_data, total_refund):
        ...
        if self.amount_total == total_refund:
            ...
            move_reversal.new_move_ids.message_post(
                body=_("Credit note generated by Webhook as Order refunded in " "Shopify.")
            )
            return [True]
        return [3]
    ...
    def create_move_and_delete_not_necessary_line(self, refunds_data, invoices):
        ...
        for new_move_line in new_move.invoice_line_ids:
            ...
            if (
                ...
                or new_move_line.product_id == self.shopify_instance_id.discount_product_id
            ):
    ...
    def process_order_fullfield_qty(self, order_response):
        ...
        for line in lines:
            ...
            sale_order_line = self.order_line.filtered(
                lambda order_line: int(order_line.shopify_line_id) == shopify_line_id
                and order_line.product_id.detailed_type != "service"
            )
    ...
    def create_stock_move_of_fullfield_qty(self, order_line, fulfilled_qty, bom_line=False):
        ...
        if product and product_qty and product_uom:
            move_vals = self.prepare_val_for_stock_move(
                product, product_qty, product_uom, customer_location, order_line
            )
            if bom_line:
        ...
        for invoice in invoices:
            if invoice.amount_residual:
                if self.is_shopify_multi_payment:
                    ...
                    if self.auto_workflow_process_id.post_payment:
                        payment_id.action_post()
                        # self.reconcile_payment_ept(payment_id, invoice)
            ...
            for user_id in log_book_id.shopify_instance_id.shopify_user_ids:
                ...
                if not mail_activity or mail_activity.note != note_2:
                    vals = {
                        "activity_type_id": activity_type_id,
                        "note": note,
                        "summary": log_book_id.name,
                        "res_id": log_book_id.id,
                    ...
                    try:
                        ...
                        _logger.info(
                            "Unable to create schedule activity, Please give proper " "access right of this user :%s  ",
                            user_id.name,
                        )
                        _logger.info(error)
        return True
