...
class ShopifyPaymentGateway(models.Model):
    ...
    def import_payment_gateway(self, instance):
        ...
        try:
            results = shopify.Order().find(
                status="any", updated_at_min=from_date, updated_at_max=to_date, fields=["gateway"], limit=250
            )
    ...
    def search_or_create_payment_gateway(self, instance, gateway_name):
        ...
        shopify_payment_gateway = self.search(
            [("code", "=", gateway_name), ("shopify_instance_id", "=", instance.id)], limit=1
        )
        if not shopify_payment_gateway:
            shopify_payment_gateway = self.create(
                {"name": gateway_name, "code": gateway_name, "shopify_instance_id": instance.id}
            )
    ...
    def shopify_search_create_gateway_workflow(
        ...
        if not workflow_config:
            ...
            message = (
                "- Automatic order process workflow configuration not found for this order "
                ...
                "under the menu Shopify > Configuration > Financial Status."
                % (order_response.get("name"), gateway, order_response.get("financial_status"))
            )
            common_log_line_obj.shopify_create_order_log_line(
                message, model_id, order_data_queue_line, log_book_id, order_response.get("name")
            )
        ...
        if auto_workflow_id and not auto_workflow_id.picking_policy:
            message = (
                "- Picking policy decides how the products will be delivered, "
