...
class ShopifyInstanceEpt(models.Model):
    ...
    def _get_default_warehouse(self):
        ...
        warehouse = self.env["stock.warehouse"].search(
            [("company_id", "=", self.shopify_company_id.id)], limit=1, order="id"
        )
    ...
    shopify_warehouse_id = fields.Many2one(
        ...
        help="Selected Warehouse will be set in your Shopify " "orders.",
        required=True,
    )
    ...
    shopify_last_date_customer_import = fields.Datetime(
        string="Last Customer Import", help="it is used to store last import customer date"
    )
    shopify_last_date_update_stock = fields.Datetime(
        string="Last Stock Update", help="it is used to store last update inventory stock date"
    )
    shopify_last_date_product_import = fields.Datetime(
        string="Last Product Import", help="it is used to store last import product date"
    )
    ...
    update_category_in_odoo_product = fields.Boolean(string="Update Category In Odoo Product ?", default=False)
    shopify_stock_field = fields.Many2one("ir.model.fields", string="Stock Field")
    last_date_order_import = fields.Datetime(
        string="Last Date Of Unshipped Order Import", help="Last date of sync orders from Shopify to Odoo"
    )
    shopify_section_id = fields.Many2one("crm.team", "Sales Team")
    is_use_default_sequence = fields.Boolean(
        "Use Odoo Default Sequence?", help="If checked,Then use default sequence of odoo while create sale " "order."
    )
    # Account field
    shopify_store_time_zone = fields.Char("Store Time Zone", help="This field used to import order process")
    ...
    shopify_date_deadline = fields.Integer(
        "Deadline lead days", help="its add number of  days in schedule activity deadline date "
    )
    is_shopify_create_schedule = fields.Boolean(
        "Create Schedule Activity ? ",
        default=False,
        help="If checked, Then Schedule Activity create on order data queues" " will any queue line failed.",
    )
    active = fields.Boolean(default=True)
    sync_product_with_images = fields.Boolean(
        "Sync Images?", help="Check if you want to import images along with " "products", default=True
    )
    ...
    last_shipped_order_import_date = fields.Datetime(
        string="Last Date Of Shipped Order Import", help="Last date of sync orders from Shopify to Odoo"
    )
    ...
    import_order_after_date = fields.Datetime(
        ...
        default=_default_shopify_import_after_date,
    )
        ...
        def get_compared_week_data(record):
            ...
            self._cr.execute(
                """select sum(amount_untaxed) as current_week from sale_order
                                ...
                                shopify_instance_id=%s and state in ('sale','done')"""
                % record.id
            )
            ...
            if previous_week_data:
                previous_total = (
                    previous_week_data.get("previous_week") if previous_week_data.get("previous_week") else 0
                )
    ...
    def get_total_orders(self):
        ...
        """
        order_query = (
            """select id from sale_order where shopify_instance_id= %s and state in ('sale','done')""" % self.id
        )

        def orders_of_current_week(order_query):
            qry = (
                order_query
                + """ and date(date_order) >= (select date_trunc('week', date(current_date))) order by
            date(date_order)"""
            )
        ...
        def orders_of_current_month(order_query):
            qry = (
                order_query
                + """ and date(date_order) >=(select date_trunc('month', date(current_date))) order by
            date(date_order)"""
            )
            self._cr.execute(qry)
            return self._cr.dictfetchall()
        ...
        def orders_of_current_year(order_query):
            qry = (
                order_query
                + """ and date(date_order) >= (select date_trunc('year', date(current_date))) order by
            date(date_order)"""
            )
            self._cr.execute(qry)
            return self._cr.dictfetchall()
    ...
    def get_total_products(self):
        ...
        product_data = {}
        self._cr.execute(
            """select count(id) as total_count from shopify_product_template_ept where
                        exported_in_shopify = True and shopify_instance_id = %s"""
            % self.id
        )
    ...
    def get_customers(self):
        ...
        """
        customer_data = {}
        self._cr.execute("""select partner_id from shopify_res_partner_ept where shopify_instance_id = %s""" % self.id)
        result = self._cr.dictfetchall()
    ...
    def get_refund(self):
        ...
        """
        refund_query = (
            """select id from account_move where shopify_instance_id=%s and
    ...
    def open_report(self, record_id):
        ...
        action.update(
            {
                "context": {
                    "search_default_shopify_instances": record_id,
    ...
    def create_sales_channel(self, name):
        ...
        vals = {"name": name, "use_quotations": True}
    ...
    def shopify_test_connection(self, vals=False):
        ...
        try:
            ...
            if error.response.body:
                ...
                raise UserError(_("{}\n{}\n{}".format(error.response.code, error.response.msg, errors.get("errors"))))
    ...
    def search_shopify_instance(self):
        ...
        if not instance:
            instance = self.search(
                [("shopify_company_id", "=", company.id), ("is_onboarding_configurations_done", "=", False)],
                limit=1,
                order="id desc",
            )
