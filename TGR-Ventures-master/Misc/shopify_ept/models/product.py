    ...
    def write(self, vals):
        ...
        if "active" in vals.keys():
            ...
            for template in self:
                shopify_templates = shopify_product_template_obj.search([("product_tmpl_id", "=", template.id)])
                if vals.get("active"):
...
class ProductProduct(models.Model):
    ...
    def write(self, vals):
        ...
        if "active" in vals.keys():
            ...
            for product in self:
                shopify_product = shopify_product_product_obj.search([("product_id", "=", product.id)])
                if vals.get("active"):
