...
class ShopifyProductDataQueueLineEpt(models.Model):
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("failed", "Failed"), ("done", "Done"), ("cancel", "Cancelled")], default="draft"
    )
    product_data_queue_id = fields.Many2one(
        "shopify.product.data.queue.ept", required=True, ondelete="cascade", copy=False
    )
    common_log_lines_ids = fields.One2many(
        "common.log.lines.ept", "shopify_product_data_queue_line_id", help="Log lines created against which line."
    )
    ...
    def process_product_queue_and_post_message(self, queues):
        ...
        for queue in queues:
            ...
            if queue.queue_process_count > 3:
                ...
                note = (
                    "<p>Need to process this product queue manually.There are 3 attempts been made by "
        ...
        if queue_id:
            ...
            if shopify_instance.active:
                ...
                if queue_id.common_log_book_id:
                    ...
                    log_book_id = common_log_book_obj.shopify_create_common_log_book(
                        "import", shopify_instance, model_id
                    )
                ...
                for product_queue_line in self:
                    ...
                    shopify_product_template_obj.shopify_sync_products(
                        product_queue_line, False, shopify_instance, log_book_id
                    )
    ...
    def shopify_image_import(self):
        ...
        for queue in product_queue_lines:
            ...
            shopify_template = shopify_template_obj.search(
                [
                    ("shopify_tmpl_id", "=", product_queue.product_data_id),
                    ("shopify_instance_id", "=", product_queue.shopify_instance_id.id),
                ],
                limit=1,
            )
