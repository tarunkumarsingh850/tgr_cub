...
class ShopifyProductDataQueue(models.Model):
    ...
    product_data_queue_lines = fields.One2many(
        "shopify.product.data.queue.line.ept", "product_data_queue_id", string="Product Queue Lines"
    )
    ...
    queue_line_total_records = fields.Integer(string="Total Records", compute="_compute_queue_line_record")
    queue_line_draft_records = fields.Integer(string="Draft Records", compute="_compute_queue_line_record")
    queue_line_fail_records = fields.Integer(string="Fail Records", compute="_compute_queue_line_record")
    queue_line_done_records = fields.Integer(string="Done Records", compute="_compute_queue_line_record")
    queue_line_cancel_records = fields.Integer(string="Cancelled Records", compute="_compute_queue_line_record")
    ...
    def shopify_create_product_data_queue(
        self, instance, import_based_on="", from_date=False, to_date=False, skip_existing_product=False, template_ids=""
    ):
        """
        ...
        if template_ids:
            ...
            if import_based_on == "create_date":
                results = shopify.Product().find(
                    status="active", created_at_min=from_date, created_at_max=to_date, limit=250
                )
            else:
                results = shopify.Product().find(
                    status="active", updated_at_min=from_date, updated_at_max=to_date, limit=250
                )
    ...
    def import_products_by_remote_ids(self, template_ids, instance):
        ...
        if len(template_ids.split(",")) <= 100:
            ...
            if results:
                _logger.info("Length of Shopify Products %s import from instance : %s", len(results), instance.name)
        ...
        if queue_id:
            ...
            for user_id in queue_id.shopify_instance_id.shopify_user_ids:
                ...
                if not mail_activity or not duplicate_note:
                    vals = common_log_book_obj.prepare_vals_for_schedule_activity(
                        activity_type_id, note, queue_id, user_id, model_id.id, date_deadline
                    )
                    try:
    ...
    def assign_queue_model_date_ref_note(self, from_sale, queue_line):
        ...
        if from_sale:
            ...
            note = (
                _(
                    "Your order has not been imported because of the product of order Has a new attribute Shopify "
                    "Order Reference : %s"
                )
                % data_ref
            )
            ...
            note = (
                _("Your product was not synced because you tried to add new attribute | Product Data Reference " ": %s")
                % data_ref
            )
    ...
    def create_shopify_product_queue_from_webhook(self, product_data, instance):
        ...
        product_data_queue = self.search(
            [("created_by", "=", "webhook"), ("state", "=", "draft"), ("shopify_instance_id", "=", instance.id)]
        )
        if product_data_queue:
            message = "Product {} added into Queue {}.".format(product_data.get("id"), product_data_queue.name)
        else:
    ...
    def retrieve_dashboard(self, *args, **kwargs):
        ...
        return dashboard.get_data(
            table="shopify.product.data.queue.line.ept",
        )
