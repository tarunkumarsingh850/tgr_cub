...
class ShopifyPaymentReportEpt(models.Model):
    ...
    payout_reference_id = fields.Char(string="Payout Reference ID", help="The unique identifier of the payout")
    ...
    def get_payout_report(self, start_date, end_date, instance):
        ...
        try:
            ...
            message = "Something is wrong while import the payout records : {}".format(error)
            ...
            log_line_obj.create({"log_book_id": log_book_id.id, "message": message, "model_id": model_id or False})
    ...
    def create_payout_reports(self, payout_reports, instance):
        ...
        for payout_report in payout_reports:
            ...
            payout = self.search([("instance_id", "=", instance.id), ("payout_reference_id", "=", payout_id)])
            if payout:
    ...
    def create_payout_transaction_lines(self, payout_data):
        ...
        # Create fees line
        fees_amount = (
            float(payout_data.get("summary").get("charges_fee_amount", 0.0))
            + float(payout_data.get("summary").get("refunds_fee_amount", 0.0))
            + float(payout_data.get("summary").get("adjustments_fee_amount", 0.0))
        )
    ...
    def prepare_transaction_vals(self, data, instance):
        ...
        if source_order_id:
            order_id = sale_order_obj.search(
                [("shopify_order_id", "=", source_order_id), ("shopify_instance_id", "=", instance.id)], limit=1
            )
    ...
    def generate_bank_statement(self):
        ...
        name = "{}_{}".format(self.instance_id.name, payout_reference_id)
        vals = {
    ...
    def create_bank_statement_lines_for_payout_report(self, bank_statement_id, regenerate=False):
        ...
        for transaction in transaction_ids:
            ...
            if transaction.transaction_type in ["charge", "refund", "payment_refund"] and not order_id:
                ...
                order_id = sale_order_obj.search(
                    [("shopify_order_id", "=", source_order_id), ("shopify_instance_id", "=", self.instance_id.id)],
                    limit=1,
                )
                if order_id:
                    ...
                    message = (
                        ...
                        "order {} is not found in odoo.".format(transaction.transaction_id, transaction.source_order_id)
                    )
                    log_lines += common_log_line_obj.create(
                        {"message": message, "shopify_payout_report_line_id": transaction.id}
                    )
    ...
    def check_for_invoice_refund(self, transaction):
        ...
        if transaction.transaction_type == "charge":
            invoice_ids = order_id.invoice_ids.filtered(
                lambda x: x.state == "posted" and x.move_type == "out_invoice" and x.amount_total == transaction.amount
            )
            if not invoice_ids:
            ...
            invoice_ids = order_id.invoice_ids.filtered(
                lambda x: x.state == "posted" and x.move_type == "out_refund" and x.amount_total == -transaction.amount
            )
            if not invoice_ids:
                message = "In Shopify Payout, there is a Refund, but Refund is not created for order %s in" "odoo" % (
    ...
    def check_journal_and_currency(self):
        ...
        if currency_id != self.currency_id.id:
            message_body = (
                "The Report currency and currency in Journal/Instance are different."
    ...
    def check_reconciled_transactions(self, transaction, aml_rec=False):
        ...
        if aml_rec and aml_rec.statement_id:
            ...
            log_line = common_log_line_obj.create({"message": message, "shopify_payout_report_line_id": transaction.id})
            reconciled = True
    ...
    def convert_move_amount_currency(self, bank_statement, moveline, amount, date):
        ...
        if moveline.company_id.currency_id.id != bank_statement.currency_id.id:
            amount_currency = moveline.currency_id._convert(
                moveline.amount_currency, bank_statement.currency_id, bank_statement.company_id, date
            )
        elif moveline.move_id and moveline.move_id.currency_id.id != bank_statement.currency_id.id:
            amount_currency = moveline.move_id.currency_id._convert(
                amount, bank_statement.currency_id, bank_statement.company_id, date
            )
        ...
        for moveline in paid_move_lines:
            ...
            if moveline.amount_currency:
                currency, amount_currency = self.convert_move_amount_currency(
                    self.statement_id, moveline, amount, statement_line.date
                )
                if currency:
    ...
    def reconcile_invoice_refund(
        self, statement_line, move_line_total_amount, currency_ids, move_line_data, paid_move_lines
    ):
        """"""
        log_line = common_log_line_obj = self.env["common.log.lines.ept"]

        if round(statement_line.amount, 10) == round(move_line_total_amount, 10) and (
            not statement_line.currency_id or statement_line.currency_id.id == self.statement_id.currency_id.id
        ):
            if currency_ids:
                currency_ids = list(set(currency_ids))
                if len(currency_ids) == 1:
                    statement_currency = (
                        statement_line.journal_id.currency_id
                        if statement_line.journal_id.currency_id
                        else statement_line.company_id.currency_id
                    )
            ...
            try:
                ...
                message = (
                    "Error occurred while reconciling statement line : "
                    + statement_line.payment_ref
                    + ".\n"
                    + str(error)
                )
                transaction_line = self.payout_transaction_ids.filtered(
                    lambda x: x.transaction_type == statement_line.shopify_transaction_type
                    and x.transaction_id == statement_line.shopify_transaction_id
                    and x.amount == statement_line.amount
                )
                log_line = common_log_line_obj.create(
                    {"message": message, "shopify_payout_report_line_id": transaction_line.id}
                )
    ...
    def process_bank_statement(self):
        ...
        for statement_line in bank_statement.line_ids.filtered(lambda x: not x.is_reconciled):
            ...
            if statement_line.shopify_transaction_type in ["charge", "refund"]:
                ...
                log_line = self.reconcile_invoice_refund(
                    statement_line, move_line_total_amount, currency_ids, move_line_data, paid_move_lines
                )
            else:
