...
class ShopifyResPartnerEpt(models.Model):
    ...
    def search_shopify_partner(self, shopify_customer_id, shopify_instance_id):
        ...
        shopify_partner = self.search(
            [("shopify_customer_id", "=", shopify_customer_id), ("shopify_instance_id", "=", shopify_instance_id)],
            limit=1,
        )
        if shopify_partner:
    ...
    def shopify_create_or_update_address(self, shopify_customer_data, parent_partner, partner_type="contact"):
        ...
        partner = partner_obj._find_partner_ept(
            partner_vals, address_key_list, [("parent_id", "=", parent_partner.id), ("type", "=", partner_type)]
        )

        if not partner:
            partner = partner_obj._find_partner_ept(
                partner_vals, address_key_list, [("parent_id", "=", parent_partner.id)]
            )
        if not partner:
