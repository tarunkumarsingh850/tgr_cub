...
class ShopifyLocationEpt(models.Model):
    ...
    name = fields.Char(
        help="Give this location a short name to make it easy to identify. You’ll see this name in areas"
        "like orders and products.",
        readonly="1",
    )
    export_stock_warehouse_ids = fields.Many2many(
        "stock.warehouse", string="Warehouses", help="Selected warehouse used while Export the stock."
    )
    import_stock_warehouse_id = fields.Many2one(
        "stock.warehouse", string="Warehouse", help="Selected warehouse used while Import the stock."
    )
    ...
    legacy = fields.Boolean(
        "Is Legacy Location",
        help="Whether this is a fulfillment service location. If true, then"
    ...
    warehouse_for_order = fields.Many2one(
        "stock.warehouse",
        "Warehouse in Order",
        help="The warehouse to set while importing order, if this" " Shopify location is found.",
    )
    ...
    def set_primary_location(self, shop, instance):
        ...
        shopify_primary_location = self.search(
            [("is_primary_location", "=", True), ("instance_id", "=", instance.id)], limit=1
        )
        ...
        primary_location = (
            self.search([("shopify_location_id", "=", primary_location_id), ("instance_id", "=", instance.id)])
