...
class ShopifyProductProductEpt(models.Model):
    ...
    shopify_template_id = fields.Many2one("shopify.product.template.ept", required=1, ondelete="cascade")
    ...
    inventory_management = fields.Selection(
        [("shopify", "Shopify tracks this product Inventory"), ("Dont track Inventory", "Don't track Inventory")],
        default="shopify",
    ...
    def shopify_set_variant_sku(self, result, product_template, instance):
        ...
        for variation in result.get("variants"):
            ...
            template_attribute_value_ids = self.prepare_template_attribute_values_ids(
                variation_attributes, product_template
            )

            odoo_product = self.search_odoo_product_and_set_sku_barcode(
                template_attribute_value_ids, variation, product_template
            )
        ...
        for variation_attribute in variation_attributes:
            ...
            if product_attribute:
                product_attribute_value = product_attribute_value_obj.get_attribute_values(
                    attribute_val, product_attribute.id
                )
    ...
    def search_odoo_product_and_set_sku_barcode(self, template_attribute_value_ids, variation, product_template):
        ...
        if odoo_product and sku.isnumeric():
            odoo_product.write({"detailed_type": 'service'})

        return odoo_product

    def prepare_shopify_product_for_update_export(
        self, new_product, template, instance, is_set_basic_detail, is_publish, is_set_price
    ):
        """
        ...
        if is_set_basic_detail or is_set_price:
            ...
            for variant in template.shopify_product_ids:
                variant_vals = self.shopify_prepare_variant_vals(instance, variant, is_set_price, is_set_basic_detail)
    ...
    def shopify_export_products(
        self, instance, is_set_basic_detail, is_set_price, is_set_images, is_publish, templates
    ):
        """
        ...
        for template in templates:
            ...
            self.prepare_shopify_product_for_update_export(
                new_product, template, instance, is_set_basic_detail, is_publish, is_set_price
            )
    ...
    def shopify_export_product_log_line(self, message, model_id, log_book_id):
        ...
        common_log_line_obj = self.env["common.log.lines.ept"]
        vals = {"message": message, "model_id": model_id, "log_book_id": log_book_id.id if log_book_id else False}
    ...
    def prepare_export_update_product_attribute_vals(self, template, new_product):
        ...
        if len(template.shopify_product_ids) > 1:
            ...
            for attribute_line in product_attribute_lines.filtered(lambda x: x.attribute_id.create_variant == "always"):
                ...
                info.update({"name": attribute.name, "values": value_names, "position": attribute_position})
    ...
    def update_products_in_shopify(
        self, instance, templates, is_set_price, is_set_images, is_publish, is_set_basic_detail
    ):
        """
    ...
    def request_for_shopify_template(self, template, model_id, log_book_id):
        ...
        try:
            ...
            message = "Template {} not found in shopify while updating Product.\nError: {}".format(
    ...
    def prepare_vals_for_product_basic_details(self, variant_vals, variant):
        ...
        att_values = attribute_value_obj.search(
            [("id", "in", variant.product_id.product_template_attribute_value_ids.ids)], order="attribute_id"
        )
    ...
    def write_variant_response_in_shopify_variant(self, result_dict, template):
        ...
        for variant_dict in result_dict.get("variants"):
            ...
            shopify_variant = template.shopify_search_odoo_product_variant(
                template.shopify_instance_id, variant_id, variant_dict.get("sku"), variant_dict.get("barcode")
            )[0]
    ...
    def export_stock_in_shopify(self, instance, product_ids):
        ...
        for location_id in location_ids:
            ...
            product_stock = self.check_stock(
                instance, odoo_product_ids, product_obj, location_id.export_stock_warehouse_ids
            )
            commit_count = 0
            for shopify_product in shopify_products:
                ...
                if odoo_product.detailed_type == "product":
                    if not shopify_product.inventory_item_id:
                        message = (
                            "Inventory Item Id did not found for Shopify Product Variant ID "
                    ...
                    try:
                        response = shopify.InventoryLevel.set(
                            location_id.shopify_location_id, shopify_product.inventory_item_id, int(quantity)
                        )
                        log_line_array = self.shopify_create_log(
                            f"Product stock updated to {quantity} for {odoo_product.name}. \n{response}",
                            model_id,
                            odoo_product,
                            log_line_array,
                        )
        ...
        if templates:
            ...
            for location_id in location_ids:
                ...
                if len(stock_inventory_array) > 0:
                    inventory_name = 'Inventory For Instance "{}" And Shopify Location "{}"'.format(
                        instance.name,
                        location_id.name,
                    )
    ...
    def request_for_the_inventory_level(self, location_id, instance, model_id, log_line_array):
        ...
        """
        try:
            inventory_levels = shopify.InventoryLevel.find(location_ids=location_id.shopify_location_id, limit=250)
            if len(inventory_levels) == 250:
                inventory_levels = self.shopify_list_all_inventory_level(inventory_levels)
        except Exception as error:
            message = "Error while import stock for instance {}\nError: {}".format(
                instance.name,
                str(error.response.code) + " " + error.response.msg,
            )
            log_line_array = self.shopify_create_log(message, model_id, False, log_line_array)
