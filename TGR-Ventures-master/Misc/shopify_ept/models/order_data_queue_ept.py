...
class ShopifyOrderDataQueueEpt(models.Model):
    ...
    shopify_instance_id = fields.Many2one(
        "shopify.instance.ept", string="Instance", help="Order imported from this Shopify Instance."
    )
    ...
    order_data_queue_line_ids = fields.One2many("shopify.order.data.queue.line.ept", "shopify_order_data_queue_id")
    order_queue_line_total_record = fields.Integer(string="Total Records", compute="_compute_order_queue_line_record")
    order_queue_line_draft_record = fields.Integer(string="Draft Records", compute="_compute_order_queue_line_record")
    order_queue_line_fail_record = fields.Integer(string="Fail Records", compute="_compute_order_queue_line_record")
    order_queue_line_done_record = fields.Integer(string="Done Records", compute="_compute_order_queue_line_record")

    order_queue_line_cancel_record = fields.Integer(string="Cancel Records", compute="_compute_order_queue_line_record")
    ...
    queue_type = fields.Selection(
        [("shipped", "Shipped Order Queue"), ("unshipped", "Unshipped Order Queue")],
    ...
    def _compute_queue_state(self):
        ...
        for record in self:
            if (
                ...
                == record.order_queue_line_done_record + record.order_queue_line_cancel_record
            ):
    ...
    def import_shipped_order_cron_action(self, ctx=False):
        ...
        self.shopify_create_order_data_queues(
            instance, from_date, to_date, created_by="scheduled_action", order_type="shipped"
        )
        return True
    ...
    def convert_dates_by_timezone(self, instance, from_date, to_date):
        ...
        from_date = (
            pytz.utc.localize(from_date)
            .astimezone(pytz.timezone(instance.shopify_store_time_zone or "UTC"))
            .strftime("%Y-%m-%dT%H:%M:%S%z")
        )
        to_date = (
            pytz.utc.localize(to_date)
            .astimezone(pytz.timezone(instance.shopify_store_time_zone or "UTC"))
            .strftime("%Y-%m-%dT%H:%M:%S%z")
        )
    ...
    def shopify_create_order_data_queues(
        self, instance, from_date, to_date, created_by="import", order_type="unshipped"
    ):
        """
        ...
        if order_type != "shipped":
            ...
            for order_status_id in instance.shopify_order_status_ids:
                ...
                if order_ids:
                    order_queues = order_data_queue_line_obj.create_order_data_queue_line(
                        order_ids, instance, queue_type, created_by
                    )
            ...
            order_queues = self.shopify_shipped_order_request(
                instance, from_date, to_date, created_by="import", order_type="shipped"
            )
    ...
    def shopify_order_request(self, instance, from_date, to_date, order_type):
        ...
        try:
            order_ids = shopify.Order().find(
                status="any", fulfillment_status=order_type, updated_at_min=from_date, updated_at_max=to_date, limit=250
            )
        ...
        while result:
            ...
            for page_link in link.split(","):
                if page_link.find("next") > 0:
                    ...
                    if result:
                        order_queues = order_data_queue_line_obj.create_order_data_queue_line(
                            result, instance, queue_type, created_by
                        )
    ...
    def import_order_process_by_remote_ids(self, instance, order_ids):
        ...
        if order_ids:
            ...
            if results:
                ...
                order_queues = order_data_queue_line_obj.create_order_data_queue_line(
                    results, instance, queue_type, created_by="import"
                )
