...
class CommonLogLineEpt(models.Model):
    ...
    shopify_product_data_queue_line_id = fields.Many2one(
        "shopify.product.data.queue.line.ept", "Shopify Product Queue Line"
    )
    shopify_order_data_queue_line_id = fields.Many2one("shopify.order.data.queue.line.ept", "Shopify Order Queue Line")
    shopify_customer_data_queue_line_id = fields.Many2one(
        "shopify.customer.data.queue.line.ept", "Shopify Customer Queue Line"
    )
    ...
    def shopify_create_product_log_line(self, message, model_id, queue_line_id, log_book_id, sku=""):
        ...
        vals.update(
            {"shopify_product_data_queue_line_id": queue_line_id.id if queue_line_id else False, "default_code": sku}
        )
    ...
    def shopify_create_order_log_line(self, message, model_id, queue_line_id, log_book_id, order_ref=""):
        ...
        vals.update(
            {"shopify_order_data_queue_line_id": queue_line_id and queue_line_id.id or False, "order_ref": order_ref}
        )
        log_line = self.create(vals)
        return log_line
