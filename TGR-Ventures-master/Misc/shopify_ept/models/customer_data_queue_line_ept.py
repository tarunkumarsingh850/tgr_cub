...
class ShopifyCustomerDataQueueLineEpt(models.Model):
    ...
    common_log_lines_ids = fields.One2many(
        "common.log.lines.ept", "shopify_customer_data_queue_line_id", help="Log lines created against which line."
    )
    ...
    def shopify_customer_data_queue_line_create(self, result, customer_queue_id):
        ...
        name = "{} {}".format(result.get("first_name") or "", result.get("last_name") or "")
    ...
    def filter_customer_queue_lines_and_post_message(self, queues):
        ...
        for queue in queues:
            ...
            if queue.queue_process_count > 3:
                ...
                note = _(
                    "<p>Need to process this customer queue manually.There are 3 attempts been made by "
        ...
        for queue in queues:
            ...
            if instance.active:
                if queue.common_log_book_id:
                    ...
                    self.env.cr.execute(
                        """update shopify_product_data_queue_ept set is_process_queue = False where
    ...
    def customer_queue_commit_and_process(self, queue, instance, log_book_id):
        ...
        for line in self:
            ...
            main_partner = shopify_partner_obj.shopify_create_contact_partner(
