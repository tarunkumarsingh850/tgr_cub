...
class ShopifyProductTemplateEpt(models.Model):
    ...
    website_published = fields.Selection(
        [
            ...
            ("published_web", "Published in Web Only"),
    ...
    total_sync_variants = fields.Integer("Total Synced Variants", compute="_compute_total_sync_variants", store=True)
                ...
                if vals.get("active"):
                    shopify_variants = shopify_product_product_obj.search(
                        [
                            ...
                            ("shopify_instance_id", "=", shopify_template.shopify_instance_id.id),
                            ("active", "=", False),
                        ]
                    )
    ...
    def find_template_attribute_values(self, template_options, product_template_id, variant):
        ...
        template_attribute_value_domain = self.prepare_template_attribute_value_domain(
            product_attribute_list, variant, product_template_id
        )
    ...
    def prepare_template_attribute_value_domain(self, product_attribute_list, variant, product_template_id):
        ...
        for product_attribute in product_attribute_list:
            ...
            product_attribute_value = product_attribute_value_obj.get_attribute_values(
    ...
    def shopify_create_simple_product(self, product_name, variant_data, description, attribute_line_data=[]):
        ...
        if sku or barcode:
            vals = {"name": product_name, "detailed_type": "product", "default_code": sku, "invoice_policy": "order"}
        ...
        try:
            ...
            if hasattr(error, "response"):
                if error.response.code == 429 and error.response.msg == "Too Many Requests":
                    ...
                    result = [shopify.Product().find(template_id)]
                    return result
                message = "Error while importing product for order. Product ID: {}.\nError: {}\n{}".format(
                    template_id,
            ...
            if order_queue_line:
                message = "Shopify product did not exist in Shopify store with product id: {} \nError : {}".format(
                    template_id,
                    str(error),
                )
                self.create_log_line_for_queue_line(message, model_id, log_book_id, False, order_queue_line, "")
    ...
    def create_log_line_for_queue_line(
        self,
        message,
        model_id,
        log_book_id,
        ...
        if product_data_line_id:
            common_log_line_obj.shopify_create_product_log_line(
                message, model_id, product_data_line_id, log_book_id, product_sku
            )
            product_data_line_id.write({"state": "failed", "last_process_date": datetime.now()})
        elif order_data_line_id:
            common_log_line_obj.shopify_create_order_log_line(message, model_id, order_data_line_id, log_book_id)
    ...
    def get_product_category(self, product_type):
        ...
        product_category = product_category_obj.search(
            [("name", "=", product_type), ("is_shopify_product_cat", "=", True)], limit=1
        )
    ...
    def shopify_sync_products(
        self, product_data_line_id, shopify_tmpl_id, instance, log_book_id, order_data_line_id=False
    ):
        """
        ...
        template_data, skip_existing_product = self.convert_shopify_template_response(
            shopify_tmpl_id, product_data_line_id, model_id, log_book_id, order_data_line_id
        )
        ...
        shopify_template = self.search(
            [("shopify_tmpl_id", "=", template_data.get("id")), ("shopify_instance_id", "=", instance.id)]
        )

        if shopify_template:
            shopify_template = self.sync_product_with_existing_template(
                shopify_template,
            ...
            if not skip_existing_product and instance.sync_product_with_images and shopify_tmpl_id and shopify_template:
                shopify_template.shopify_sync_product_images(template_data)
        else:
            shopify_template = self.sync_new_product(
                template_data,
                instance,
                product_category,
                model_id,
    ...
    def convert_shopify_template_response(
        self, shopify_tmpl_id, product_data_line_id, model_id, log_book_id, order_data_line_id
    ):
        ...
        if shopify_tmpl_id and not product_data_line_id:
            result = self.import_product_for_order(shopify_tmpl_id, order_data_line_id, model_id, log_book_id)
            if not result:
                return False
    ...
    def sync_product_with_existing_template(
        self,
        shopify_template,
        skip_existing_product,
        template_data,
        ...
        variant_ids, need_to_archive = self.sync_variant_data_with_existing_template(
            instance,
            variant_data,
            template_data,
            shopify_template,
        ...
        return shopify_template if variant_ids else False

    def sync_variant_data_with_existing_template(
        self,
        instance,
        variant_data,
        template_data,
        ...
        for variant in variant_data:
            ...
            if message:
                self.create_log_line_for_queue_line(
                    message, model_id, log_book_id, product_data_line_id, order_data_line_id, sku
                )
                continue
            ...
            if not shopify_product:
                ...
                if odoo_product:
                    shopify_product = self.create_or_update_shopify_variant(
                        variant_vals, shopify_product, shopify_template, odoo_product
                    )

                elif instance.auto_import_product:
                    if odoo_template.attribute_line_ids:
                        shopify_product = self.check_for_new_variant(
                            odoo_template, shopify_attributes, variant, shopify_template, variant_vals
                        )
                    else:
                        attribute_line_data = self.prepare_attribute_line_data_for_variant(shopify_attributes, variant)
                        odoo_product = self.shopify_create_simple_product(
                            name, variant, template_vals.get("body_html"), attribute_line_data
                        )
                    ...
                    if instance.shopify_sync_product_with == "sku":
                        message = "Product {} not found for SKU {} in Odoo.".format(name, sku)
                    elif instance.shopify_sync_product_with == "barcode":
                        message = "Product {} not found for Barcode {} in Odoo.".format(name, barcode)
                    else:
                        message = "Product {} not found for SKU {} and Barcode {} in Odoo.".format(name, sku, barcode)
    ...
    def sync_new_product(
        self, template_data, instance, product_category, model_id, log_book_id, product_data_line_id, order_data_line_id
    ):
        """
        ...
        for variant in variant_data:
            ...
            if message:
                self.create_log_line_for_queue_line(
                    message, model_id, log_book_id, product_data_line_id, order_data_line_id, sku, create_activity=True
                )
                break
            ...
            if shopify_product:
                ...
                if need_to_update_template:
                    shopify_template = self.create_or_update_shopify_template(
                        template_vals, len(variant_data), shopify_product.shopify_template_id, odoo_product, False
                    )
            elif odoo_product:
                shopify_template, shopify_product = self.create_or_update_shopify_template_and_variant(
                    template_vals,
                    ...
                    shopify_product,
                    odoo_product,
                    update_template=need_to_update_template,
                    update_variant=True,
                )
                ...
                if odoo_template and odoo_template.attribute_line_ids:
                    if not shopify_template:
                        shopify_template = self.create_or_update_shopify_template(
                            template_vals, len(variant_data), False, False, odoo_template
                        )
                    shopify_product = self.check_for_new_variant(
                        odoo_template, shopify_attributes, variant, shopify_template, variant_vals
                    )
                    ...
                    if (
                        shopify_attributes[0].get("name") == "Title"
                        and shopify_attributes[0].get("values") == ["Default Title"]
                        and len(variant_data) == 1
                    ):
                        odoo_product = self.shopify_create_simple_product(name, variant, template_vals.get("body_html"))
                    else:
                        odoo_template = shopify_product_obj.shopify_create_variant_product(
                            template_data, instance, variant.get("price")
                        )
                        attribute_value_domain = self.find_template_attribute_values(
                            shopify_attributes, odoo_template.id, variant
                        )
            ...
            if need_to_update_template and shopify_template:
                shopify_template = self.create_or_update_shopify_template(
                    template_vals, len(variant_data), shopify_template
                )
                need_to_update_template = False

            # instance.shopify_pricelist_id.set_product_price_ept(shopify_product.product_id, variant.get("price"))
    ...
    def check_sku_barcode(self, sku, barcode, name, variant_id, match_by):
        ...
        if match_by == "sku" and not sku:
            message = "Product {} have no sku having variant id {}.".format(name, variant_id)
        elif match_by == "barcode" and not barcode:
            message = "Product {} have no barcode having variant id {}.".format(name, variant_id)
        elif match_by == "sku_or_barcode" and not sku and not barcode:
            message = "Product {} have no sku and barcode having variant id {}.".format(name, variant_id)
        return message

    def create_or_update_shopify_template_and_variant(
        self,
        template_vals,
        variant_vals,
        variant_length,
        ...
        if update_template:
            shopify_template = self.create_or_update_shopify_template(
                template_vals, variant_length, shopify_template, odoo_product
            )
    ...
    def check_for_new_variant(self, odoo_template, shopify_attributes, variant_data, shopify_template, variant_vals):
        ...
        if len(odoo_attribute_lines) != len(shopify_attributes):
            message = "Product {} has tried to add new attribute for sku {} in Odoo.".format(shopify_template.name, sku)
            return message
        ...
        if not odoo_product:
            message = "Unknown error occurred. Couldn't find product {} with sku {} in Odoo.".format(
                shopify_template.title,
                sku,
            )
            return message
        ...
        for shopify_attribute in shopify_attributes:
            ...
            if attribute_value:
                ...
                attribute_line_vals = (
                    0,
                    False,
                    {"attribute_id": attribute.id, "value_ids": [[6, False, attribute_value.ids]]},
                )
                attribute_line_data.append(attribute_line_vals)

        return attribute_line_data

    def create_or_update_shopify_variant(
        self, variant_vals, shopify_product, shopify_template=False, odoo_product=False
    ):
        """
        ...
        if not shopify_product and shopify_template and odoo_product:
            variant_vals.update(
                {"name": odoo_product.name, "product_id": odoo_product.id, "shopify_template_id": shopify_template.id}
            )
        ...
        for image in template_data.get("images", {}):
            if image.get("src"):
                ...
                if not variant_ids:
                    ...
                    shopify_product_images += self.sync_simple_product_images(
                        shopify_image_id, existing_common_template_images, url
                    )
                else:
                    # The below method is used to sync variable(variation) product images.
                    shopify_product_images += self.sync_variable_product_images(
                        shopify_image_id, url, variant_ids, is_template_image_set
                    )

        all_shopify_product_images = shopify_product_image_obj.search([("shopify_template_id", "=", self.id)])
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if key in existing_common_template_images.keys():
                        shopify_product_image = self.create_shopify_layer_image(
                            shopify_image_id, existing_common_template_images, key, False
                        )
                    else:
                        ...
                        shopify_product_image = self.search_shopify_product_images(
                            self.id, False, False, common_product_image.id
                        )
    ...
    def search_shopify_product_images(
        self, shopify_template_id, shopify_variant_id, shopify_image_id, common_product_image
    ):
    ...
    def sync_variable_product_images(self, shopify_image_id, url, variant_ids, is_template_image_set):
        ...
        for shopify_product in shopify_products:
            ...
            shopify_product_image = self.search_shopify_product_images(
                False, shopify_product.id, shopify_image_id, False
            )
            if not shopify_product_image:
                try:
                    response = requests.get(url, stream=True, verify=True, timeout=10)
                    if response.status_code == 200:
                        ...
                        if key in existing_common_variant_images.keys():
                            shopify_product_image = self.create_shopify_layer_image(
                                shopify_image_id, existing_common_variant_images, key, shopify_product
                            )
                        else:
                            ...
                            shopify_product_image = self.search_shopify_product_images(
                                self.id, shopify_product.id, False, common_product_image.id
                            )
                            if shopify_product_image:
                                shopify_product_image.shopify_image_id = shopify_image_id
                except Exception:
    ...
    def shopify_prepare_template_dic(self, template_data, instance, product_category):
        ...
        template_dict = {
            ...
            "created_at": self.convert_shopify_date_into_odoo_format(template_data.get("created_at")),
            "updated_at": self.convert_shopify_date_into_odoo_format(template_data.get("updated_at")),
            "published_at": self.convert_shopify_date_into_odoo_format(template_data.get("published_at")),
    ...
    def shopify_search_odoo_product_variant(self, shopify_instance, variant_id, product_sku, barcode):
        ...
        shopify_product = shopify_product_obj.search(
            [("variant_id", "=", variant_id), ("shopify_instance_id", "=", shopify_instance.id)], limit=1
        )
        ...
        if shopify_instance.shopify_sync_product_with == "sku" and product_sku:
            ...
            if not shopify_product:
                shopify_product = shopify_product_obj.search(
                    [("product_id.barcode", "=", barcode), ("shopify_instance_id", "=", shopify_instance.id)], limit=1
                )
            if not shopify_product:
    ...
    def create_or_update_shopify_template(
        self, template_dict, variant_length, shopify_template, odoo_product=False, odoo_template=False
    ):
        """
    ...
    def is_product_importable(self, template_data, instance, odoo_product, shopify_product):
            ...
            if barcode:
                ...
                if duplicate_barcode:
                    ...
                    if (
                        ...
                        message = "Duplicate barcode found in Product: {} and ID: {}.".format(template_title, template_id)
                        return message

        total_shopify_sku = len(set(shopify_skus))
        if len(shopify_skus) != total_shopify_sku:
            message = "Duplicate SKU found in Product {} and ID: {}.".format(template_title, template_id)
            return message
        ...
        if (
            not odoo_product
            and not shopify_product
            and instance.shopify_sync_product_with in ["barcode", "sku_or_barcode"]
        ):
