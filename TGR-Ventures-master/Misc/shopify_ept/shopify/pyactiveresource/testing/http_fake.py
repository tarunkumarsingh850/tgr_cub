...
def create_response_key(method, url, request_headers):
    ...
    return str(
        (
            method,
            urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, parsed.path, query, parsed.fragment)),
...
class TestHandler(urllib.request.HTTPHandler, urllib.request.HTTPSHandler):
    ...
    @classmethod
    def respond_to(cls, method, path, request_headers, body, code=200, response_headers=None):
        ...
        Returns:
            None
        """
        key = create_response_key(method, urllib.parse.urljoin(cls.site, path), request_headers)
    ...
    def do_open(self, http_class, request, **http_conn_args):
        ...
        if self._response_map:
            key = create_response_key(request.get_method(), request.get_full_url(), request.headers)
            if str(key) in self._response_map:
                ...
                raise Error(
                    "Unknown request %s %s"
                    "\nrequest:%s\nresponse_map:%s"
                    % (request.get_method(), request.get_full_url(), str(key), pformat(list(self._response_map.keys())))
                )
