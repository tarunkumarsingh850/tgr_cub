...
class FakeConnection(object):
    ...
    def respond_to(self, method, path, headers, data, body, response_headers=None):
        ...
        if response_headers is None:
            response_headers = {}
        self._request_map.setdefault(method, []).append(((path_only, query, headers, data), (body, response_headers)))
    ...
    def _lookup_response(self, method, path, headers, data):
        ...
        raise Error("Invalid or unknown request: {} {}\n{}".format(path, headers, data))

    def get(self, path, headers=None):
        """Perform an HTTP get request."""
        return self.format.decode(self._lookup_response("get", path, headers, None).body)
