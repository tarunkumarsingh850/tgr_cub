...
UNCOUNTABLES = ["equipment", "information", "rice", "money", "species", "series", "fish", "sheep"]
...
SERIALIZERS = [
    ...
    {"type": six.integer_types, "method": lambda value: ("integer", six.text_type(value))},
]
if six.PY2:
    SERIALIZERS.append({"type": str, "method": lambda value: (None, unicode(value, "utf-8"))})
else:
    SERIALIZERS.append(
        {"type": bytes, "method": lambda value: ("base64Binary", base64.b64encode(value).decode("ascii"))}
    )

DEFAULT_SERIALIZER = {"type": object, "method": lambda value: (None, six.text_type(value))}
...
def camelize(word):
    ...
    Returns:
        The modified string.
    """
    return "".join(w[0].upper() + w[1:] for w in re.sub("[^A-Z^a-z^0-9^:]+", " ", word).strip().split(" "))
...
def xml_to_dict(xmlobj, saveroot=True):
    ...
    if element_type == "array":
        ...
        if saveroot:
            return element_containers.ElementDict(element_list_type, {element_list_type: return_list})
        else:
            return return_list
    elif element.get("nil") == "true":
        return None
    elif element_type in ("integer", "datetime", "date", "decimal", "double", "float") and not element.text:
        return None
        ...
        if date_parse:
            ...
            try:
                timestamp = calendar.timegm(time.strptime(element.text, "%Y-%m-%dT%H:%M:%S+0000"))
        ...
        if element_type:
            attributes = element_containers.ElementDict(underscore(element.get("type", "")), element.items())
        else:
            attributes = element_containers.ElementDict(singularize(element.tag.replace("-", "_")), element.items())
        for child in element.getchildren():
        ...
        return element_containers.ElementDict(element.tag.replace("-", "_"), element.items())
    else:
        return element.text
