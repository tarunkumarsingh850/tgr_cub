...
class ResourceMeta(type):
    ...
    def connection(cls):
        ...
        if super_class == object or "_connection" in cls.__dict__:
            if cls._connection is None:
                cls._connection = connection.Connection(cls.site, cls.user, cls.password, cls.timeout, cls.format)
    ...
    password = property(get_password, set_password, None, "A password for HTTP Basic Auth.")
    ...
    timeout = property(get_timeout, set_timeout, None, "Socket timeout for HTTP operations")
    ...
    format = property(get_format, set_format, None, "A format object for encoding/decoding requests")
    ...
    plural = property(get_plural, set_plural, None, "The plural name of this object type.")
    ...
    singular = property(get_singular, set_singular, None, "The singular name of this object type.")
    ...
    primary_key = property(
        get_primary_key, set_primary_key, None, "Name of attribute that uniquely identies the resource"
    )
...
class ActiveResource(six.with_metaclass(ResourceMeta, object)):
    ...
    def _find_single(cls, id_, **kwargs):
        ...
        prefix_options, query_options = cls._split_options(kwargs)
        path = cls._element_path(id_, prefix_options, query_options)
        return cls._build_object(cls.connection.get_formatted(path, cls.headers), prefix_options)
    ...
    def _element_path(cls, id_, prefix_options=None, query_options=None):
        ...
        """
        return "{prefix}/{plural}/{id}.{format}{query}".format(
    ...
    def _collection_path(cls, prefix_options=None, query_options=None):
        ...
        """
        return "{prefix}/{plural}.{format}{query}".format(
            prefix=cls._prefix(prefix_options),
            plural=cls._plural,
            format=cls.format.extension,
            query=cls._query_string(query_options),
    ...
    def _custom_method_collection_url(cls, method_name, options):
        ...
        path = "{prefix}/{plural}/{method_name}.{format}{query}".format(
            prefix=cls._prefix(prefix_options),
            plural=cls._plural,
        ...
        try:
            ...
            if self.id:
                response = self.klass.connection.put(
                    self._element_path(self.id, self._prefix_options), self.klass.headers, data=self.encode()
                )
            else:
                response = self.klass.connection.post(
                    self._collection_path(self._prefix_options), self.klass.headers, data=self.encode()
                )
    ...
    def _id_from_response(self, response):
        ...
        Returns:
           An id string.
        """
        match = re.search(r"\/([^\/]*?)(\.\w+)?$", response.get("Location", response.get("location", "")))
        if match:
            try:
    ...
    def destroy(self):
        ...
        Returns:
            None
        """
        self.klass.connection.delete(self._element_path(self.id, self._prefix_options), self.klass.headers)
    ...
    def __repr__(self):
        return "{}({})".format(self._singular, self.id)
    ...
    def _custom_method_element_url(self, method_name, options):
        ...
        path = "{prefix}/{plural}/{id}/{method_name}.{format}{query}".format(
    ...
    def _custom_method_new_element_url(self, method_name, options):
        ...
        prefix_options, query_options = self._split_options(options)
        prefix_options.update(self._prefix_options)
        path = "{prefix}/{plural}/new/{method_name}.{format}{query}".format(
            prefix=self.klass.prefix(prefix_options),
            plural=self._plural,
            method_name=method_name,
            format=self.klass.format.extension,
