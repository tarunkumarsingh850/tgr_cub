...
class ResourceNotFound(ClientError):
    ...
    def __init__(self, response=None, message=None):
        if response is not None and message is None:
            message = "{}: {}".format(response.msg, response.url)
...
class Response(object):
    ...
    def __eq__(self, other):
        if isinstance(other, Response):
            return (self.code, self.body, self.headers) == (other.code, other.body, other.headers)
        return False

    def __repr__(self):
        return 'Response(code={}, body="{}", headers={}, msg="{}")'.format(self.code, self.body, self.headers, self.msg)
    ...
    def from_httpresponse(cls, response):
        ...
        Returns:
            A Response object.
        """
        return cls(response.code, response.read(), dict(response.headers), response.msg, response)
...
class Connection(object):
    ...
    def __init__(self, site, user=None, password=None, timeout=None, format=formats.JSONFormat):
        ...
        if self.user or self.password:
            self.auth = base64.b64encode(("{}:{}".format(self.user, self.password)).encode("utf-8")).decode("utf-8")
        else:
            self.auth = None
    ...
    def _open(self, method, path, headers=None, data=None):
        ...
        if request.headers:
            header_string = "\n".join([":".join((k, v)) for k, v in six.iteritems(request.headers)])
        ...
        try:
            ...
            self.log.debug('Response(code=%d, headers=%s, msg="%s")', response.code, response.headers, response.msg)
        finally:
            if http_response:
                http_response.close()
            if self.timeout and not _urllib_has_timeout():
