...
class Main(http.Controller):
    @http.route(
        ["/shopify_odoo_webhook_for_product_update", "/shopify_odoo_webhook_for_product_delete"],
        csrf=False,
        auth="public",
        type="json",
    )
    def create_update_delete_product_webhook(self):
        ...
        shopify_template = (
            ...
            .search([("shopify_tmpl_id", "=", res.get("id")), ("shopify_instance_id", "=", instance.id)], limit=1)
        )
    ...
    @http.route(
        ["/shopify_odoo_webhook_for_customer_create", "/shopify_odoo_webhook_for_customer_update"],
        csrf=False,
        auth="public",
        type="json",
    )
    ...
    def order_create_or_update_webhook(self):
        ...
        if sale_order.sudo().search_read(
            [
            ...
            sale_order.sudo().with_context({"is_new_order": True}).process_shopify_order_via_webhook(res, instance)
        return
    ...
    def get_basic_info(self, route):
        ...
        webhook = (
            ...
            .sudo()
            .search([("delivery_url", "ilike", route), ("instance_id", "=", instance.id)], limit=1)
        )

        if not instance.active or not webhook.state == "active":
            _logger.info(
                "The method is skipped. It appears the instance:%s is not active or that "
                "the webhook %s is not active.",
                instance.name,
