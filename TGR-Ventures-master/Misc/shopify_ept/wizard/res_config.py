...
class ShopifyInstanceConfig(models.TransientModel):
    ...
    shopify_api_key = fields.Char(
        "API Key", required=True, help="Shopify API Key. You can find " "it under Shopify store control " "panel."
    )
    shopify_password = fields.Char(
        "Password", required=True, help="Shopify API Password. You can " "find it under Shopify store control panel"
    )
    shopify_shared_secret = fields.Char(
        "Secret Key",
        required=True,
        help="Shopify API Shared Secret. You can find it " "under Shopify store control panel",
    )
    shopify_host = fields.Char(
        "Host", required=True, help="Add your shopify store URL, for example, https://my-shopify-store.myshopify.com"
    )
    shopify_company_id = fields.Many2one(
        "res.company", string="Instance Company", help="Orders and Invoices will be generated of this company."
    )
    ...
    def create_pricelist(self, shop_currency):
        ...
        if not pricelist:
            pricelist = pricelist_obj.create(
                {"name": price_list_name, "currency_id": currency_id.id, "company_id": self.shopify_company_id.id}
            )
    ...
    def reset_credentials(self):
        ...
        if (
            instance.shopify_api_key == self.shopify_api_key
            or instance.shopify_password == self.shopify_password
            or instance.shopify_shared_secret == self.shopify_shared_secret
        ):
        ...
        if context.get("test_connection"):
            return {
                ...
                "params": {"title": "Shopify", "message": "New Credentials are working properly!", "sticky": False},
            }
        instance.write(vals)
...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_shopify_default_financial_statuses(self):
        if self._context.get("default_shopify_instance_id", False):
            financial_status_ids = (
                self.env["sale.auto.workflow.configuration.ept"]
                .search([("shopify_instance_id", "=", self._context.get("default_shopify_instance_id", False))])
                .ids
            )
    ...
    shopify_company_id = fields.Many2one(
        "res.company", string="Shopify Instance Company", help="Orders and Invoices will be generated of this company."
    )
    shopify_warehouse_id = fields.Many2one(
        "stock.warehouse", string="Shopify Warehouse", domain="[('company_id', '=',shopify_company_id)]"
    )
    ...
    shopify_is_use_default_sequence = fields.Boolean(
        "Use Odoo Default Sequence in Shopify Orders",
    ...
    shopify_date_deadline = fields.Integer(
        "Deadline Lead Days for Shopify", default=1, help="its add number of  days in schedule activity deadline date "
    )
    is_shopify_create_schedule = fields.Boolean(
        "Create Schedule activity ? ",
        default=False,
        help="If checked, Then Schedule Activity create on order dara queues" " will any queue line failed.",
    )
    shopify_sync_product_with_images = fields.Boolean(
        "Shopify Sync/Import Images?", default=False, help="Check if you want to import images along with products"
    )
    create_shopify_products_webhook = fields.Boolean(
        "Manage Shopify Products via Webhooks",
    ...
    create_shopify_customers_webhook = fields.Boolean(
        "Manage Shopify Customers via Webhooks",
    ...
    create_shopify_orders_webhook = fields.Boolean(
        "Manage Shopify Orders via Webhooks",
    ...
    last_date_order_import = fields.Datetime(
        string="Import Unshipped Orders ", help="Last date of sync orders from Shopify to Odoo"
    )
    last_shipped_order_import_date = fields.Datetime(
        string="Import Shipped Orders", help="Last date of sync shipped orders from Shopify to Odoo"
    )
    shopify_last_date_customer_import = fields.Datetime(
        string="Import Customers", help="it is used to store last import customer date"
    )
    shopify_last_date_update_stock = fields.Datetime(
        string="Update Stock", help="it is used to store last update inventory stock date"
    )
    shopify_last_date_product_import = fields.Datetime(
        string="Import Products", help="it is used to store last import product date"
    )
    ...
    shopify_payout_last_date_import = fields.Date(
        string="Import Payout Reports", help="It is used to store last update shopify payout report"
    )
    shopify_financial_status_ids = fields.Many2many(
        "sale.auto.workflow.configuration.ept",
        "shopify_sale_auto_workflow_conf_rel",
        "financial_onboarding_status_id",
        "workflow_id",
    ...
    shopify_set_sales_description_in_product = fields.Boolean(
        "Use Sales Description of Odoo Product for shopify",
    ...
    def onchange_shopify_instance_id(self):
        ...
        if instance:
            ...
            self.shopify_invoice_tax_account_id = (
                instance.invoice_tax_account_id and instance.invoice_tax_account_id.id or False
            )
            self.shopify_credit_tax_account_id = (
                instance.credit_tax_account_id and instance.credit_tax_account_id.id or False
            )
    ...
    def execute(self):
        ...
        if instance:
            ...
            values["invoice_tax_account_id"] = (
                self.shopify_invoice_tax_account_id and self.shopify_invoice_tax_account_id.id or False
            )
            values["credit_tax_account_id"] = (
                self.shopify_credit_tax_account_id and self.shopify_credit_tax_account_id.id or False
            )
            values["notify_customer"] = self.shopify_notify_customer
            values["shopify_activity_type_id"] = (
                self.shopify_activity_type_id and self.shopify_activity_type_id.id or False
            )
    ...
    def shopify_res_config_view_action(self, view_id):
        ...
        :return: True
        """
        action = self.env["ir.actions.actions"]._for_xml_id("shopify_ept.action_shopify_instance_config")
        action_data = {
            "view_id": view_id.id,
