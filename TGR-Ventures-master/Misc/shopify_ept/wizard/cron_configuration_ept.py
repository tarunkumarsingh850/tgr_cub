...
class ShopifyCronConfigurationEpt(models.TransientModel):
    ...
    shopify_order_auto_import = fields.Boolean(
        "Import Order", default=False, help="Check if you want to automatically Import Orders from Shopify to" " Odoo."
    )
    ...
    shopify_import_order_user_id = fields.Many2one(
        "res.users", string="User for Import Order", help="User for Import Order", default=lambda self: self.env.user
    )

    # Auto cron for Import Shipped Order
    shopify_shipped_order_auto_import = fields.Boolean(
        "Import Shipped Order",
        default=False,
        help="Check if you want to automatically Import Shipped Orders from Shopify to" " Odoo.",
    )
    shopify_import_shipped_order_interval_number = fields.Integer(
        "Interval Number for Import Shipped Order", help="Repeat every x."
    )
    shopify_import_shipped_order_interval_type = fields.Selection(
        [("minutes", "Minutes"), ("hours", "Hours"), ("days", "Days"), ("weeks", "Weeks"), ("months", "Months")],
        "Interval Unit for Import Shipped Order",
    )
    shopify_import_shipped_order_next_execution = fields.Datetime(
        "Next Execution for Import Shipped Order", help="Next Execution for Import Shipped Order"
    )
    shopify_import_shipped_order_user_id = fields.Many2one(
        "res.users",
        string="User for Import Shipped Order",
    ...
    shopify_order_status_next_execution = fields.Datetime(
        "Next Execution for Update Order Status", help="Next Execution for Update Order Status"
    )
    ...
    shopify_payout_import_user_id = fields.Many2one(
        "res.users", string="Payout Import User", help="User", default=lambda self: self.env.user
    )
    ...
    @api.constrains(
        "shopify_inventory_export_interval_number",
        "shopify_payout_import_interval_number",
        "shopify_import_order_interval_number",
        "shopify_order_status_interval_number",
    )
    ...
    def setup_shopify_import_shipped_order_cron(self, instance):
        ...
        """
        try:
            cron_exist = self.env.ref("shopify_ept.ir_cron_shopify_auto_import_shipped_order_instance_%d" % instance.id)
        except:
            cron_exist = False
        if self.shopify_shipped_order_auto_import:
            ...
            vals.update(
                {
                    "nextcall": self.shopify_import_shipped_order_next_execution
                    or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def setup_shopify_payout_auto_import_payout_report_cron(self, instance):
        ...
        """
        try:
            cron_exist = self.env.ref("shopify_ept.ir_cron_auto_import_payout_report_instance_%d" % instance.id)
        except:
            cron_exist = False
    ...
    def setup_shopify_payout_report_cron(self, instance):
        ...
        if self.shopify_auto_process_bank_statement:
            ...
            try:
                cron_exist = self.env.ref("shopify_ept.ir_cron_auto_process_bank_statement_instance_%d" % instance.id)
            except:
                cron_exist = False
            if cron_exist:
                cron_exist.write({"active": False})
    ...
    def create_ir_module_data(self, name, new_cron):
        ...
        self.env["ir.model.data"].create(
            {"module": "shopify_ept", "name": name, "model": "ir.cron", "res_id": new_cron.id, "noupdate": True}
        )
    ...
    def action_shopify_open_cron_configuration_wizard(self):
        ...
        if instance:
            action.get("context").update({"default_shopify_instance_id": instance.id, "is_instance_exists": True})
        return action
