...
class ShopifyCancelRefundOrderWizard(models.TransientModel):
    ...
    notify_by_email = fields.Boolean(
        "Notify By Email ?", default=True, help="Whether to send an email to the customer notifying the cancellation."
    )
    auto_create_credit_note = fields.Boolean(
        "Create Credit Note In ERP", default=False, help="It will create a credit not in Odoo"
    )
        ...
        try:
            ...
            if hasattr(error, "response"):
                if hasattr(error.response, "body") and error.response.code == 422:
                    msg = _(
                        "Error while cancelling the order in shopify :\n%s"
                        % (json.loads(error.response.body.decode()).get("error"))
                    )
                else:
                    msg = _(
                        "Error while cancelling the order in shopify : %s\n%s"
    ...
    def shopify_create_credit_note(self, order_id):
        ...
        if not moves:
            ...
            order_id._cr.commit()
            warning_message = (
                "1.Order cancel in Shopify but unable to create a credit note."
    ...
    def refund_in_shopify(self):
        ...
        for credit_note_id in credit_note_ids:
            ...
            log_lines = self.create_refund_in_shopify(
                orders, credit_note_id, refund_lines_list, model_id, do_not_order_process_ids
            )
            ...
            if restock_type in ["cancel", "return"]:
                ...
                if not shopify_location_id and restock_type == "cancel":
                    ...
                    shopify_location_id = shopify_location_obj.search(
                        [("is_primary_location", "=", True), ("instance_id", "=", shopify_instance.id)]
                    )
                if not shopify_location_id:
                    if restock_type == "cancel":
                        ...
                        log_message = (
                            "Location is not set in order (%s).Unable to refund in shopify.\n You can see "
                    ...
                    log_line = self.env["common.log.lines.ept"].shopify_create_order_log_line(
                        log_message, model_id, False, False, order_ref=order_id.name
                    )
                    mismatch_log_lines.append(log_line.id)
                    do_not_order_process_ids.append(order_id.id)
                    continue
                refund_line_dict.update({"location_id": shopify_location_id.shopify_location_id})
            refund_lines_list.append(refund_line_dict)

        return refund_lines_list, do_not_order_process_ids, mismatch_log_lines

    def create_refund_in_shopify(self, orders, credit_note_id, refund_lines_list, model_id, do_not_order_process_ids):
        ...
        for order in orders:
            ...
            try:
                ...
                log_message = "When creating refund in Shopify for order (%s), issue arrive in " "request (%s)" % (
                    order.name,
                    error,
                )
                log_line = self.env["common.log.lines.ept"].shopify_create_order_log_line(
        ...
        if multi_payments:
            ...
            for order_payment in multi_payments.filtered(
                ...
                transaction = self.preapre_refund_transaction_vals(
                    parent_id, order_payment.refund_amount, order_payment.payment_gateway_id.code
                )
