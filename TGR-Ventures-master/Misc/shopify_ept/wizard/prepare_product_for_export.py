...
class PrepareProductForExport(models.TransientModel):
    ...
    export_method = fields.Selection(
        [("direct", "Export in Shopify Layer"), ("csv", "Export in CSV file"), ("xlsx", "Export in XLSX file")],
        default="direct",
    )
    ...
    def export_direct_in_shopify(self, product_templates):
        ...
        for variant in variants:
            ...
            if (
                ...
                and len(
                    product_template.attribute_line_ids.filtered(lambda x: x.attribute_id.create_variant == "always")
                )
                > 3
            ):
                continue
            ...
            shopify_variant = self.create_or_update_shopify_layer_variant(
                variant, shopify_template_id, shopify_instance, shopify_template, sequence
            )
    ...
    def create_or_update_shopify_layer_template(
        ...
        shopify_template = shopify_template_obj.search(
            [("shopify_instance_id", "=", shopify_instance.id), ("product_tmpl_id", "=", product_template.id)], limit=1
        )

        if not shopify_template:
            shopify_product_template_vals = self.prepare_template_val_for_export_product_in_layer(
    ...
    def preapre_product_data_for_file(self, product_templates):
        ...
        for template in product_templates:
            if (
                ...
                and len(template.attribute_line_ids.filtered(lambda x: x.attribute_id.create_variant == "always")) > 3
            ):
                continue
    ...
    def export_csv_file(self, product_templates):
        ...
        self.write({"choose_file": base64.encodebytes(file_data), "file_name": "Shopify_export_product_"})
    ...
    def create_shopify_template_images(self, shopify_template):
        ...
        for odoo_image in product_template.ept_image_ids.filtered(lambda x: not x.product_id):
            shopify_product_image = shopify_product_image_obj.search_read(
                [("shopify_template_id", "=", shopify_template.id), ("odoo_image_id", "=", odoo_image.id)], ["id"]
            )
            if not shopify_product_image:
                shopify_product_image_list.append(
                    {"odoo_image_id": odoo_image.id, "shopify_template_id": shopify_template.id}
                )
