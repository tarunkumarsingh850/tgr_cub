...
class ShopifyProcessImportExport(models.TransientModel):
    ...
    shopify_instance_ids = fields.Many2many(
        "shopify.instance.ept", "shopify_instance_import_export_rel", "process_id", "shopify_instance_id", "Instances"
    )
    shopify_is_set_price = fields.Boolean(
        string="Set Price ?", help="If is a mark, it set the price with product in the Shopify store.", default=False
    )
    shopify_is_set_stock = fields.Boolean(
        string="Set Stock ?", help="If is a mark, it set the stock with product in the Shopify store.", default=False
    )
    ...
    shopify_is_set_image = fields.Boolean(
        string="Set Image ?", help="If is a mark, it set the image with product in the Shopify store.", default=False
    )
    shopify_is_set_basic_detail = fields.Boolean(
        string="Set Basic Detail ?", help="If is a mark, it set the product basic detail in shopify store", default=True
    )
    ...
    shopify_template_ids = fields.Text(
        string="Template Ids", help="Based on template ids get product from shopify and import in odoo"
    )
    shopify_order_ids = fields.Text(
        string="Order Ids", help="Based on template ids get product from shopify and import products in odoo"
    )
    ...
    skip_existing_product = fields.Boolean(
        string="Do Not Update Existing Products", help="Check if you want to skip existing products."
    )
    csv_file = fields.Binary(help="Select CSV file to upload.")
    file_name = fields.Char(help="Name of CSV file.")
    cron_process_notification = fields.Text(
        string="Shopify Note: ", store=False, help="Used to display that cron will be run after some time"
    )
    is_hide_operation_execute_button = fields.Boolean(
        default=False,
        store=False,
    ...
    import_products_based_on_date = fields.Selection(
        [("create_date", "Create Date"), ("update_date", "Update Date")],
        default="update_date",
        string="Import Based On",
    )
    is_auto_validate_inventory = fields.Boolean(
        default=False,
    ...
    def shopify_execute(self):
        ...
        if self.shopify_operation == "sync_product":
            product_queue_ids = product_data_queue_obj.shopify_create_product_data_queue(
                instance,
                self.import_products_based_on_date,
            ...
            order_queues = order_date_queue_obj.shopify_create_order_data_queues(
                instance, self.orders_from_date, self.orders_to_date, order_type="unshipped"
            )
            if order_queues:
            ...
            order_queues = order_date_queue_obj.shopify_create_order_data_queues(
                instance, self.orders_from_date, self.orders_to_date, order_type="shipped"
            )
            if order_queues:
                queue_ids = order_queues
        ...
        if queue_ids and action_name and form_view_name:
            ...
            if len(queue_ids) == 1:
                action.update(
                    {
                        "view_id": (form_view.id, form_view.name),
                        "res_id": queue_ids[0],
    ...
    def create_customer_data_queues(self, customer_data):
        ...
        if len(customer_data) > 0:
            for customer_id_chunk in split_every(125, customer_data):
                ...
                bus_bus_obj._sendone(
                    self.env.user.partner_id,
                    "simple_notification",
                    {"title": "Shopify Notification", "message": message, "sticky": False, "warning": True},
                )
    ...
    def webhook_customer_create_process(self, res, instance):
        ...
        customer_queue_id = data_queue_obj.search(
            [
        ...
        if customer_queue_id:
            message = "Customer {} added into Queue {}.".format(res.get("first_name"), customer_queue_id.name)
        else:
    ...
    def shopify_selective_product_stock_export(self):
        ...
        for instance in shopify_instances:
            shopify_products = shopify_product_obj.search(
                [("shopify_instance_id", "=", instance.id), ("shopify_template_id", "in", shopify_template_ids)]
            )
    ...
    def update_order_status_cron_action(self, ctx=False):
        ...
        _logger.info(_("Auto cron update order status process start with instance: '%s'"), instance.name)
    ...
    def create_products_from_file(self, file_data):
        ...
        for record in file_data:
            ...
            if not record["PRODUCT_TEMPLATE_ID"] or not record["PRODUCT_ID"] or not record["CATEGORY_ID"]:
                ...
                vals = {"message": message, "model_id": model_id, "log_book_id": log_book_id.id}
                common_log_line_obj.create(vals)
                continue

            shopify_template, shopify_template_id, sequence = self.create_or_update_shopify_template_from_csv(
    ...
    def create_or_update_shopify_template_from_csv(self, instance, record, shopify_template_id, sequence):
        ...
        shopify_template = shopify_product_template.search(
            [("shopify_instance_id", "=", instance.id), ("product_tmpl_id", "=", int(record["PRODUCT_TEMPLATE_ID"]))],
            limit=1,
        )
    ...
    def shopify_check_running_schedulers(self, cron_xml_id):
        ...
        if cron_id and cron_id.sudo().active:
            ...
            if res and res.get("result"):
                self.cron_process_notification = (
                    "This process is also performed by a scheduler, and the next "
