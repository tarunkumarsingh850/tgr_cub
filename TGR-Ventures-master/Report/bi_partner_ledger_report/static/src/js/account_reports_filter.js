odoo.define('account_reports_filter.account_report_generic', function (require) {
'use strict';
    ...
    accountReportsWidget.include({
        custom_events: {
            'value_changed': function(ev) {
                var self = this;
                self.report_options.zone_ids = ev.data.zone_ids;
                self.report_options.district_ids = ev.data.district_ids;
                ...
                self.report_options.user_id = ev.data.user_id;
                return self.reload().then(function () {
                    ...
                    self.$searchview_buttons.find('.salesperson_filter').click();
                });
            },
        },
        render_searchview_buttons: function() {
            this._super();
            // zones & districts filter
            if (this.report_options.user_id) {
                if (!this.M2MSalespersonFilters) {
                    ...
                    if (this.report_options.user_id) {
                        fields['user_id'] = {
                            ...
                            modelName: 'res.users',
                            value: this.report_options.user_id.map(Number),
                        };
                    }
                    if (!_.isEmpty(fields)) {
                        this.M2MSalespersonFilters = new M2MSalespersonFilters(this, fields);
                        this.M2MSalespersonFilters.appendTo(this.$searchview_buttons.find('.js_saleperson_m2o'));
                    }
                } else {
                    this.$searchview_buttons.find('.js_saleperson_m2o').append(this.M2MSalespersonFilters.$el);
                }
            }
        },
    });
