...
class AccountingReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
            ...
            new_move_ids = (
                ...
                .search(
                    [
                        ...
                        ("move_id.move_type", "=", "out_invoice"),
                        ("tax_tag_ids", "in", account_tag_id.ids),
                        ("move_id.partner_country_id.code", "=", country_code),
                        ("move_id", "not in", move_ids.ids),
                    ]
                )
                .mapped("move_id")
            )

            if not tax and new_move_ids:
                tax = new_move_ids[0].invoice_line_ids[0].tax_ids[0]

            untaxed_amount = 0
            amount_tax = 0
            amount_total = 0
            for record in move_ids:
                untaxed_amount += record.amount_untaxed_signed
                amount_tax += record.amount_tax_signed
                amount_total += record.amount_total_signed

            for line in new_move_ids:
                untaxed_amount += line.amount_untaxed_signed
                amount_tax += line.amount_tax_signed
                amount_total += line.amount_total_signed
        ...
        for country_code in eu_country_codes:
            ...
            # new_move_ids =  move_ids = self.env['account.move.line'].search([
            #         ("date", ">=", start_date),
            #         ("date", "<=", end_date),
            #         ("company_id", "=", company_id.id),
            #         ('parent_state','=','posted'),
            ...
            # for line in  new_move_ids:
            #     untaxed_amount += line.amount_untaxed
