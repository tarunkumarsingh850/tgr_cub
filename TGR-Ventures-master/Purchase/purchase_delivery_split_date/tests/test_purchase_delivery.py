...
class TestDeliverySingle(TransactionCase):
    ...
    def test_check_multiple_dates(self):
        ...
        self.assertEqual(
            len(self.po.picking_ids),
            2,
            "There must be 2 pickings for the PO when confirmed. %s found" % len(self.po.picking_ids),
        )
    ...
    def test_group_multiple_picking_same_date(self):
        ...
        self.po.order_line[0].date_planned = self.date_later
        self.po.button_confirm()
        moves = self.env["stock.move"].search([("purchase_line_id", "in", self.po.order_line.ids)])
    ...
    def test_purchase_line_date_change_split_picking(self):
        ...
        self.assertEqual(
            move1.picking_id,
            move2.picking_id,
            "If I change the other line to the same date as the first, " "both moves must be in the same picking",
        )
    ...
    def test_purchase_line_created_afer_confirm(self):
        ...
        new_date = "2016-01-30"
        moves_before = self.env["stock.move"].search([("purchase_line_id", "in", self.po.order_line.ids)])
        ...
        moves_after = self.env["stock.move"].search([("purchase_line_id", "in", self.po.order_line.ids)])
