...
class TestDeliverySingle(TransactionCase):
    def setUp(self):
        ...
        self.l2 = self.env["stock.location"].create({"location_id": self.l1.id, "name": "Shelf 1", "usage": "internal"})
    ...
    def test_check_multiple_dates(self):
        ...
        self.assertEqual(
            len_pickings,
            2,
            "There must be 2 pickings for the PO when confirmed. %s found" % len_pickings,
        )
        ...
        l2_picking = self.po.picking_ids.filtered(lambda p: p.location_dest_id == self.l2)
        self.assertEqual(len(l2_picking), 0, "There must be 0 picking for location Shelf 1")
        l1_picking = self.po.picking_ids.filtered(lambda p: p.location_dest_id == self.l1)
        self.assertEqual(len(l1_picking), 2, "There must be 2 pickings for location Stock")
    ...
    def test_check_multiple_locations_multiple_dates(self):
        ...
        l2_picking = self.po.picking_ids.filtered(lambda p: p.location_dest_id == self.l2)
        self.assertGreaterEqual(len(l2_picking), 1, "There must be 1 picking for location Shelf 1")
        l1_picking = self.po.picking_ids.filtered(lambda p: p.location_dest_id == self.l1)
        self.assertGreaterEqual(len(l1_picking), 2, "There must be 2 or more pickings for location Stock")
    ...
    def test_check_multiple_locations_multiple_dates_02(self):
        ...
        default_location_picking = self.po.picking_ids.filtered(
            lambda p: p.location_dest_id == self.po.picking_type_id.default_location_dest_id
        )
        self.assertGreaterEqual(
    ...
    def test_modify_confirmed(self):
        ...
        self.assertEqual(self.po.order_line[0].move_ids.mapped("location_dest_id"), self.l1)
